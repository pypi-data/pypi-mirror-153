...
class Sampling:
    ...
    def sample(self, ncells=10000, pop_fp=None, sim_fp=None, cache=True, return_data=False):
        ...
        if cache:
            ...
            cells_meta.to_csv(os.path.join(self.metadata_dir,
                                           'cells_metadata.csv.bz2'),
                              compression='bz2', index=False)
        
        if return_data:
            fp = os.path.join(self.metadata_dir, 'gene_expression.csv.bz2')
    ...
    def downsampling(self, realcount, sim_counts, lib_sizes, cache_size=100000):
        ...
        gene_expr_fp = os.path.join(self.metadata_dir, 'gene_expression.csv.bz2')
        
        if len(lib_sizes) < cache_size:
            cache_size = len(lib_sizes)
        
        for index in tqdm(range(len(lib_sizes))):
            lib_size = lib_sizes[index]
            cpm = sim_counts.iloc[index] / sim_counts.iloc[index].sum()
            cpm = self.calc_cpm(cpm.values, real_cpm)
            gene_expr.append(np.random.multinomial(lib_size, cpm))
            
            if cache_size == gene_expr:
                gene_expr = pd.DataFrame(data=[np.random.multinomial(lib_size, cpm)],
                                         columns=sim_cols, dtype=np.int16)
                
                self.save_dataframe(gene_expr, gene_expr_fp)
                gene_expr = []
    ...
    def calc_cpm(self, sim_cpm, real_cpm):
        
        if self.map_reference_cpm:
            # sort sim counts data via least to greatest
            sim_probs = np.random.uniform(size=len(sim_cpm))
            sim_cpm = np.quantile(real_cpm, sim_probs)
            sim_cpm = sim_cpm / np.sum(sim_cpm)
