...
def context():
    ...
    required = ['CrossPeaks_steps',
                'peak_lower_threshold', 'CrossPeaks_sliced_bins', 'NSIDE',
                'scales', 'selected_scales', 'min_count', 'SNR_peaks',
                'max_SNR']
...
def CrossPeaks(map_w, weights, ctx):
    """
    ...
    peaks2 = np.all(np.reshape(map_w[n1], (-1, 8))
    ...
    peak_vals = peak_vals[peak_vals / sig <= ctx['max_SNR']]

    # edge case (nothing found)
    if len(peak_vals) == 0:
        return np.full(ctx['Peaks_steps'] + 1, np.nan)
    # Binning for values
    if len(peak_vals) > (ctx['min_count'] * 200.):
        minimum = np.max(np.partition(
            peak_vals, ctx['min_count'])[:ctx['min_count']])
        maximum = np.min(np.partition(
            peak_vals, -ctx['min_count'])[-ctx['min_count']:])
    else:
        maximum = -1.
        minimum = 1.
    if maximum < minimum:
        minimum = np.min(peak_vals)
        maximum = np.max(peak_vals)
    peak_bins = np.linspace(
        minimum, maximum, ctx['Peaks_steps'] - 1)

    peak_vals = np.histogram(peak_vals, bins=peak_bins)[0]

    # first and last bins indicate maximum and
    # minmum of the kappa range and std
    peak_vals = np.hstack((minimum, peak_vals, maximum, np.asarray([sig])))
...
def slice(ctx):
    ...
    n_bins_sliced = ctx['CrossPeaks_sliced_bins']
...
def decide_binning_scheme(data, meta, bin, ctx):
    num_of_scales = len(ctx['scales'])
    n_bins_original = ctx['CrossPeaks_steps']
    n_bins_sliced = ctx['CrossPeaks_sliced_bins']
