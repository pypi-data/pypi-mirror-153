...
class Search():
    ...
    def add_new_class_paramfeature(self, solution):
        ...
        ii = np.random.choice(len(class_params_spec), 1)
    ...
    def add_new_member_paramfeature(self, solution):
        ...
        ii = np.random.choice(len(member_params_spec), 1)
        ...
        if len(member_i) > 0:
            ...
            if len(new_params) > 0:
                new_param = np.random.choice(new_params, 1)
                # TODO? remove redundant asvars ?
                new_member_spec = np.sort(np.union1d(member_i, new_param))
            # TODO! consider randvars
    ...
    def improvise_harmony(self, HCR_max, HCR_min, PR_max, PR_min, har_mem,
        ...
        while itr < max_itr:
            ...
            if itr == max_itr:  # Plot on final iteration
                ...
                all_bic_points = [har_mem_iteration_order[x]['bic'] for x in valid_idx]
                all_val_points = [har_mem_iteration_order[x]['MAE'] for x in valid_idx]
