...
class Search():
                ...
                if all(v for v in sig <= self.p_val):
                    ...
                    while any([v for v in sig if self.p_val]):
                        ...
                        if rem_alvars:
                            ...
                            if conv:
                                sol = Solution(bic=bic, MAE=val, asvars=asvars,
                                               isvars=isvars,
                                               randvars=randvars,
    ...
    def assess_sol(self, solution, har_mem, seed):
        ...
        seen = set()
        seen_add = seen.add
        new_hm = [x for x in har_mem if tuple([x['bic'], x['MAE']]) not in seen and
                  not seen_add(tuple([x['bic'], x['MAE']]))]
    ...
    def best_features(self, har_mem):
        ...
        if HM['class_params_spec'] is not None:
            best_class_params_spec = HM[class_params_spec].deepcopy()
    ...
    def improvise_harmony(self, HCR_max, HCR_min, PR_max, PR_min, har_mem,
        ...
        while itr < max_itr:
            ...
            if itr == max_itr:  # Plot on final iteration
                valid_idx = [ii for ii, har_mem_ii in enumerate(har_mem) if np.abs(har_mem_ii['bic']) != 1000000]
                all_bic_points = [har_mem[x]['bic'] for x in range(len(har_mem))]
                all_val_points = [har_mem[x]['MAE'] for x in range(len(har_mem))]
    ...
    def get_fronts(self, HM):
        ...
        for i in range(len(HM)):
            ...
            for j in range(len(HM)):
                if i != j:
                    dominance = self.check_dominance([HM[i]['bic'], HM[i]['MAE']],
                                                     [HM[j]['bic'], HM[j]['MAE']])
                    if dominance:
                        sp_i.append(j)
                    else:
                        dominance = self.check_dominance([HM[j]['bic'], HM[j]['MAE']],
                                                         [HM[i]['bic'], HM[i]['MAE']])
                        if dominance:
                            np_i += 1
            si.update({i: sp_i})
        ...
        for v in Fronts.values():
            for l in v:
                if v[0] == l or v[-1] == l:
                    ...
                    dis = abs(q_dis.get(l) + ((HM[v[v.index(l)+1]]['MAE'] -
                                               HM[l]['MAE'])
                                               / (max(HM[x]['MAE'] for x
                                                     in range(len(HM))) -
                                                 min(HM[x]['MAE'] for x in
                                                     range(len(HM))))))
                    q_dis.update({l: dis})
