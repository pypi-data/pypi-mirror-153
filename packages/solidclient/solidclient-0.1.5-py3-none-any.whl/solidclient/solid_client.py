...
_ISSUER = os.environ.get("IDENTITY_PROVIDER_BASE_URL", "https://oak-identity-provider-oak-develop.test.services.jtech.se/")
_POD_PROVIDER = os.environ.get("POD_PROVIDER_BASE_URL", "https://oak-pod-provider-oak-develop.test.services.jtech.se/")
...
class SolidSession:
    ...
    def provider_info(self):
        return requests.get(_ISSUER + ".well-known/openid-configuration",
                            verify='/mnt/extra-addons/case_management/docker/cert.pem').json()

    @cached_property
    def solid_metadata(self):
        return requests.get(_ISSUER + ".well-known/solid",
                            verify='/mnt/extra-addons/case_management/docker/cert.pem').json()

    @cached_property
    def idp_info(self):
        return requests.get(_ISSUER + "idp/",
                            verify='/mnt/extra-addons/case_management/docker/cert.pem').json().get('controls')
    ...
    def send_request(self, uri, method, data=None, content_type="text/turtle"):
        ...
        if method == "GET":
            resp = requests.get(uri, headers=headers, verify='/mnt/extra-addons/case_management/docker/cert.pem')
            auth_url = self.get_authorization_endpoint_uri()
            return SolidResponse(resp, self, auth_url=auth_url)

        elif method == "DELETE":
            resp = requests.delete(uri, headers=headers, verify='/mnt/extra-addons/case_management/docker/cert.pem')
        ...
        # Is POST needed? Just PUT 4Head
            ...
            resp = requests.post(uri, data=data, headers=headers,
                                 verify='/mnt/extra-addons/case_management/docker/cert.pem')
