...
class Resources:
  ...
  def uploadFile(self, newName: str, folderPath: str, localFilePath: str) -> ChiliResponse:
    return self.connector.makeRequest(
      method='post',
      endpoint=f"/resources/assets/items",
      queryParams={'newName':newName, 'folderPath':folderPath},
      json={'xml':'', 'fileData':getBase64String(localFilePath)}
    )
