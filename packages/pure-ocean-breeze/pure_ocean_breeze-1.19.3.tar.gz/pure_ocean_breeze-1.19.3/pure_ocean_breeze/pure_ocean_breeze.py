...
def other_periods_comments_nets(fac,period=None,comments_writer=None,nets_writer=None,sheetname=None,group_num=10):
    '''不同频率下的评价指标，请输入行业市值中性化后的因子值'''
    closes=read_daily(close=1)
    df=al.utils.get_clean_factor_and_forward_returns(fac,closes,quantiles=group_num,periods=(period,))
    fac=fac.stack()
    df=df.reset_index()
    ...
    df=df.assign(多空对冲=df.分组10-df.分组1)
    nets=(df+1).cumprod()
