    ...
    def predict(self):
        ...
        if self.name_item == None and self.name_user != None:
            ...
            if self.number_items == None:
                ...
                print(f"For User {self.name_user} approach follow {self.number_items} items: in return")
                score_tuples = sorted(scores.items(), key=lambda item: item[1], reverse=True)
                if self.number_items <= len(score_tuples):
                    sorted_score = {k: v for k, v in score_tuples[:self.number_items]}
                else:
        ...
        if self.name_item != None and self.name_user == None:
            ...
            if self.number_users == None:
                print(f"Done. Prediction for user {self.name_item} to return dict item_name -> score")
                return scores
            else:
                print(f"For Item {self.name_item} approach follow {self.number_users} users: in return")
                score_tuples = sorted(scores.items(), key=lambda item: item[1], reverse=True)
                if self.number_users <= len(score_tuples):
                    sorted_score = {k: v for k, v in score_tuples[:self.number_users]}
                else:
                    sorted_score = {k: v for k, v in score_tuples}
                return sorted_score 