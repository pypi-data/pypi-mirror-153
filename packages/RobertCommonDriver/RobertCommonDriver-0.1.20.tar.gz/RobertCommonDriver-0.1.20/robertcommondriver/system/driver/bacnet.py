...
class BacnetReadWriteApplication(BIPSimpleApplication):
    ...
    def __init__(self, local_device, local_address: str, parent: Any=None):
        ...
        self.timer = BaseCommon.SimpleTimer()
    ...
    def find_reason(self, apdu) -> str:
        try:
            ...
            return f"KeyError: {type(apdu)} has no key {err.args[0]}"

    def confirmation(self, apdu):
        invoke_key = (apdu.pduSource, apdu.apduInvokeID)
        if invoke_key == self.last_invoke_key:  # 匹配请求id
            if isinstance(apdu, AbortPDU):
                ...
                if invoke_key in self.bacnet_iocb_request.keys():
                    ...
                    self.parent.set_call_result('debug_log', content=f"bacnet({self}) no match request({invoke_key} {apdu})")
    ...
    def make_request_iocb(self, request, process: str = ''):
        ...
        self.parent.set_call_result('debug_log', content=f"bacnet({self}) send{process} address({request.pduDestination}) invoke({request.apduInvokeID})")

        self.timer.run(self.bacnet_timeout, self.request_timeout, args=[request.pduDestination, request.apduInvokeID])
        ...
        self.parent.set_call_result('debug_log', content=f"bacnet({self}) ack{process} invoke({request.apduInvokeID})")

        if bacnet_apdu_iocb.ioResponse is None:
            raise Exception(f"No Response({bacnet_error})")
            ...
            try:
                ...
                if pdu_destination:  # Receive device target address
                    ...
                    try:
                        ...
                        self.parent.set_call_result('debug_log', content=f"bacnet({self}) read ({read_key}) success points({len(bacnet_tag_dict)}): {list(bacnet_tag_dict.values())} response({len(result.listOfReadAccessResults)})")
