...
def vect_batch(batch, mask_prob, one_in_middle):
...
def ds_iter(src_file, seq_len, mask_prob, batch_size, one_in_middle=False):
    batch = []
    while True:
        with open(src_file, "r", encoding="utf-8") as fr:
            ...
            for line in fr:
                ...
                if len(batch) >= batch_size:
                    v_batch = vect_batch(batch, mask_prob, one_in_middle)
                    yield v_batch
                    del batch[:]


def double_mask(src_file, seq_len, mask_prob_1, mask_prob_2, batch_size, use_mask):
    wrapped_iter = ds_iter(src_file, seq_len, mask_prob_1, batch_size)

    while True:
        x, y, mask = next(wrapped_iter)

        x2 = x.copy()
        x2 = mask_data(x2, mask_prob_2)
...
if __name__ == '__main__':
    ...
    # ds_it = double_mask(ROOT / 'refprot_random_taxfree.tsv', seq_len, mask_prob_1, mask_prob_2, batch_size, use_mask)
