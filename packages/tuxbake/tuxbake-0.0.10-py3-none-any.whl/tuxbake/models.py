...
class OEBuild(Base):
    ...
    def do_build(self):
        ...
        cmd = [
            "bash",
            "-c",
            f"rm {self.build_dir}/conf/bblayers.conf; rm {self.build_dir}/conf/local.conf; source {self.envsetup} {self.build_dir}; cat ../extra_local.conf >> conf/local.conf; cat ../bblayers.conf >> conf/bblayers.conf || true; echo 'Dumping local.conf..'; cat conf/local.conf; bitbake -e > bitbake-environment;",
        ]
