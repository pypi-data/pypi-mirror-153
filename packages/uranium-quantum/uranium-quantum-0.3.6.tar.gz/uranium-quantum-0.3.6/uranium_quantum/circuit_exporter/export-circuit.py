    ...
    if files:
      for file in files:
          with open(file, "r") as stream:
              circuit_contents.append(stream.read())

    if file_fields:
        for file_field in file_fields:
            stream = file_field.read().decode('utf-8')
            circuit_contents.append(stream)

    for circuit_content in circuit_contents:
        collect_circuit_name(circuit_content, circuit_names)

    for circuit_content in circuit_contents:
        collect_circuit(circuit_content, circuit_objects, circuit_codes, circuit_names, exporter, export_format, add_comments)
