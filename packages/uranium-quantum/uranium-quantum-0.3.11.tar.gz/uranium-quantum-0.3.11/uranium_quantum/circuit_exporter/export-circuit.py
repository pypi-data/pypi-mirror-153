...
def collect_circuit_names_from_file(file, circuit_names):
    with open(file, "r") as stream:
        yaml_data = yaml.safe_load(stream)
        circuit_id = yaml_data["circuit_id"]
        circuit_names[circuit_id] = yaml_data["circuit_name"].lower().replace(" ", "_")
 
def collect_circuit_names_from_object(object, circuit_names):
    circuit_id = object["circuit_id"]
    circuit_names[circuit_id] = object["circuit_name"].lower().replace(" ", "_")

def collect_circuit_objects_from_file(file, circuit_names, circuit_objects, circuit_codes, exporter, export_format, add_comments):
    with open(file, "r") as stream:
        yaml_data = yaml.safe_load(stream)
        circuit_id = yaml_data["circuit_id"]
...
def collect_circuit_objects_from_object(object, circuit_names, circuit_objects, circuit_codes, exporter, export_format, add_comments):
    circuit_id = object["circuit_id"]
    circuit_objects[circuit_id] = object
    no_qubits = get_number_qubits(object)
    exporter.set_number_qubits(no_qubits)
    # a circuit with classical bits cannot be converted to a gate
    exporter.set_number_bits(0)
    circuit_name = object["circuit_name"].lower().replace(" ", "_")
    circuit_code = process_circuit_yaml(object, circuit_name, circuit_names, exporter, export_format, add_comments, True)
    circuit_codes[circuit_id] = circuit_code

def get_exported_code(main_circuit_id, export_format, comments, files=None, objects=None):
    """Get circuit code in exported format"""
    add_comments = True if comments else False

    if files and objects:
        raise Exception(f"Either a lis of files or a list of circuit objects must be supplied as input but not both.")
    ...
    if files:
        for file in files:
            try:
                collect_circuit_names_from_file(file, circuit_names)
            except yaml.YAMLError as ex:
                quantum_code = str(ex)
                return quantum_code
        for file in files:
            try:
                collect_circuit_objects_from_file(file, circuit_names, circuit_objects, circuit_codes, exporter, export_format, add_comments)
            except yaml.YAMLError as ex:
                quantum_code = str(ex)
                return quantum_code
    elif objects:
        for object in objects:
            collect_circuit_names_from_object(object, circuit_names)

        for object in objects:
            collect_circuit_objects_from_object(object, circuit_names, circuit_objects, circuit_codes, exporter, export_format, add_comments)
