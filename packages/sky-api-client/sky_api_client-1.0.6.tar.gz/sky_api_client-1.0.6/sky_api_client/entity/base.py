...
class Entity(object):
    ...
    DELETE_URL = None
    SEARCH_URL = None
    LIST_FOR_CONSTITUENT_URL = None
    TYPES_URL = None

    def __init__(self, api: client) -> None:
        self._api = api

    def list(self, id='', constituent_id=None):
        if constituent_id is not None:
            if self.LIST_FOR_CONSTITUENT_URL:
                return self._api.request(
                    method='GET', path=self.LIST_FOR_CONSTITUENT_URL.format(constituent_id=constituent_id)
                )
        elif self.LIST_URL:
