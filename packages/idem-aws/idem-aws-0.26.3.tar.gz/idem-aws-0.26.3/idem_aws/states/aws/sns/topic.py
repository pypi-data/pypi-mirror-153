    ...
    **Autogenerated function**
    ...
    if before:
        ...
        if ctx.get("test", False):
            ...
            result["comment"] = (f"Would create aws.sns.topic '{name}'",)
            return result
        try:
            ret = await hub.exec.boto3.client.sns.create_topic(
                ctx, Name=name, Attributes=attributes, Tags=tags
            )
            ...
            result["comment"] = (f"Created aws.sns.topic '{name}'",)
        except hub.tool.boto3.exception.ClientError as e:
            result["comment"] = result["comment"] + (f"{e.__class__.__name__}: {e}",)
            result["result"] = False
...
async def absent(hub, ctx, name: str, resource_id: str) -> Dict[str, Any]:
    ...
    **Autogenerated function**
    ...
    if not before:
        result["comment"] = (f"aws.sns.topic '{name}' already absent",)
        ...
        if ctx.get("test", False):
            result["comment"] = (f"Would delete aws.sns.topic '{name}'",)
            return result
        try:
            ...
            if not result["result"]:
                ...
                result["result"] = False
                return result
            result["comment"] = (f"Deleted aws.sns.topic '{name}'",)
        except hub.tool.boto3.exception.ClientError as e:
            result["comment"] = result["comment"] + (f"{e.__class__.__name__}: {e}",)
