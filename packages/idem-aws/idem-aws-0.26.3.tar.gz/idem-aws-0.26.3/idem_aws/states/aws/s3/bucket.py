...
async def present(
    ...
    r"""
        ...
        create_bucket_configuration (dict) --
            ...
            LocationConstraint (Text, optional) --
        ...
        tags (list, optional) - Sets the tags for a bucket and a container of a key value name pair.
    ...
    if before:
        ...
        result["comment"] = (f"aws.s3.bucket {name} already exists.",)
        if (
            object_lock_enabled_for_bucket is not None
            and object_lock_enabled_for_bucket
            != result["old_state"].get("object_lock_enabled_for_bucket")
        ):
        ...
        if (
            ...
            and not hub.tool.aws.state_comparison_utils.are_lists_identical(
                result["old_state"].get("tags"), tags
            )
        ):
            if not ctx.get("test", False):
