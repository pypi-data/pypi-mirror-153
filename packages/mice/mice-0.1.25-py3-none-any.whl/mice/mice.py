...
class MICE:
    ...
    def __init__(self,
        ...
        if self.mice_type in ['resampling', 'bayesian hessian']:
            ...
            self.re_cost = 1.
            self.re_min_n = re_min_n
            self.define_tol = self._define_tol_norm_sparse
            self.norm_estim = None
        else:
            self.err_tol = .0
    ...
    def _define_tol_norm_sparse(self):
        t0 = time()
        ml = self.sample_sizes
        opt_ml = self.get_opt_ml(self.deltas)
        cost = np.maximum(opt_ml - ml, 0).sum()
        ...
        estims = sp.vstack(self.deltas[0].f_deltas[samples[:, 0]], format='csr')
        for delta, samples_ in zip(self.deltas[1:], samples[:, 1:].T):
            estims += delta.f_deltas[samples_]
        norms = self.norm(estims)
        norms = np.array(np.vstack([norms, self.norm(self.aggr_deltas())])).ravel()
