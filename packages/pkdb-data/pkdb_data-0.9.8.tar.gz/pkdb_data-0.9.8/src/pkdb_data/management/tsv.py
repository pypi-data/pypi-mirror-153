"""
Helper function which creates TSV files from Excel spreadsheet.
"""
import re
import time
...
def build_tsvs(path_xlsx: Path):
    ...
    try:
        ...
        logger.error(
            f"Problems reading xlsx file. Probably stored in incorrect format. "
...
def validate_excel_sheet(df, sheet_name, study_id):
...
def validate_no_empty_rows(df, sheet_name, study_id):
...
def validate_format(df, sheet_name, study_id):
    ...
    for n, column in enumerate(df.columns):
        ...
        #    continue
        if n == 0:
            if not _validate_first_column_name(column, sheet_name, study_id):
                is_valid = False
            if not _validate_first_column_id(df, sheet_name, study_id):
                is_valid = False

        if not _validate_lower_cause(column, study_id, sheet_name, n):
            is_valid = False
...
def validate_popular_suffix(column_names, sheet_name, study_id):
    """
...
def validate_forbidden_prefix(column_names, sheet_name, study_id):
...
def _validate_first_column_name(first_column, sheet_name, study_id):
...
def _validate_special_characters(column_name, study_id, sheet_name, column_number):
...
def _validate_no_whitespace(column, study_id, sheet_name, n):
    ...
    if " " in column:
        msg = (
            ...
            f"<{study_id}.xlsx> {n}th column is <{column}>."
        )
        logger.error(msg)
        return False
    return True
...
def _validate_first_column_id(df: pd.DataFrame, sheet_name, study_id):
...
def _validate_sheet_name(sheet_name, study_id):
