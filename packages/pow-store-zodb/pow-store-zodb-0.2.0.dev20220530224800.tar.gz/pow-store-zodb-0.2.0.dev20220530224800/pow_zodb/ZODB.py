...
class ZODBStore(Persistent, Store):
    ...
    def bind(self, prefix, namespace, override=True):
        if prefix is None:
            ...
            old_ns = self._namespace.get(prefix)
            if prefix in self._namespace:
                del self._namespace[prefix]
            if old_ns in self._prefix:
                del self._prefix[old_ns]
        else:
    ...
    def triples(self, triplein, context=None):
        ...
        if isinstance(triplein[2], tuple):
            enctriple = (self._obj2id_finf(triplein[0]),
                         self._obj2id_finf(triplein[1]),
                         None)
            rng = triplein[2]
        else:
