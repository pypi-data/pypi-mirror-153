...
from pow_zodb.ZODB import ZODBStore
...
class ZODBContextTestCase(ContextTestCase):
    ...
    def testTriples(self):
        ...
        for c in [graph, self.get_context(c1)]:
            ...
            asserte(set(c.subject_objects(likes)), set([(tarek, cheese), (michel, cheese),
            ...
            asserte(set(c), set([(bob, hates, michel), (bob, likes, cheese),
                (tarek, likes, pizza), (michel, likes, pizza), (michel, likes, cheese),
                (bob, hates, pizza), (tarek, likes, cheese)]))
...
def test_namespace_bind_none_clears_url():
    cut = ZODBStore()
    cut.bind('test', URIRef('http://example.org/test/'))
    assert ('test', URIRef('http://example.org/test/')) in cut.namespaces()
    cut.bind('test', None)
    assert None is cut.namespace('test')
