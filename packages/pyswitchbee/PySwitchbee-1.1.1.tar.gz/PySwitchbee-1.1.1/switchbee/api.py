...
from .const import ApiAttribute, ApiCommand, ApiStatus
...
TOKEN_EXPIRATION = int(timedelta(hours=1).total_seconds()) * 1000  # 1 hour
TOKEN_RENEW_EARLY = int(timedelta(minutes=5).total_seconds()) * 1000
...
class CentralUnitAPI:
    ...
    async def _send_request(self, command: ApiCommand, params: dict = {}) -> dict:
        if not self._token or (
            self._token
            and timestamp_now() >= self._token_expiration + TOKEN_RENEW_EARLY
        ):
            logger.info(
        ...
        for zone in data[ApiAttribute.DATA][ApiAttribute.ZONES]:
            for item in zone[ApiAttribute.ITEMS]:
                ...
                if device_type == DeviceType.Switch.value:
                    ...
                    DeviceType.Scenario.value,
                    DeviceType.GroupSwitch.value,
                ]:
