...
#     trips_file = 'trips_file_tmp'
    ...
    if modos != 'Transporte Público, Automovil':
        trips_file += '_'
        if transit: trips_file += 'd'
        if driving: trips_file += 'c'
        if walking: trips_file += 'w'
        if bicycling: trips_file += 'b'
                ...
                if answer.lower() == 'si':
                    ...
                    for trip_datetime in od_matrix_agg.trip_datetime.unique():
                        ...
                        if not only_distance_duration:
                            ...
                            save_key(tmp / Path('save_key.csv'), key, pd.to_datetime(trip_datetime), (transit+((driving)*1.5)+walking+bicycling)*len(trips_new), current_path)
    ...
    if geo_origin in od_matrix_all.columns:    
        od_matrix_all.loc[od_matrix_all[geo_origin] == od_matrix_all[geo_destination]] = od_matrix_all.loc[od_matrix_all[geo_origin] == od_matrix_all[geo_destination]].fillna(0)
