...
def _tsf_preprocess_data(data: pd.DataFrame):
    ...
    tsf_data[TSF_SORT_COL] = range(tsf_data.shape[0])
    tsf_data[TSF_SORT_COL] = tsf_data[TSF_SORT_COL].astype('int32')
    return tsf_data
...
def calc_rolling_feature(ohlc: pd.DataFrame,
    ...
    df_rolled = roll_time_series(tsf_data,
                                 column_id="Tsf_Id",
                                 column_sort=TSF_SORT_COL,
                                 max_timeshift=win_len,
                                 min_timeshift=win_len)
