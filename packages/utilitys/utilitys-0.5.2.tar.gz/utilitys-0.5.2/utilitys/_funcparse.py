...
def createFuncParameter(name, signatureParam, docDict, overridesDict, title=None):
    ...
    if (
        ...
        signatureDict = {"value": default, "type": type(default).__name__.lower()}
    else:
        signatureDict = {}
...
def parseIniDocstring(doc):
    ...
    *not required* since they can be inferred from the properly formatted docstring.
    ...
    try:
        ...
        return _parseIniDocstringDocstringParser(doc)
    except ImportError:
        # noinspection PyBroadException
        try:
            return _parseIniDocstringBasic(doc)
        except:
...
def _parseIniDocstringDocstringParser(doc):
    """
    ...
    for param in parsed.params:
        ...
        # noinspection PyBroadException
        try:
            update = _parseIniDocstringBasic(miniDoc)
        except:
...
def _parseIniDocstringBasic(doc):
...
class InteractiveFunction:
    ...
    def runOnChanging(self, _param, value):
        if self._disconnected:
            return
        return self(**{_param.name(): value})

    def runOnChangedOrButton(self, **kwargs):
        if self._disconnected:
            return
        return self(**kwargs)
    ...
    **overrides,
    ...
    for chDict in children:
        ...
        if chDict["value"] is RunOpts.PARAM_UNSET and name not in toExec.deferred:
            raise ValueError(
                f'Cannot interact with "{func} since it has required parameter "{name}"'
...
def _createFuncParamChild(parent, chDict, runOpts, existOk, toExec):
    ...
    if RunOpts.ON_CHANGED in runOpts:
        ...
        def runOnChange():
            toExec.runOnChangedOrButton()

        child.sigValueChanged.connect(runOnChange)
    if RunOpts.ON_CHANGING in runOpts:

        def runOnChanging(_param, _val):
            toExec.runOnChanging(_param, _val)

        child.sigValueChanging.connect(runOnChanging)
    return child
...
def _makeRunButton(nest, tip, interactiveFunc):
    ...
    def run():
        interactiveFunc.runOnChangedOrButton()
