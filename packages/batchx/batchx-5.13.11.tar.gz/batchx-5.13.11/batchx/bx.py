...
class JobService:
    ...
    def Run(self, submit_request):
        ...
        if submit_response.WhichOneof("case") == "suggested_parameters":
            raise Exception("Unsupported job parametrization. Suggested one is: " + str(getattr(submit_response,suggested_parameters)))
        request_stream = self.StreamRequest(environment=submit_request.environment, job_id=submit_response.job_id)
