...
class AnnotatedField(mm_fields.Field):
    ...
    def _serialize(self, value, attr, obj, **kwargs):
        return self.sub._serialize(value, attr, obj, **kwargs)

    def _deserialize(self, value, attr, data, **kwargs):
        return self.sub._deserialize(value, attr, data, **kwargs)
...
class UnionField(mm_fields.Field):
    ...
    def _serialize(self, value, attr, obj, **kwargs):
        ...
        lit, pytype = self.tr._to_literal_ext(FlyteContext.current_context(), value, self.typ, self.expected)
...
og_build_type = dataclasses_json_mm.build_type
def build_type_hack(type_, options, mixin, field, cls):
    ...
    if get_origin(type_) is typing.Union:
        return UnionField(type_, [dataclasses_json_mm.build_type(x, {}, mixin, field, cls) for x in get_args(type_)])

    if get_origin(type_) is Annotated:
        return AnnotatedField(type_, dataclasses_json_mm.build_type(get_args(type_)[0], options, mixin, field, cls))

    return og_build_type(type_, options, mixin, field, cls)
                            ...
                            "oneOf": [
                                {
                                    ...
                                    "type": {
                                        "const": _MessageToDict(TypeEngine.to_literal_type(t).to_flyte_idl())
                                    }
...
class UnionTransformer(TypeTransformer[T]):
    ...
    def _to_literal_ext(self, ctx: FlyteContext, python_val: T, python_type: Type[T], expected: LiteralType) -> Tuple[Literal, Any]:
        if get_origin(python_type) is Annotated:
            python_type = get_args(python_type)[0]
        ...
        if found_res:
            return Literal(scalar=Scalar(union=Union(value=res, stored_type=res_type))), res_pytype
