...
class equality_constraints(basic_manifold_torch):
    def __init__(self,name,variable_shape, constraint_shape, manifold: basic_manifold_torch,  device = torch.device('cpu'), dtype = torch.float64,  autodiff_type = "autograd", regularize_value = 0.01, safegurad_value = 0) -> None:
        self.name = name
    ...
    def CE(self, X):
        """Returns the expression of the equality constraints
        """
    ...
    # Here self.A(X) = self.A_CE( self.CM.A(X) )
    ...
    def C_quad_penalty(self, X):
        return torch.sum(self.C(X) **2) + self.CM.C_quad_penalty(X)
    ...
    def Init_point(self, Xinit = None):
        ...
        return self.CM.Init_point(Xinit=Xinit)
    ...
    def generate_cdf_grad(self, obj_grad, beta):
        return lambda X:  self.JA(X,obj_grad(self.A(X))) + beta * (self.JC(X, self.C(X)) + self.CM.JC(X, self.CM.C(X)))
    ...
    def generate_cdf_hess(self, obj_grad, obj_hess, beta):
        return lambda X, D: self.JA( X, obj_hess( self.A(X), self.JA_transpose(X,D) ) ) + self.hessA(X, obj_grad(X), D) + beta * (self.hess_feas(X, D) + self.CM.hess_feas(X,D))
