...
def format_call(
    ...
    for key in kwargs:
        try:
            val = data.pop(key)
            # Use enforced_state value over sls value if the parameter is in ignore_changes list
            if ignore_changes and key in ignore_changes and key in enforced_state:
                continue
