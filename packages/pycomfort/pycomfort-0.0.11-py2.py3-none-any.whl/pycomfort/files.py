...
def children(p: Path, in_name: str = None) -> seq:
    """
    files and subfolders in the folder as sequence
    :param p:
    :return:
    """
    chs = seq(list(p.iterdir()))
    return chs if in_name is None else chs.filter(lambda c: in_name in c.name)


def dirs(p: Path, in_name: str = None) -> seq:
    """
    ...
    :return:
    """
    return children(p, in_name).filter(lambda f: f.is_dir())


def files(p: Path, in_name: str = None) -> seq:
    """
    ...
    :return:
    """
    return children(p, in_name).filter(lambda f: f.is_file())
...
def tprint(p: Path, prefix: str = "", depth: int = -1, debug: bool = False):
    """
    ...
    if fl.len() > 0:
        if debug:
            print(prefix + f"FILES:")
        files(p).for_each(lambda f: print(f"\t"+prefix+f.name))
    folders = dirs(p)
    if folders.len() > 0:
        if debug:
            print(prefix+"FOLDERS")
        folders.for_each(lambda d: tprint(d, f"\t"+prefix, depth-1))