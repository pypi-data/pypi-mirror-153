...
class rteDbServer:
    ...
    def __init__(self, p_project_name, p_dir, p_parent_log_name='', p_verbose=True):
        ...
        if p_parent_log_name:
            self.log_name = '{}.{}'.format(p_parent_log_name, rteDbServer)
...
def do_examples(p_cls=True):
    '''A collection of implementation examples for rteDbServer.

    A collection of implementation examples for rteDbServer. The examples
...
def do_example1(p_cls=True):
    '''A working example of the implementation of rteDbServer.
    ...
    t_rtedbserver = rteDbServer("rteDbServer", t_dir)
    t_rtedbserver.method_1('This is do_example1')
...
def do_example2(p_cls=True):
    '''Another working example of the implementation of rteDbServer.
    ...
    t_dir = beeutils.get_tmp_dir()
    t_rtedbserver = rteDbServer("rteDbServer", t_dir)
    t_rtedbserver.method_1('This is do_example2')
    beeutils.rm_tree(t_dir)
    archiver.print_footer()
    return success
