...
class Disys_Calib:
    ...
    def PointsWithMaxErr(self,toTake):
        try:
            ...
            selectedPointsDF = self.df.iloc[sortedErrorsIndex[0:toTake],:]
            # Add the error to the selected points
            selectedPointsDF['ErrorNet'] = np.take(self.err, sortedErrorsIndex[0:toTake])
            # Save the points in a csv file
            selectedPointsDF.to_csv(self.main_path+R"\dati calibrazione\maxerrorpoints.csv", index=False)
        except Exception as err:
            print("Error")