...
class Disys_Calib:
    ...
    def CreateNN(self,width:int,height:int):
        ...
        print("\n********** Start training **********")
        self.f.write("Amount of data for training: {}".format(n_train))
        self.f.write("Amount of data for test: {}".format(n_test))
        self.f.write("\n********** Start training **********")
        ...
        for i in range(step):
            ...
            self.f.write("ERRORE RETE step {}/{}: Y_test ->  min {:.4f} , mean {:.4f} , max {:.4f} ".format((i+1),step,np.min(err),np.mean(err),np.max(err)) )
