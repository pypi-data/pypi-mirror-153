...
def get_fund_manager_info(fund_code):
    ...
    for manager_code in manager['MANAGER_CODE'].unique().tolist():
        ...
        manager_funds = manager_funds[['FUND_TYPE_1ST', 'EQUITY']].groupby('FUND_TYPE_1ST').sum().reset_index().sort_values('EQUITY', ascending=False)
        manager_funds['EQUITY'] = manager_funds['EQUITY'].apply(lambda x: str(round(x / 100000000.0, 2)))
        scale_type = '、'.join(manager_funds['FUND_TYPE_1ST'].unique().tolist())
