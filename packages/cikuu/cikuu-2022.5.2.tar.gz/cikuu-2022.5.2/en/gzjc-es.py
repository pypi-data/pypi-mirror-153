...
class ES(object):
	...
	def spacybs(self, infile, idxname=None, refresh:bool=True, batch:int=700000):
		...
		start = time.time()
		actions=[]
		for rowid, snt, bs in Spacybs(infile).items() :
			doc = spacy.frombs(bs) 
			sid = f"{idxname}-{rowid}"
			...
			[ actions.append( {'_op_type':'index', '_index':idxname, '_id': f"{sid}-NP-{sp.start}", '_source': 
				{"type":"NP", "src":sid,  'lem':doc[sp.end-1].lemma_, 'chunk':sp.text, 'start':sp.start, 'end':sp.end} }) for sp in doc.noun_chunks ]
			[ actions.append( {'_op_type':'index', '_index':idxname, '_id': f"{sid}-{id}", '_source': dict(sour, **{"src":sid}) } ) for id, sour in attach(doc).items()]
