...
class main():
    def main(self, year, c_year, school):
        ...
        if hikaku > 0:
            print('\n', school,'について',year, 'は', c_year,'よりも', hikaku, '件多いです。')
            fw = str(school) + 'について、' + str(year) + 'は' + str(c_year) + 'よりも' + str(hikaku) + '件多いです。'
            if os.path.exists('bully.txt')==True:
                os.remove('bully.txt')
            f = open('bully.txt', 'w')
            f.write(fw)
            f.close()
            ...
            hikaku = hikaku * -1
            print('\n', school,'について',year, 'は', c_year,'よりも', hikaku, '件少ないです。')
            fw = str(school) + 'について、' + str(year) + 'は' + str(c_year) + 'よりも' + str(hikaku) + '件少ないです。'
            if os.path.exists('bully.txt')==True:
                os.remove('bully.txt')
            f = open('bully.txt' ,'w')
            f.write(fw)
