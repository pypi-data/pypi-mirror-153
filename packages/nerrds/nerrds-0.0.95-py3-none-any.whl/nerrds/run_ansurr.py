...
def run_ansurr(home_dir, job_id, pdb_id, path_to_shifts,quiet=False):
    ...
    check_quiet_print(quiet,home_dir,"running ANSURR")
    ...
    if ansurr_output > 0:
        check_quiet_print(quiet,home_dir,"ANSURR ran for "+str(ansurr_output)+" models")
    else:
        check_quiet_print(quiet,home_dir,"ERROR ANSURR failed to run for any models, quitting")
        sys.exit(0)
    ...
    if os.path.exists(home_dir+"/"+job_id+"/ansurr/anm*/ANSURR_output/figs"):
        shutil.rmtree(home_dir+"/"+job_id+"/ansurr/anm*/ANSURR_output/figs")
