...
def reweight_ensemble(home_dir,job_id,pdb_id,shifts_id,quiet=False):
    ...
    check_quiet_print(quiet,home_dir,"determining ensemble using BME")
    ...
    if len(chosen_ensemble) > 0:
        check_quiet_print(quiet,home_dir,"determined an ensemble consisting of "+str(len(chosen_ensemble))+" models with the following weights:")
        max_pdb_name = []
        ...
        for model in chosen_ensemble:
            check_quiet_print(quiet,home_dir," -> model "+model+" "+chosen_ensemble[model]["pdb"]+" "+chosen_ensemble[model]["weight"])
        check_quiet_print(quiet,home_dir,"the ensemble has been saved here: "+ensemble_dir+pdb_id+'_ensemble.pdb')
    else:
        check_quiet_print(quiet,home_dir,"BME failed to determine an ensemble, quitting")
        sys.exit(0)
