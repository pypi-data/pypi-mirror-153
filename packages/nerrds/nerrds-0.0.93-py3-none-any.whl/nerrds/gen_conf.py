...
def gen_conf(path_to_pdb, home_dir, job_id, pdb_id, conformers, average_rmsd, quiet=False):
    ...
    check_quiet_print(quiet,home_dir,"generating "+str(conformers)+" conformers for each of "+str(num_models)+" models, ("+str(int(conformers) * int(num_models))+" in total)\nprody output:\n")
    ...
    if generated_conformers > 0:
        check_quiet_print(quiet,home_dir,"\ngenerated "+str(generated_conformers)+" conformers")
    else:
        check_quiet_print(quiet,home_dir,"ERROR failed to generate any conformers, quitting")
        sys.exit(0)
