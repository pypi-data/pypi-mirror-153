...
def main():
    ...
    now = datetime.now()
    check_quiet_print(args.quiet,home_dir,"\nNERRDS v"+nerrds_version+" "+now.strftime("%d/%m/%Y %H:%M:%S"))
    check_quiet_print(args.quiet,home_dir," -> pdb: "+path_to_pdb)
    check_quiet_print(args.quiet,home_dir," -> shifts: "+path_to_shifts)
    ...
    if args.env is not None: # check if job to be run on HT condor
        ...
        check_quiet_print(args.quiet,home_dir,"submitted subsequent processing to the job queue, check NERRDS.log for progress")
