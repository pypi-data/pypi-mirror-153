...
class FastScanCommand:
    ...
    def _run_acquisition(self):
        while True:
            if self.stopped.is_set():
                return
            self._detectorAcquisitions += (time.time(), self._controller.adc.read())
            if self.stopped.wait(0.001):
                return

    def record_callback(self, x, y, t):
        self._vcmAcquisitions += (t, x, y)
    ...
    def execute(self):
        try:
            if not self._controller.testMode or self._testWithMocks:
                ...
                self._controller.extVcm.register_record_callback(self.record_callback)
                for wavelength in self.wavelengths:
                    self._completedWl += 1
                    if self.stopped.is_set():
                        return
                    ...
                    self.results['x'].append(interpX.tolist())
                    self.results['y'].append(interpY.tolist())
                    self.results['wavelength_set_point'].append(wavelength)
                    self.results['wavelength'].append(cavity.get_wavelength())
                    self.results['detector_voltage'].append(detectorValues.tolist())
            ...
            self.stop()
            self._controller.stop_all_pulsing()
            self._controller.extVcm.reset_static_input()
            self._controller.extVcm.unregister_record_callback(self.record_callback)
