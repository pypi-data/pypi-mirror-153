...
from johnsnowlabs.auto_install.spark_nlp_software import SparkNlpSoftware, Software
...
from johnsnowlabs.auto_install.resolve_software import check_and_install_dependencies

if not try_import_lib('pyspark'):
...
def install(
    ...
    secrets: JslSecrets = JslSecrets.build_or_try_find_secrets(secrets_file, hc_license, hc_secret,
    ...
    if not product:
        ...
        # sine NLU depends on everything
        product = ProductName.nlu

    if product == ProductName.spark_nlp:
        product = Software.spark_nlp
    elif product == ProductName.nlu:
        product = Software.nlu
    elif product == ProductName.healthcare:
        product = Software.spark_hc
    elif product == ProductName.ocr:
        product = Software.spark_ocr
    elif product == ProductName.nlp_display:
        product = Software.sparknlp_display
    elif product == ProductName.pyspark:
        product = Software.pyspark
    else:
        raise Exception(f'Invalid install target: {product}'
                        f' please specify on of :{"/n".join([n.value for n in ProductName])}')
...
def start(
    ...
    secrets: Union[JslSecrets, bool] = JslSecrets.build_or_try_find_secrets(secrets_file, hc_license, hc_secret,
