...
class LibDependencyResolverABC(ABC):
    has_gpu_jars: bool
    has_cpu_jars: bool
    ...
    @classmethod
    @abstractmethod
    def get_jsl_lib_version_identifier_for_env(cls, ) -> LibVersionIdentifier:
        """
...
class OcrLibResolver(LibDependencyResolverABC, metaclass=ABCMeta):
    """
    ...
    def get_cpu_jar_urls(cls, secret: Optional[str],
        ...
        if not jsl_lib_version:
            jsl_lib_version = OcrLibResolver.get_jsl_lib_version(secret)
    ...
    @classmethod
    def get_py_urls(cls, jsl_lib_version: LibVersionIdentifier, secret: Optional[str], install_type: PyInstallTypes,
                    spark_version: Optional[JslLibSparkVersionIdentifier] = None) -> UrlDependency:
        """
...
class HcLibResolver(LibDependencyResolverABC, metaclass=ABCMeta):
    """
    Each JslLibDependencyResolver util must implement the following :
    - get_jar_urls(lib_version,spark_version,Optional[secret])->UrlDependency
    - get_mvn_coordinates(lib_version,spark_version,Optional[secret])->RepoDependency
    ...
    has_gpu_jars = True
    product_name = Products.healthcare
    ...
    def get_cpu_jar_urls(cls, secret: str, jsl_lib_version: Optional[LibVersionIdentifier] = None,
        ...
        if not spark_version:
            spark_version = cls.get_spark_version_id()
        if not jsl_lib_version:
            try_import_lib('sparknlp_jsl', True)
            import sparknlp_jsl
            jsl_lib_version = sparknlp_jsl.version()
    ...
    @classmethod
    def get_py_urls(cls, jsl_lib_version: LibVersionIdentifier, install_type: PyInstallTypes, secret: Optional[str],
                    spark_version: Optional[JslLibSparkVersionIdentifier] = None, ) -> UrlDependency:
        """
        install_type_tar = '.tar.gz'
        install_type_wheel = '-py3-none-any.whl'
...
class NlpLibResolver(LibDependencyResolverABC, metaclass=ABCMeta):
    """
    Each JslLibDependencyResolver util must implement the following :
    - get_jar_urls(lib_version,spark_version,Optional[secret])->UrlDependency
    - get_mvn_coordinates(lib_version,spark_version,Optional[secret])->RepoDependency
    ...
    @classmethod
    def get_py_urls(cls, jsl_lib_version: LibVersionIdentifier, install_type: PyInstallTypes,
                    spark_version: Optional[JslLibSparkVersionIdentifier] = None,
                    secret: Optional[str] = None) -> UrlDependency:
        """
        install_type_tar = '.tar.gz'
        ...
        offline = False
        if offline:
            # Fallback, we cannot find exact URL withouth internet
