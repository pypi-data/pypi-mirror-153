...
def do_test(pretrained, input, output_file, is_del_venv, is_verbose):
    ...
    verbose_arg = 1 if is_verbose else 0
    # input can have space so added quotes
    p = subprocess.run(
        f'{TIYARO_TEST_VENV_PATH}/bin/python {HANDLER_MODEL_MODEL_TEST_FILE} {TEST_COMMAND_ARG_PRETRAINED} {pretrained} {TEST_COMMAND_ARG_INPUT}="{input}" {TEST_COMMAND_ARG_OUTPUT} {output_file} {TEST_COMMAND_ARG_VERBOSE} {verbose_arg}', shell=True)
