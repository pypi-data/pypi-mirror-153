...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    iodi__zdg = pq_node.out_vars[0].name
    jzne__ibayn = pq_node.out_vars[1].name
    if iodi__zdg not in lives and jzne__ibayn not in lives:
        return None
    elif iodi__zdg not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif jzne__ibayn not in lives:
