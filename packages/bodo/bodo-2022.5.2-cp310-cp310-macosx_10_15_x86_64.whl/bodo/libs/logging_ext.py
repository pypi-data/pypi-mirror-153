...
def lower_constant_logger(context, builder, ty, pyval):
    cbsi__ppjvw = context.get_python_api(builder)
    return cbsi__ppjvw.unserialize(cbsi__ppjvw.serialize_object(pyval))
...
class LoggingLoggerAttribute(AttributeTemplate):
    ...
    def _resolve_helper(self, logger_typ, args, kws):
        kws = dict(kws)
        snzy__sulcc = ', '.join('e{}'.format(ncebo__ztf) for ncebo__ztf in
            range(len(args)))
        if snzy__sulcc:
            snzy__sulcc += ', '
        wnlq__bdn = ', '.join("{} = ''".format(utfv__aemn) for utfv__aemn in
            kws.keys())
        qck__fqjva = f'def format_stub(string, {snzy__sulcc} {wnlq__bdn}):\n'
        qck__fqjva += '    pass\n'
        gxkgt__vqdn = {}
        exec(qck__fqjva, {}, gxkgt__vqdn)
        iuauo__gfzg = gxkgt__vqdn['format_stub']
        ntq__xbqcq = numba.core.utils.pysignature(iuauo__gfzg)
        epmwf__lhzwr = (logger_typ,) + args + tuple(kws.values())
        return signature(logger_typ, epmwf__lhzwr).replace(pysig=ntq__xbqcq)
    func_names = ('debug', 'warning', 'warn', 'info', 'error', 'exception',
        'critical', 'log', 'setLevel')
    for pqlmu__zwo in ('logging.Logger', 'logging.RootLogger'):
        for dlgjk__dro in func_names:
            qefqt__wdnz = f'@bound_function("{pqlmu__zwo}.{dlgjk__dro}")\n'
            qefqt__wdnz += (
                f'def resolve_{dlgjk__dro}(self, logger_typ, args, kws):\n')
            qefqt__wdnz += (
...
def _install_logging_logger_unsupported_objects():
    for hpnqu__cexfb in logging_logger_unsupported_attrs:
        galol__crt = 'logging.Logger.' + hpnqu__cexfb
        overload_attribute(LoggingLoggerType, hpnqu__cexfb)(
            create_unsupported_overload(galol__crt))
    for msvx__jkbfo in logging_logger_unsupported_methods:
        galol__crt = 'logging.Logger.' + msvx__jkbfo
        overload_method(LoggingLoggerType, msvx__jkbfo)(
            create_unsupported_overload(galol__crt))
