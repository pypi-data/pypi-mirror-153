...
def box_regex_flag(typ, val, c):
    mnwdk__iic = c.box(types.int64, val)
    gqlxw__xfry = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(gqlxw__xfry, (mnwdk__iic,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    mnwdk__iic = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, mnwdk__iic)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    cek__zeidt = context.get_python_api(builder)
    kdgbg__jgyw = builder.icmp_signed('==', val, cek__zeidt.borrow_none())
    with builder.if_then(kdgbg__jgyw):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    gyrm__mdfe = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(gyrm__mdfe)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        jbard__lkl = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if jbard__lkl > 1:
            typ = types.List(types.Tuple([string_type] * jbard__lkl))
...
def overload_match_group(m, *args):
    ...
    rxayn__aiil = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            mfbi__eoxx = args[0]
            with numba.objmode(out=rxayn__aiil):
                out = m.group(mfbi__eoxx)
            return out
        return _match_group_impl_one
    adcsf__eyx = 'tuple_str_{}'.format(len(args))
    setattr(types, adcsf__eyx, types.Tuple([rxayn__aiil] * len(args)))
    pdfd__shjt = ', '.join('group{}'.format(fbh__uah + 1) for fbh__uah in
        range(len(args)))
    raqh__mrhj = 'def _match_group_impl(m, *args):\n'
    raqh__mrhj += '  ({}) = args\n'.format(pdfd__shjt)
    raqh__mrhj += "  with numba.objmode(out='{}'):\n".format(adcsf__eyx)
    raqh__mrhj += '    out = m.group({})\n'.format(pdfd__shjt)
    raqh__mrhj += '  return out\n'
    pxuf__jseo = {}
    exec(raqh__mrhj, globals(), pxuf__jseo)
    uzwqd__gscv = pxuf__jseo['_match_group_impl']
    return uzwqd__gscv
...
def overload_match_groups(m, default=None):
    lvnc__szeom = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=lvnc__szeom):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(ulp__jtgbi is None for ulp__jtgbi in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=rxayn__aiil):
