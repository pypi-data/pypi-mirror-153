...
def _global_except_hook(exctype, value, traceback):
    ...
    pddy__wzad = MPI.COMM_WORLD
    gmwx__cxfy = pddy__wzad.Get_rank()
    nwrl__rvxj = 3.0
    eegg__kbj = True
    bwg__wchc = pddy__wzad.Ibarrier()
    evd__orwyb = time.time()
    while time.time() - evd__orwyb < nwrl__rvxj:
        time.sleep(0.1)
        if bwg__wchc.Test():
            eegg__kbj = False
            break
    try:
        ...
        if eegg__kbj:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {gmwx__cxfy}. \n')
