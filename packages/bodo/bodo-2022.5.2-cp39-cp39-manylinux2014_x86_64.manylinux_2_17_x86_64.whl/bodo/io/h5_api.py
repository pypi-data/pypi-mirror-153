...
def overload_h5_file_keys(obj_id):
    ...
    def h5f_keys_impl(obj_id):
        naw__sddp = []
        myyp__yqd = h5g_get_num_objs(obj_id)
        for yqzt__cdf in range(myyp__yqd):
            cutmg__hqcl = h5g_get_objname_by_idx(obj_id, yqzt__cdf)
            naw__sddp.append(cutmg__hqcl)
        return naw__sddp
...
def overload_h5_file_create_dataset(obj_id, name, shape=None, dtype=None,
    ...
    ichy__inbnn = parse_dtype(dtype)
    fwne__fdcpx = np.int32(numba_to_c_type(ichy__inbnn))
    ...
    def impl(obj_id, name, shape=None, dtype=None, data=None):
        ...
        return h5_create_dset(unify_h5_id(obj_id), unicode_to_utf8(name),
            ndim, counts.ctypes, fwne__fdcpx)
    return impl
...
def overload_getitem_file(in_f, in_idx):
    if in_f in (h5file_type, h5dataset_or_group_type
        ...
        def impl(in_f, in_idx):
            tbpth__hszt = h5_open_dset_or_group_obj(unify_h5_id(in_f),
...
def h5read(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ujgok__bymu = tuple_to_ptr(starts)
    dcqvx__lij = tuple_to_ptr(counts)
    lnrt__ziqkm = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_read(cast_to_h5_dset(dset_id), ndim, ujgok__bymu, dcqvx__lij,
        is_parallel, out_arr.ctypes, lnrt__ziqkm)
...
def h5write(dset_id, ndim, starts, counts, is_parallel, out_arr):
    ...
    dcqvx__lij = tuple_to_ptr(counts)
    lnrt__ziqkm = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5_write(cast_to_h5_dset(dset_id), ndim, ujgok__bymu,
        dcqvx__lij, is_parallel, out_arr.ctypes, lnrt__ziqkm)
...
class H5ReadType(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        kjmpk__aoa = types.Array(dtype, ndim, 'C')
        kmiw__oxljc = args[3]
        kjmpk__aoa = self.context.resolve_function_type(operator.getitem, [
            kjmpk__aoa, kmiw__oxljc], {}).return_type.copy(layout='C')
        return signature(kjmpk__aoa, *args)
...
def get_filter_read_indices(bool_arr):
    jrcvq__jve = bool_arr.nonzero()[0]
    mzfj__rue = bodo.libs.distributed_api.get_rank()
    pni__qnp = bodo.libs.distributed_api.get_size()
    ucpil__ybcp = np.empty(pni__qnp, np.int64)
    asm__dgux = len(bool_arr)
    bodo.libs.distributed_api.allgather(ucpil__ybcp, asm__dgux)
    arp__dtr = ucpil__ybcp.cumsum()[mzfj__rue] - asm__dgux
    jrcvq__jve += arp__dtr
    reky__huo = bodo.libs.distributed_api.dist_reduce(len(jrcvq__jve), np.
        int32(sum_op))
    xnvc__iwoim = bodo.libs.distributed_api.gatherv(jrcvq__jve)
    if mzfj__rue == 0:
        phimt__pudo = xnvc__iwoim
    else:
        phimt__pudo = np.empty(reky__huo, jrcvq__jve.dtype)
    bodo.libs.distributed_api.bcast(phimt__pudo)
    osli__bjbno = bodo.libs.distributed_api.get_start(reky__huo, pni__qnp,
        mzfj__rue)
    ckq__syt = bodo.libs.distributed_api.get_end(reky__huo, pni__qnp, mzfj__rue
        )
    return phimt__pudo[osli__bjbno:ckq__syt]
...
def tuple_to_ptr(typingctx, tuple_tp=None):
    ...
    def codegen(context, builder, sig, args):
        pqkn__smx = cgutils.alloca_once(builder, args[0].type)
        builder.store(args[0], pqkn__smx)
        return builder.bitcast(pqkn__smx, lir.IntType(8).as_pointer())
...
def h5read_filter(dset_id, ndim, starts, counts, is_parallel, out_arr,
    ...
    dcqvx__lij = tuple_to_ptr(counts)
    lnrt__ziqkm = bodo.libs.distributed_api.get_type_enum(out_arr)
    return _h5read_filter(cast_to_h5_dset(dset_id), ndim, ujgok__bymu,
        dcqvx__lij, is_parallel, out_arr.ctypes, lnrt__ziqkm, read_indices.
