...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        elnps__rlcz = context.get_abi_sizeof(context.get_data_type(dtype.dtype)
            )
        return context.get_constant(types.intp, elnps__rlcz)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            zivo__couil = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(zivo__couil.dtype)
            file_write(unicode_to_utf8(fname), zivo__couil.ctypes, 
                dtype_size * zivo__couil.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            zivo__couil = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(zivo__couil.dtype)
            qki__ece = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                zivo__couil.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), zivo__couil.ctypes,
                start, count, qki__ece)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            ick__kmldz = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                ick__kmldz = min(ick__kmldz, count * dtype_size)
            if ick__kmldz < 0:
                return -1
            return ick__kmldz
        return impl
