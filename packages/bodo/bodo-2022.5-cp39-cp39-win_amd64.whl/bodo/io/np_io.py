...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        tlt__dys = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, tlt__dys)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            eflp__xiofz = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(eflp__xiofz.dtype)
            file_write(unicode_to_utf8(fname), eflp__xiofz.ctypes, 
                dtype_size * eflp__xiofz.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            eflp__xiofz = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(eflp__xiofz.dtype)
            ogf__zgs = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                eflp__xiofz.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), eflp__xiofz.ctypes,
                start, count, ogf__zgs)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            bnq__kng = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                bnq__kng = min(bnq__kng, count * dtype_size)
            if bnq__kng < 0:
                return -1
            return bnq__kng
        return impl
