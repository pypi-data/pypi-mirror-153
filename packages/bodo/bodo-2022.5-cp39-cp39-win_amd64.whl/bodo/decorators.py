...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        iclg__gswec = self.overloads[sig]
        dzqcw__yqdqu = iclg__gswec.metadata.get('distributed_diagnostics', None
            )
        if dzqcw__yqdqu is None:
            tskkf__ntkh = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(tskkf__ntkh)
        dzqcw__yqdqu.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        zhxjz__woow = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(zhxjz__woow)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        wttfj__xhzfk = options.pop('distributed')
        pipeline_class = (pipeline_class if wttfj__xhzfk else bodo.compiler
            .BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        ogz__tnaa = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if ogz__tnaa else
            pipeline_class)
    jus__eau = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(jus__eau, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(jus__eau)
        else:
            return master_mode_wrapper(jus__eau)
    else:
        return jus__eau
