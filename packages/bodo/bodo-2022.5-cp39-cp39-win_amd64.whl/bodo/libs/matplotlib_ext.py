...
def install_mpl_class(types_name, python_type):
    rdn__zwwh = ''.join(map(str.title, types_name.split('_')))
    hem__ocbz = f'class {rdn__zwwh}(types.Opaque):\n'
    hem__ocbz += '    def __init__(self):\n'
    hem__ocbz += f"       types.Opaque.__init__(self, name='{rdn__zwwh}')\n"
    hem__ocbz += '    def __reduce__(self):\n'
    hem__ocbz += (
        f"        return (types.Opaque, ('{rdn__zwwh}',), self.__dict__)\n")
    ftqui__lxpin = {}
    exec(hem__ocbz, {'types': types, 'bodo': bodo}, ftqui__lxpin)
    vxcq__jzvd = ftqui__lxpin[rdn__zwwh]
    bifn__mzfx = sys.modules[__name__]
    setattr(bifn__mzfx, rdn__zwwh, vxcq__jzvd)
    class_instance = vxcq__jzvd()
    setattr(types, types_name, class_instance)
    register_model(vxcq__jzvd)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(vxcq__jzvd)(unbox_mpl_obj)
    box(vxcq__jzvd)(box_mpl_obj)
...
def _install_mpl_types():
    dla__uqxa = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for sgc__teryb, ccb__kxev in dla__uqxa:
        install_mpl_class(sgc__teryb, ccb__kxev)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    aulm__zrv = ', '.join(f'e{aln__smo}' for aln__smo in range(len(args)))
    if aulm__zrv:
        aulm__zrv += ', '
    vis__wkz = ', '.join(f"{yau__oiz} = ''" for yau__oiz in kws.keys())
    usgbs__azc = 'matplotlib_obj, ' if obj_typ is not None else ''
    jit__clut = f'def mpl_stub({usgbs__azc} {aulm__zrv} {vis__wkz}):\n'
    jit__clut += '    pass\n'
    xmntu__ldl = {}
    exec(jit__clut, {}, xmntu__ldl)
    vuafk__frzck = xmntu__ldl['mpl_stub']
    gsii__kwcb = numba.core.utils.pysignature(vuafk__frzck)
    cbkr__wrw = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(cbkr__wrw)).replace(pysig=
        gsii__kwcb)


def generate_axes_typing(mod_name, nrows, ncols):
    afj__xppmy = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(afj__xppmy.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(afj__xppmy.format(mod_name, 'ncols'))
    ask__nglw = get_overload_const_int(nrows)
    cilge__sdgg = get_overload_const_int(ncols)
    if ask__nglw < 1:
        raise BodoError(afj__xppmy.format(mod_name, 'nrows'))
    if cilge__sdgg < 1:
        raise BodoError(afj__xppmy.format(mod_name, 'ncols'))
    if ask__nglw == 1 and cilge__sdgg == 1:
        qlt__ckfom = types.mpl_axes_type
    else:
        if cilge__sdgg == 1:
            ghy__qzql = types.mpl_axes_type
        else:
            ghy__qzql = types.Tuple([types.mpl_axes_type] * cilge__sdgg)
        qlt__ckfom = types.Tuple([ghy__qzql] * ask__nglw)
    return qlt__ckfom


def generate_pie_return_type(args, kws):
    wfsvq__vgjmd = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if wfsvq__vgjmd == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    zcsm__uvku = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(zcsm__uvku):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(zcsm__uvku):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        ztv__ghwui = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, ztv__ghwui]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    ztv__ghwui = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols)
    sgc__teryb = str(ztv__ghwui)
    if not hasattr(types, sgc__teryb):
        sgc__teryb = f'objmode_type{ir_utils.next_label()}'
        setattr(types, sgc__teryb, ztv__ghwui)
    jit__clut = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{sgc__teryb}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    xmntu__ldl = {}
    exec(jit__clut, {'numba': numba, 'np': np}, xmntu__ldl)
    impl = xmntu__ldl['impl']
    return impl
