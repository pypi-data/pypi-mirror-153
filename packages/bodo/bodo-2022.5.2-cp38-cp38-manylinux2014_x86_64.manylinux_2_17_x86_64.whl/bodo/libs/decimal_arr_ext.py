...
def decimal_to_str_codegen(context, builder, signature, args, scale):
    ...
    qopyk__koemt = cgutils.create_struct_proxy(types.unicode_type)(context,
        builder)
    okv__ysd = lir.FunctionType(lir.VoidType(), [lir.IntType(128), lir.
        IntType(8).as_pointer().as_pointer(), lir.IntType(64).as_pointer(),
        lir.IntType(32)])
    dpvxc__thqaj = cgutils.get_or_insert_function(builder.module, okv__ysd,
        name='decimal_to_str')
    builder.call(dpvxc__thqaj, [val, qopyk__koemt._get_ptr_by_name(
        'meminfo'), qopyk__koemt._get_ptr_by_name('length'), scale])
    qopyk__koemt.kind = context.get_constant(types.int32, numba.cpython.
        unicode.PY_UNICODE_1BYTE_KIND)
    qopyk__koemt.is_ascii = context.get_constant(types.int32, 1)
    qopyk__koemt.hash = context.get_constant(numba.cpython.unicode.
        _Py_hash_t, -1)
    qopyk__koemt.data = context.nrt.meminfo_data(builder, qopyk__koemt.meminfo)
    qopyk__koemt.parent = cgutils.get_null_value(qopyk__koemt.parent.type)
    return qopyk__koemt._getvalue()
...
def str_to_decimal_codegen(context, builder, signature, args):
    val, klm__wjo, klm__wjo = args
    val = bodo.libs.str_ext.gen_unicode_to_std_str(context, builder, val)
    okv__ysd = lir.FunctionType(lir.IntType(128), [lir.IntType(8).as_pointer()]
        )
    dpvxc__thqaj = cgutils.get_or_insert_function(builder.module, okv__ysd,
        name='str_to_decimal')
    ytqni__wzg = builder.call(dpvxc__thqaj, [val])
    return ytqni__wzg
...
def decimal128type_to_int64_tuple(typingctx, val):
    ...
    def codegen(context, builder, signature, args):
        uspj__oywph = cgutils.alloca_once(builder, lir.ArrayType(lir.
            IntType(64), 2))
        builder.store(args[0], builder.bitcast(uspj__oywph, lir.IntType(128
            ).as_pointer()))
        return builder.load(uspj__oywph)
...
def decimal128type_cmp(typingctx, val1, scale1, val2, scale2, func_name):
    ...
    xzugg__gwh = get_overload_const_str(func_name)

    def codegen(context, builder, signature, args):
        val1, scale1, val2, scale2, klm__wjo = args
        okv__ysd = lir.FunctionType(lir.IntType(1), [lir.IntType(128), lir.
            IntType(64), lir.IntType(128), lir.IntType(64)])
        dpvxc__thqaj = cgutils.get_or_insert_function(builder.module,
            okv__ysd, name=xzugg__gwh)
        return builder.call(dpvxc__thqaj, (val1, scale1, val2, scale2))
...
def decimal_create_cmp_op_overload(op):
    ...
    def overload_cmp(lhs, rhs):
        if isinstance(lhs, Decimal128Type) and isinstance(rhs, Decimal128Type):
            xzugg__gwh = 'decimal_cmp_' + op.__name__
            scale1 = lhs.scale
            scale2 = rhs.scale

            def impl(lhs, rhs):
                return decimal128type_cmp(lhs, scale1, rhs, scale2, xzugg__gwh)
            return impl
    return overload_cmp
...
def lower_constant_decimal(context, builder, ty, pyval):
    yrtzg__yiot = numba.njit(lambda v: decimal128type_to_int64_tuple(v))(pyval)
    kjrec__hqcsb = [context.get_constant_generic(builder, types.int64, v) for
        v in yrtzg__yiot]
    lzdyl__tymzg = cgutils.pack_array(builder, kjrec__hqcsb)
    uspj__oywph = cgutils.alloca_once(builder, lir.IntType(128))
    builder.store(lzdyl__tymzg, builder.bitcast(uspj__oywph, lir.ArrayType(
...
def unbox_decimal(typ, val, c):
    okv__ysd = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer(
        ), lir.IntType(128).as_pointer()])
    dpvxc__thqaj = cgutils.get_or_insert_function(c.builder.module,
        okv__ysd, name='unbox_decimal')
    uspj__oywph = cgutils.alloca_once(c.builder, c.context.get_value_type(
        int128_type))
    c.builder.call(dpvxc__thqaj, [val, uspj__oywph])
    llqci__nzl = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    nlmor__kxt = c.builder.load(uspj__oywph)
    return NativeValue(nlmor__kxt, is_error=llqci__nzl)


@box(Decimal128Type)
def box_decimal(typ, val, c):
    tyi__yke = decimal_to_str_codegen(c.context, c.builder, bodo.
        string_type(typ), (val,), typ.scale)
    wltem__xgzj = c.pyapi.from_native_value(bodo.string_type, tyi__yke, c.
        env_manager)
    pkmb__lell = c.context.insert_const_string(c.builder.module, 'decimal')
    wjc__gbd = c.pyapi.import_module_noblock(pkmb__lell)
    uspj__oywph = c.pyapi.call_method(wjc__gbd, 'Decimal', (wltem__xgzj,))
    c.pyapi.decref(wltem__xgzj)
    c.pyapi.decref(wjc__gbd)
    return uspj__oywph
...
class DecimalArrayModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        jrr__ehlho = [('data', data_type), ('null_bitmap', nulls_type)]
        models.StructModel.__init__(self, dmm, fe_type, jrr__ehlho)
...
def init_decimal_array(typingctx, data, null_bitmap, precision_tp, scale_tp
    ...
    def codegen(context, builder, signature, args):
        ncnpn__drux, jtxp__tusph, klm__wjo, klm__wjo = args
        fszs__rgnnx = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        fszs__rgnnx.data = ncnpn__drux
        fszs__rgnnx.null_bitmap = jtxp__tusph
        context.nrt.incref(builder, signature.args[0], ncnpn__drux)
        context.nrt.incref(builder, signature.args[1], jtxp__tusph)
        return fszs__rgnnx._getvalue()
    precision = get_overload_const_int(precision_tp)
    scale = get_overload_const_int(scale_tp)
    wysb__ifw = DecimalArrayType(precision, scale)
    hrl__ttmd = wysb__ifw(data, null_bitmap, precision_tp, scale_tp)
    return hrl__ttmd, codegen


@lower_constant(DecimalArrayType)
def lower_constant_decimal_arr(context, builder, typ, pyval):
    n = len(pyval)
    fzd__absrs = context.get_constant(types.int64, n)
    ohvec__hsk = bodo.utils.utils._empty_nd_impl(context, builder, types.
        Array(int128_type, 1, 'C'), [fzd__absrs])
    nqh__kfgu = np.empty(n + 7 >> 3, np.uint8)

    def f(arr, idx, val):
        arr[idx] = decimal128type_to_int128(val)
    for ojmt__clsy, pfiwa__yuxbr in enumerate(pyval):
        tgcw__kwykv = pd.isna(pfiwa__yuxbr)
        bodo.libs.int_arr_ext.set_bit_to_arr(nqh__kfgu, ojmt__clsy, int(not
            tgcw__kwykv))
        if not tgcw__kwykv:
            context.compile_internal(builder, f, types.void(types.Array(
                int128_type, 1, 'C'), types.int64, Decimal128Type(typ.
                precision, typ.scale)), [ohvec__hsk._getvalue(), context.
                get_constant(types.int64, ojmt__clsy), context.
                get_constant_generic(builder, Decimal128Type(typ.precision,
                typ.scale), pfiwa__yuxbr)])
    udqbf__rwnh = context.get_constant_generic(builder, nulls_type, nqh__kfgu)
    fszs__rgnnx = context.make_helper(builder, typ)
    fszs__rgnnx.data = ohvec__hsk._getvalue()
    fszs__rgnnx.null_bitmap = udqbf__rwnh
    return fszs__rgnnx._getvalue()


@numba.njit(no_cpython_wrapper=True)
def alloc_decimal_array(n, precision, scale):
    zylzs__gdkz = np.empty(n, dtype=int128_type)
    hnsau__nrdp = np.empty(n + 7 >> 3, dtype=np.uint8)
    return init_decimal_array(zylzs__gdkz, hnsau__nrdp, precision, scale)
...
def box_decimal_arr(typ, val, c):
    xzpe__btifr = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    zylzs__gdkz = c.context.make_array(types.Array(int128_type, 1, 'C'))(c.
        context, c.builder, xzpe__btifr.data)
    korrz__pbh = c.context.make_array(types.Array(types.uint8, 1, 'C'))(c.
        context, c.builder, xzpe__btifr.null_bitmap).data
    n = c.builder.extract_value(zylzs__gdkz.shape, 0)
    scale = c.context.get_constant(types.int32, typ.scale)
    okv__ysd = lir.FunctionType(c.pyapi.pyobj, [lir.IntType(64), lir.
        IntType(128).as_pointer(), lir.IntType(8).as_pointer(), lir.IntType
        (32)])
    esjl__arz = cgutils.get_or_insert_function(c.builder.module, okv__ysd,
        name='box_decimal_array')
    mohr__yqoop = c.builder.call(esjl__arz, [n, zylzs__gdkz.data,
        korrz__pbh, scale])
...
def unbox_decimal_arr(typ, val, c):
    fszs__rgnnx = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    azhmj__htf = c.pyapi.call_method(val, '__len__', ())
    n = c.pyapi.long_as_longlong(azhmj__htf)
    c.pyapi.decref(azhmj__htf)
    btvnb__yaab = c.builder.udiv(c.builder.add(n, lir.Constant(lir.IntType(
        64), 7)), lir.Constant(lir.IntType(64), 8))
    ohvec__hsk = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(int128_type, 1, 'C'), [n])
    hcxdm__bjh = bodo.utils.utils._empty_nd_impl(c.context, c.builder,
        types.Array(types.uint8, 1, 'C'), [btvnb__yaab])
    okv__ysd = lir.FunctionType(lir.VoidType(), [lir.IntType(8).as_pointer(
        ), lir.IntType(64), lir.IntType(128).as_pointer(), lir.IntType(8).
        as_pointer()])
    dpvxc__thqaj = cgutils.get_or_insert_function(c.builder.module,
        okv__ysd, name='unbox_decimal_array')
    c.builder.call(dpvxc__thqaj, [val, n, ohvec__hsk.data, hcxdm__bjh.data])
    fszs__rgnnx.null_bitmap = hcxdm__bjh._getvalue()
    fszs__rgnnx.data = ohvec__hsk._getvalue()
    llqci__nzl = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(fszs__rgnnx._getvalue(), is_error=llqci__nzl)
...
def decimal_arr_getitem(A, ind):
    ...
    if is_list_like_index_type(ind) and ind.dtype == types.bool_:
        ...
        def impl(A, ind):
            dnm__jzq, xcit__yiq = array_getitem_bool_index(A, ind)
            return init_decimal_array(dnm__jzq, xcit__yiq, precision, scale)
        return impl
    if is_list_like_index_type(ind) and isinstance(ind.dtype, types.Integer):
        ...
        def impl(A, ind):
            dnm__jzq, xcit__yiq = array_getitem_int_index(A, ind)
            return init_decimal_array(dnm__jzq, xcit__yiq, precision, scale)
        return impl
    if isinstance(ind, types.SliceType):
        ...
        def impl_slice(A, ind):
            dnm__jzq, xcit__yiq = array_getitem_slice_index(A, ind)
            return init_decimal_array(dnm__jzq, xcit__yiq, precision, scale)
        return impl_slice
    raise BodoError(
