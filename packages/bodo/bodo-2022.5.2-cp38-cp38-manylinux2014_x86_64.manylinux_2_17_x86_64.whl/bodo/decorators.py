...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        fowo__cui = self.overloads[sig]
        ggs__eptza = fowo__cui.metadata.get('distributed_diagnostics', None)
        if ggs__eptza is None:
            hpc__stn = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(hpc__stn)
        ggs__eptza.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        jeiwe__tohp = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(jeiwe__tohp)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        emon__vmvx = options.pop('distributed')
        pipeline_class = (pipeline_class if emon__vmvx else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        qlhz__qgbj = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if qlhz__qgbj else
            pipeline_class)
    edr__nwcha = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(edr__nwcha, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(edr__nwcha)
        else:
            return master_mode_wrapper(edr__nwcha)
    else:
        return edr__nwcha
