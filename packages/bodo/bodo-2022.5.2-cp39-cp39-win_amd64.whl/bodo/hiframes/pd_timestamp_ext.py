...
class PandasTimestampType(types.Type):
    ...
    def __init__(self, tz_val=None):
        self.tz = tz_val
        if tz_val is None:
            arub__qydxa = 'PandasTimestampType()'
        else:
            arub__qydxa = f'PandasTimestampType({tz_val})'
        super(PandasTimestampType, self).__init__(name=arub__qydxa)
...
def check_tz_aware_unsupported(val, func_name):
    ...
    if isinstance(val, PandasTimestampType) and val.tz is not None:
        ...
        for dxn__bhget in val.data:
            if isinstance(dxn__bhget, bodo.DatetimeArrayType):
                raise BodoError(
...
class PandasTimestampModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        nuw__imzyy = [('year', ts_field_typ), ('month', ts_field_typ), (
            ...
            ts_field_typ)]
        models.StructModel.__init__(self, dmm, fe_type, nuw__imzyy)
...
def unbox_pandas_timestamp(typ, val, c):
    rzmkz__qpcj = c.pyapi.object_getattr_string(val, 'year')
    tnv__hqlwr = c.pyapi.object_getattr_string(val, 'month')
    riaq__ywtx = c.pyapi.object_getattr_string(val, 'day')
    grez__nay = c.pyapi.object_getattr_string(val, 'hour')
    wytle__vtqq = c.pyapi.object_getattr_string(val, 'minute')
    fny__loqk = c.pyapi.object_getattr_string(val, 'second')
    jpqq__sxae = c.pyapi.object_getattr_string(val, 'microsecond')
    yna__gpa = c.pyapi.object_getattr_string(val, 'nanosecond')
    cagy__kcz = c.pyapi.object_getattr_string(val, 'value')
    hffph__qwzzs = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    hffph__qwzzs.year = c.pyapi.long_as_longlong(rzmkz__qpcj)
    hffph__qwzzs.month = c.pyapi.long_as_longlong(tnv__hqlwr)
    hffph__qwzzs.day = c.pyapi.long_as_longlong(riaq__ywtx)
    hffph__qwzzs.hour = c.pyapi.long_as_longlong(grez__nay)
    hffph__qwzzs.minute = c.pyapi.long_as_longlong(wytle__vtqq)
    hffph__qwzzs.second = c.pyapi.long_as_longlong(fny__loqk)
    hffph__qwzzs.microsecond = c.pyapi.long_as_longlong(jpqq__sxae)
    hffph__qwzzs.nanosecond = c.pyapi.long_as_longlong(yna__gpa)
    hffph__qwzzs.value = c.pyapi.long_as_longlong(cagy__kcz)
    c.pyapi.decref(rzmkz__qpcj)
    c.pyapi.decref(tnv__hqlwr)
    c.pyapi.decref(riaq__ywtx)
    c.pyapi.decref(grez__nay)
    c.pyapi.decref(wytle__vtqq)
    c.pyapi.decref(fny__loqk)
    c.pyapi.decref(jpqq__sxae)
    c.pyapi.decref(yna__gpa)
    c.pyapi.decref(cagy__kcz)
    yio__cguh = cgutils.is_not_null(c.builder, c.pyapi.err_occurred())
    return NativeValue(hffph__qwzzs._getvalue(), is_error=yio__cguh)


@box(PandasTimestampType)
def box_pandas_timestamp(typ, val, c):
    hbned__yemje = cgutils.create_struct_proxy(typ)(c.context, c.builder,
        value=val)
    rzmkz__qpcj = c.pyapi.long_from_longlong(hbned__yemje.year)
    tnv__hqlwr = c.pyapi.long_from_longlong(hbned__yemje.month)
    riaq__ywtx = c.pyapi.long_from_longlong(hbned__yemje.day)
    grez__nay = c.pyapi.long_from_longlong(hbned__yemje.hour)
    wytle__vtqq = c.pyapi.long_from_longlong(hbned__yemje.minute)
    fny__loqk = c.pyapi.long_from_longlong(hbned__yemje.second)
    bbun__jyhai = c.pyapi.long_from_longlong(hbned__yemje.microsecond)
    eww__jyd = c.pyapi.long_from_longlong(hbned__yemje.nanosecond)
    fcu__eza = c.pyapi.unserialize(c.pyapi.serialize_object(pd.Timestamp))
    if typ.tz is None:
        res = c.pyapi.call_function_objargs(fcu__eza, (rzmkz__qpcj,
            tnv__hqlwr, riaq__ywtx, grez__nay, wytle__vtqq, fny__loqk,
            bbun__jyhai, eww__jyd))
    else:
        if isinstance(typ.tz, int):
            gelzr__nxbau = c.pyapi.long_from_longlong(lir.Constant(lir.
                IntType(64), typ.tz))
        else:
            bnnl__ljwn = c.context.insert_const_string(c.builder.module,
                str(typ.tz))
            gelzr__nxbau = c.pyapi.string_from_string(bnnl__ljwn)
        args = c.pyapi.tuple_pack(())
        kwargs = c.pyapi.dict_pack([('year', rzmkz__qpcj), ('month',
            tnv__hqlwr), ('day', riaq__ywtx), ('hour', grez__nay), (
            'minute', wytle__vtqq), ('second', fny__loqk), ('microsecond',
            bbun__jyhai), ('nanosecond', eww__jyd), ('tz', gelzr__nxbau)])
        res = c.pyapi.call(fcu__eza, args, kwargs)
        c.pyapi.decref(args)
        c.pyapi.decref(kwargs)
        c.pyapi.decref(gelzr__nxbau)
    c.pyapi.decref(rzmkz__qpcj)
    c.pyapi.decref(tnv__hqlwr)
    c.pyapi.decref(riaq__ywtx)
    c.pyapi.decref(grez__nay)
    c.pyapi.decref(wytle__vtqq)
    c.pyapi.decref(fny__loqk)
    c.pyapi.decref(bbun__jyhai)
    c.pyapi.decref(eww__jyd)
    return res
...
def init_timestamp(typingctx, year, month, day, hour, minute, second,
    ...
    def codegen(context, builder, sig, args):
        (year, month, day, hour, minute, second, kbzp__vqft, hjq__nds,
            value, ede__ohoki) = args
        ...
        ts.minute = minute
        ts.second = second
        ts.microsecond = kbzp__vqft
        ts.nanosecond = hjq__nds
        ts.value = value
        return ts._getvalue()
...
def overload_pd_timestamp(ts_input=_no_input, freq=None, tz=None, unit=None,
    ...
    if isinstance(ts_input, types.Number):
        ...
        odflo__jinqe, precision = (pd._libs.tslibs.conversion.
        ...
        def impl_float(ts_input=_no_input, freq=None, tz=None, unit=None,
            year=None, month=None, day=None, hour=None, minute=None, second
            =None, microsecond=None, nanosecond=None, tzinfo=None):
            ieqyc__csq = np.int64(ts_input)
            tqyp__nfywx = ts_input - ieqyc__csq
            if precision:
                tqyp__nfywx = np.round(tqyp__nfywx, precision)
            value = ieqyc__csq * odflo__jinqe + np.int64(tqyp__nfywx *
                odflo__jinqe)
            return convert_val_to_timestamp(value, tz)
        return impl_float
    ...
    if isinstance(ts_input, numba.core.types.scalars.NPDatetime):
        ...
        def impl_date(ts_input=_no_input, freq=None, tz=None, unit=None,
            year=None, month=None, day=None, hour=None, minute=None, second
            =None, microsecond=None, nanosecond=None, tzinfo=None):
            value = np.int64(ts_input) * odflo__jinqe
...
def overload_week_number(ptt):
    ...
    def pd_week_number(ptt):
        ede__ohoki, pfln__mla, ede__ohoki = get_isocalendar(ptt.year, ptt.
            month, ptt.day)
        return pfln__mla
...
def overload_pd_timestamp_isocalendar(ptt):
    ...
    def impl(ptt):
        year, pfln__mla, veln__ias = get_isocalendar(ptt.year, ptt.month,
            ptt.day)
        return year, pfln__mla, veln__ias
    return impl
...
def overload_pd_timestamp_isoformat(ts, sep=None):
    if is_overload_none(sep):
        ...
        def timestamp_isoformat_impl(ts, sep=None):
            ...
            lmvql__wtgh = str_2d(ts.hour) + ':' + str_2d(ts.minute
                ) + ':' + str_2d(ts.second)
...
def overload_pd_timestamp_day_name(ptt, locale=None):
    hnv__aou = dict(locale=locale)
    aehro__yku = dict(locale=None)
    check_unsupported_args('Timestamp.day_name', hnv__aou, aehro__yku,
        package_name='pandas', module_name='Timestamp')

    def impl(ptt, locale=None):
        weo__wdw = ('Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday',
            'Saturday', 'Sunday')
        ede__ohoki, ede__ohoki, sqmwi__boz = ptt.isocalendar()
        return weo__wdw[sqmwi__boz - 1]
    return impl
...
def overload_pd_timestamp_month_name(ptt, locale=None):
    hnv__aou = dict(locale=locale)
    aehro__yku = dict(locale=None)
    check_unsupported_args('Timestamp.month_name', hnv__aou, aehro__yku,
        package_name='pandas', module_name='Timestamp')

    def impl(ptt, locale=None):
        fikzv__pvdt = ('January', 'February', 'March', 'April', 'May',
            'June', 'July', 'August', 'September', 'October', 'November',
            'December')
        return fikzv__pvdt[ptt.month - 1]
    return impl
...
def overload_pd_timestamp_tz_localize(ptt, tz, ambiguous='raise',
    ...
    hnv__aou = dict(ambiguous=ambiguous, nonexistent=nonexistent)
    nwkb__xoge = dict(ambiguous='raise', nonexistent='raise')
    check_unsupported_args('Timestamp.tz_localize', hnv__aou, nwkb__xoge,
        package_name='pandas', module_name='Timestamp')
    if is_overload_none(tz):
...
def extract_year_days(typingctx, dt64_t=None):
    ...
    def codegen(context, builder, sig, args):
        evv__wmg = cgutils.alloca_once(builder, lir.IntType(64))
        builder.store(args[0], evv__wmg)
        year = cgutils.alloca_once(builder, lir.IntType(64))
        buv__ykxwb = cgutils.alloca_once(builder, lir.IntType(64))
        fekqq__hkubz = lir.FunctionType(lir.VoidType(), [lir.IntType(64).
            as_pointer(), lir.IntType(64).as_pointer(), lir.IntType(64).
            as_pointer()])
        xat__gnja = cgutils.get_or_insert_function(builder.module,
            fekqq__hkubz, name='extract_year_days')
        builder.call(xat__gnja, [evv__wmg, year, buv__ykxwb])
        return cgutils.pack_array(builder, [builder.load(evv__wmg), builder
            .load(year), builder.load(buv__ykxwb)])
...
def get_month_day(typingctx, year_t, days_t=None):
    ...
    def codegen(context, builder, sig, args):
        ...
        fekqq__hkubz = lir.FunctionType(lir.VoidType(), [lir.IntType(64),
            ...
            as_pointer()])
        xat__gnja = cgutils.get_or_insert_function(builder.module,
            fekqq__hkubz, name='get_month_day')
        builder.call(xat__gnja, [args[0], args[1], month, day])
...
def get_day_of_year(year, month, day):
    dyiq__mpd = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334, 365,
        0, 31, 60, 91, 121, 152, 182, 213, 244, 274, 305, 335, 366]
    ewv__yath = is_leap_year(year)
    xqlzg__wcem = dyiq__mpd[ewv__yath * 13 + month - 1]
    axpyo__gppom = xqlzg__wcem + day
    return axpyo__gppom


@register_jitable
def get_day_of_week(y, m, d):
    adjag__aictp = [0, 3, 2, 5, 0, 3, 5, 1, 4, 6, 2, 4]
    y -= m < 3
    day = (y + y // 4 - y // 100 + y // 400 + adjag__aictp[m - 1] + d) % 7
    return (day + 6) % 7
...
def get_days_in_month(year, month):
    ...
    bvsb__yvl = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31, 31, 29, 31,
        30, 31, 30, 31, 31, 30, 31, 30, 31]
    return bvsb__yvl[12 * is_leap_year + month - 1]
...
def convert_val_to_timestamp(ts_input, tz=None, is_convert=True):
    xojpn__xldgx = pexrb__tkq = np.array([])
    qlfmr__bhiyu = '0'
    if is_overload_constant_str(tz):
        bnnl__ljwn = get_overload_const_str(tz)
        gelzr__nxbau = pytz.timezone(bnnl__ljwn)
        if isinstance(gelzr__nxbau, pytz.tzinfo.DstTzInfo):
            xojpn__xldgx = np.array(gelzr__nxbau._utc_transition_times,
                dtype='M8[ns]').view('i8')
            pexrb__tkq = np.array(gelzr__nxbau._transition_info)[:, 0]
            pexrb__tkq = (pd.Series(pexrb__tkq).dt.total_seconds() * 1000000000
                ).astype(np.int64).values
            qlfmr__bhiyu = (
                "deltas[np.searchsorted(trans, ts_input, side='right') - 1]")
        else:
            pexrb__tkq = np.int64(gelzr__nxbau._utcoffset.total_seconds() *
                1000000000)
            qlfmr__bhiyu = 'deltas'
    elif is_overload_constant_int(tz):
        mmc__zhed = get_overload_const_int(tz)
        qlfmr__bhiyu = str(mmc__zhed)
    ...
    if is_convert:
        zoc__gunb = 'tz_ts_input'
        hjc__xkcfx = 'ts_input'
    else:
        zoc__gunb = 'ts_input'
        hjc__xkcfx = 'tz_ts_input'
    yxy__zhqvf = 'def impl(ts_input, tz=None, is_convert=True):\n'
    yxy__zhqvf += f'  tz_ts_input = ts_input + {qlfmr__bhiyu}\n'
    yxy__zhqvf += (
        f'  dt, year, days = extract_year_days(integer_to_dt64({zoc__gunb}))\n'
        )
    yxy__zhqvf += '  month, day = get_month_day(year, days)\n'
    yxy__zhqvf += '  return init_timestamp(\n'
    yxy__zhqvf += '    year=year,\n'
    yxy__zhqvf += '    month=month,\n'
    yxy__zhqvf += '    day=day,\n'
    yxy__zhqvf += '    hour=dt // (60 * 60 * 1_000_000_000),\n'
    yxy__zhqvf += '    minute=(dt // (60 * 1_000_000_000)) % 60,\n'
    yxy__zhqvf += '    second=(dt // 1_000_000_000) % 60,\n'
    yxy__zhqvf += '    microsecond=(dt // 1000) % 1_000_000,\n'
    yxy__zhqvf += '    nanosecond=dt % 1000,\n'
    yxy__zhqvf += f'    value={hjc__xkcfx},\n'
    yxy__zhqvf += '    tz=tz,\n'
    yxy__zhqvf += '  )\n'
    cju__fjxw = {}
    exec(yxy__zhqvf, {'np': np, 'pd': pd, 'trans': xojpn__xldgx, 'deltas':
        pexrb__tkq, 'integer_to_dt64': integer_to_dt64, 'extract_year_days':
        extract_year_days, 'get_month_day': get_month_day, 'init_timestamp':
        init_timestamp, 'zero_if_none': zero_if_none}, cju__fjxw)
    impl = cju__fjxw['impl']
    return impl
...
def convert_datetime64_to_timestamp(dt64):
    evv__wmg, year, buv__ykxwb = extract_year_days(dt64)
    month, day = get_month_day(year, buv__ykxwb)
    return init_timestamp(year=year, month=month, day=day, hour=evv__wmg //
        (60 * 60 * 1000000000), minute=evv__wmg // (60 * 1000000000) % 60,
        second=evv__wmg // 1000000000 % 60, microsecond=evv__wmg // 1000 % 
        1000000, nanosecond=evv__wmg % 1000, value=dt64, tz=None)


@numba.njit(no_cpython_wrapper=True)
def convert_numpy_timedelta64_to_datetime_timedelta(dt64):
    jywtf__zvegy = (bodo.hiframes.datetime_timedelta_ext.
        cast_numpy_timedelta_to_int(dt64))
    yga__nnrpq = jywtf__zvegy // (86400 * 1000000000)
    aiyh__qegb = jywtf__zvegy - yga__nnrpq * 86400 * 1000000000
    ssvj__dhe = aiyh__qegb // 1000000000
    roapq__hge = aiyh__qegb - ssvj__dhe * 1000000000
    tutan__yvq = roapq__hge // 1000
    return datetime.timedelta(yga__nnrpq, ssvj__dhe, tutan__yvq)
...
def convert_numpy_timedelta64_to_pd_timedelta(dt64):
    jywtf__zvegy = (bodo.hiframes.datetime_timedelta_ext.
        cast_numpy_timedelta_to_int(dt64))
    return pd.Timedelta(jywtf__zvegy)
...
def pandas_dict_string_array_to_datetime(arr, errors, dayfirst, yearfirst,
    ...
    jzyvl__moh = len(arr)
    rgdub__gqlz = np.empty(jzyvl__moh, 'datetime64[ns]')
    xni__kyi = arr._indices
    tfru__ywjer = pandas_string_array_to_datetime(arr._data, errors,
        dayfirst, yearfirst, utc, format, exact, unit,
        infer_datetime_format, origin, cache).values
    for arbrz__bbfpw in range(jzyvl__moh):
        if bodo.libs.array_kernels.isna(xni__kyi, arbrz__bbfpw):
            bodo.libs.array_kernels.setna(rgdub__gqlz, arbrz__bbfpw)
            continue
        rgdub__gqlz[arbrz__bbfpw] = tfru__ywjer[xni__kyi[arbrz__bbfpw]]
    return rgdub__gqlz
...
def overload_to_datetime(arg_a, errors='raise', dayfirst=False, yearfirst=
    ...
    if isinstance(arg_a, bodo.hiframes.pd_series_ext.SeriesType):
        ...
        def impl_series(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            gyige__fpo = bodo.hiframes.pd_series_ext.get_series_index(arg_a)
            arub__qydxa = bodo.hiframes.pd_series_ext.get_series_name(arg_a)
            etbao__unqx = bodo.utils.conversion.coerce_to_ndarray(pd.
            ...
            return bodo.hiframes.pd_series_ext.init_series(etbao__unqx,
                gyige__fpo, arub__qydxa)
        return impl_series
    if arg_a == bodo.hiframes.datetime_date_ext.datetime_date_array_type:
        divb__vnwbf = np.dtype('datetime64[ns]')
        iNaT = pd._libs.tslibs.iNaT

        def impl_date_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            False, utc=None, format=None, exact=True, unit=None,
            infer_datetime_format=False, origin='unix', cache=True):
            jzyvl__moh = len(arg_a)
            rgdub__gqlz = np.empty(jzyvl__moh, divb__vnwbf)
            for arbrz__bbfpw in numba.parfors.parfor.internal_prange(jzyvl__moh
                ):
                val = iNaT
                if not bodo.libs.array_kernels.isna(arg_a, arbrz__bbfpw):
                    data = arg_a[arbrz__bbfpw]
                    val = (bodo.hiframes.pd_timestamp_ext.
                        npy_datetimestruct_to_datetime(data.year, data.
                        month, data.day, 0, 0, 0, 0))
                rgdub__gqlz[arbrz__bbfpw
                    ] = bodo.hiframes.pd_timestamp_ext.integer_to_dt64(val)
            return bodo.hiframes.pd_index_ext.init_datetime_index(rgdub__gqlz,
                None)
    ...
    if isinstance(arg_a, CategoricalArrayType
        ...
        def impl_cat_arr(arg_a, errors='raise', dayfirst=False, yearfirst=
            ...
            jzyvl__moh = len(arg_a)
            rgdub__gqlz = np.empty(jzyvl__moh, divb__vnwbf)
            ihaj__bue = (bodo.hiframes.pd_categorical_ext.
                get_categorical_arr_codes(arg_a))
            tfru__ywjer = pandas_string_array_to_datetime(arg_a.dtype.
                categories.values, errors, dayfirst, yearfirst, utc, format,
                exact, unit, infer_datetime_format, origin, cache).values
            for arbrz__bbfpw in numba.parfors.parfor.internal_prange(jzyvl__moh
                ):
                c = ihaj__bue[arbrz__bbfpw]
                if c == -1:
                    bodo.libs.array_kernels.setna(rgdub__gqlz, arbrz__bbfpw)
                    continue
                rgdub__gqlz[arbrz__bbfpw] = tfru__ywjer[c]
            return bodo.hiframes.pd_index_ext.init_datetime_index(rgdub__gqlz,
                None)
        return impl_cat_arr
    if arg_a == bodo.dict_str_arr_type:
        ...
        def impl_dict_str_arr(arg_a, errors='raise', dayfirst=False,
            yearfirst=False, utc=None, format=None, exact=True, unit=None,
            infer_datetime_format=False, origin='unix', cache=True):
            rgdub__gqlz = pandas_dict_string_array_to_datetime(arg_a,
...
def overload_to_timedelta(arg_a, unit='ns', errors='raise'):
    ...
    if isinstance(arg_a, types.Float):
        m, zlzf__uku = pd._libs.tslibs.conversion.precision_from_unit(unit)

        def impl_float_scalar(arg_a, unit='ns', errors='raise'):
            val = float_to_timedelta_val(arg_a, zlzf__uku, m)
            return pd.Timedelta(val)
        return impl_float_scalar
    if isinstance(arg_a, types.Integer):
        m, ede__ohoki = pd._libs.tslibs.conversion.precision_from_unit(unit)
    ...
    if is_iterable_type(arg_a) and not isinstance(arg_a, types.BaseTuple):
        m, zlzf__uku = pd._libs.tslibs.conversion.precision_from_unit(unit)
        prju__ihnmq = np.dtype('timedelta64[ns]')
        if isinstance(arg_a.dtype, types.Float):

            def impl_float(arg_a, unit='ns', errors='raise'):
                jzyvl__moh = len(arg_a)
                rgdub__gqlz = np.empty(jzyvl__moh, prju__ihnmq)
                for arbrz__bbfpw in numba.parfors.parfor.internal_prange(
                    jzyvl__moh):
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, arbrz__bbfpw):
                        val = float_to_timedelta_val(arg_a[arbrz__bbfpw],
                            zlzf__uku, m)
                    rgdub__gqlz[arbrz__bbfpw
        ...
        if arg_a.dtype == datetime_timedelta_type:
            ...
            def impl_datetime_timedelta(arg_a, unit='ns', errors='raise'):
                ...
                for arbrz__bbfpw in numba.parfors.parfor.internal_prange(
                    ...
                    val = iNaT
                    if not bodo.libs.array_kernels.isna(arg_a, arbrz__bbfpw):
                        ibj__dni = arg_a[arbrz__bbfpw]
                        val = (ibj__dni.microseconds + 1000 * 1000 * (
                            ibj__dni.seconds + 24 * 60 * 60 * ibj__dni.days)
                            ) * 1000
                    rgdub__gqlz[arbrz__bbfpw
                        ] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                        val)
...
def float_to_timedelta_val(data, precision, multiplier):
    ieqyc__csq = np.int64(data)
    tqyp__nfywx = data - ieqyc__csq
    if precision:
        tqyp__nfywx = np.round(tqyp__nfywx, precision)
    return ieqyc__csq * multiplier + np.int64(tqyp__nfywx * multiplier)
...
def overload_freq_methods(method):
    ...
    def freq_overload(td, freq, ambiguous='raise', nonexistent='raise'):
        ...
        hnv__aou = dict(ambiguous=ambiguous, nonexistent=nonexistent)
        ukdvn__ppmbt = dict(ambiguous='raise', nonexistent='raise')
        check_unsupported_args(f'Timestamp.{method}', hnv__aou,
            ukdvn__ppmbt, package_name='pandas', module_name='Timestamp')
        irt__azcc = ["freq == 'D'", "freq == 'H'",
            "freq == 'min' or freq == 'T'", "freq == 'S'",
            "freq == 'ms' or freq == 'L'", "freq == 'U' or freq == 'us'",
            "freq == 'N'"]
        hzkgr__covf = [24 * 60 * 60 * 1000000 * 1000, 60 * 60 * 1000000 * 
            1000, 60 * 1000000 * 1000, 1000000 * 1000, 1000 * 1000, 1000, 1]
        yxy__zhqvf = (
            "def impl(td, freq, ambiguous='raise', nonexistent='raise'):\n")
        for arbrz__bbfpw, yex__vyx in enumerate(irt__azcc):
            bgoi__sap = 'if' if arbrz__bbfpw == 0 else 'elif'
            yxy__zhqvf += '    {} {}:\n'.format(bgoi__sap, yex__vyx)
            yxy__zhqvf += '        unit_value = {}\n'.format(hzkgr__covf[
                arbrz__bbfpw])
        yxy__zhqvf += '    else:\n'
        yxy__zhqvf += (
...
"""
            ...
            if method == 'round':
                yxy__zhqvf += '    if unit_value == 1:\n'
                yxy__zhqvf += '        value = td.value\n'
                yxy__zhqvf += '    else:\n'
                yxy__zhqvf += (
                    '        quotient, remainder = np.divmod(td.value, unit_value)\n'
                    )
                yxy__zhqvf += """        mask = np.logical_or(remainder > (unit_value // 2), np.logical_and(remainder == (unit_value // 2), quotient % 2))
"""
                yxy__zhqvf += '        if mask:\n'
                yxy__zhqvf += '            quotient = quotient + 1\n'
                yxy__zhqvf += '        value = quotient * unit_value\n'
            yxy__zhqvf += '    return pd.Timestamp(value)\n'
        cju__fjxw = {}
        exec(yxy__zhqvf, {'np': np, 'pd': pd}, cju__fjxw)
        impl = cju__fjxw['impl']
        return impl
...
def _install_freq_methods():
    bgj__utxu = ['ceil', 'floor', 'round']
    for method in bgj__utxu:
        xrbj__wcr = overload_freq_methods(method)
        overload_method(PDTimeDeltaType, method, no_unliteral=True)(xrbj__wcr)
...
def compute_pd_timestamp(totmicrosec, nanosecond):
    ...
    rwrvc__xmt = totmicrosec // 1000000
    second = rwrvc__xmt % 60
    wwny__hmnrr = rwrvc__xmt // 60
    minute = wwny__hmnrr % 60
    ipd__iqw = wwny__hmnrr // 60
    hour = ipd__iqw % 24
    chj__xmc = ipd__iqw // 24
    year, month, day = _ord2ymd(chj__xmc)
...
def overload_sub_operator_timestamp(lhs, rhs):
    if lhs == pd_timestamp_type and rhs == datetime_timedelta_type:
        ...
        def impl(lhs, rhs):
            jowvw__trqec = lhs.toordinal()
            zjsng__cqr = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            halbz__xctxl = lhs.microsecond
            nanosecond = lhs.nanosecond
            bup__ytffd = rhs.days
            phyj__dtyr = rhs.seconds
            tzqte__ipax = rhs.microseconds
            loms__huvp = jowvw__trqec - bup__ytffd
            hrjec__sqx = zjsng__cqr - phyj__dtyr
            xda__qjbk = halbz__xctxl - tzqte__ipax
            totmicrosec = 1000000 * (loms__huvp * 86400 + hrjec__sqx
                ) + xda__qjbk
...
def overload_add_operator_timestamp(lhs, rhs):
    ...
    if lhs == pd_timestamp_type and rhs == pd_timedelta_type:
        ...
        def impl(lhs, rhs):
            ...
            zjsng__cqr = lhs.second + lhs.minute * 60 + lhs.hour * 3600
            halbz__xctxl = lhs.microsecond
            urvfs__rwhz = lhs.nanosecond
            tzqte__ipax = rhs.value // 1000
            gprp__zfqrs = rhs.nanoseconds
            xda__qjbk = halbz__xctxl + tzqte__ipax
            totmicrosec = 1000000 * (jowvw__trqec * 86400 + zjsng__cqr
                ) + xda__qjbk
            pdo__vndlz = urvfs__rwhz + gprp__zfqrs
            return compute_pd_timestamp(totmicrosec, pdo__vndlz)
        return impl
...
def strftime(ts, format):
    if isinstance(ts, DatetimeDateType):
        lxl__ifgtm = 'datetime.date'
    else:
        lxl__ifgtm = 'pandas.Timestamp'
    if types.unliteral(format) != types.unicode_type:
        raise BodoError(
            f"{lxl__ifgtm}.strftime(): 'strftime' argument must be a string")
...
def typeof_python_calendar(val, c):
    return types.Tuple([types.StringLiteral(vawr__sws) for vawr__sws in val])
...
def _install_pd_timestamp_unsupported():
    ...
    for uuhxo__djv in timestamp_unsupported_attrs:
        opg__rpumv = 'pandas.Timestamp.' + uuhxo__djv
        overload_attribute(PandasTimestampType, uuhxo__djv)(
            create_unsupported_overload(opg__rpumv))
    for tbo__pukv in timestamp_unsupported_methods:
        opg__rpumv = 'pandas.Timestamp.' + tbo__pukv
        overload_method(PandasTimestampType, tbo__pukv)(
            create_unsupported_overload(opg__rpumv + '()'))
