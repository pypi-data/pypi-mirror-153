...
def _global_except_hook(exctype, value, traceback):
    ...
    gkzg__hmbqs = MPI.COMM_WORLD
    kwh__rfveb = gkzg__hmbqs.Get_rank()
    xday__wzxj = 3.0
    kapxe__qfgm = True
    qtcm__jlwd = gkzg__hmbqs.Ibarrier()
    mvwvv__prf = time.time()
    while time.time() - mvwvv__prf < xday__wzxj:
        time.sleep(0.1)
        if qtcm__jlwd.Test():
            kapxe__qfgm = False
            break
    try:
        ...
        if kapxe__qfgm:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {kwh__rfveb}. \n')
