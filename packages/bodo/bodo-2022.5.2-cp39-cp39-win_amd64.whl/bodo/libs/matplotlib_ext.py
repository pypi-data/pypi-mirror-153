...
def install_mpl_class(types_name, python_type):
    hfxi__cry = ''.join(map(str.title, types_name.split('_')))
    lrv__cdpy = f'class {hfxi__cry}(types.Opaque):\n'
    lrv__cdpy += '    def __init__(self):\n'
    lrv__cdpy += f"       types.Opaque.__init__(self, name='{hfxi__cry}')\n"
    lrv__cdpy += '    def __reduce__(self):\n'
    lrv__cdpy += (
        f"        return (types.Opaque, ('{hfxi__cry}',), self.__dict__)\n")
    xjmt__vjv = {}
    exec(lrv__cdpy, {'types': types, 'bodo': bodo}, xjmt__vjv)
    lzlbh__eup = xjmt__vjv[hfxi__cry]
    riuk__qjg = sys.modules[__name__]
    setattr(riuk__qjg, hfxi__cry, lzlbh__eup)
    class_instance = lzlbh__eup()
    setattr(types, types_name, class_instance)
    register_model(lzlbh__eup)(models.OpaqueModel)
    typeof_impl.register(python_type)(lambda val, c: class_instance)
    unbox(lzlbh__eup)(unbox_mpl_obj)
    box(lzlbh__eup)(box_mpl_obj)
...
def _install_mpl_types():
    iozbg__dlm = [('mpl_figure_type', matplotlib.figure.Figure), (
    ...
    for zivvl__hhxl, bpy__usk in iozbg__dlm:
        install_mpl_class(zivvl__hhxl, bpy__usk)
...
def generate_matplotlib_signature(return_typ, args, kws, obj_typ=None):
    kws = dict(kws)
    ovd__ofq = ', '.join(f'e{xkk__fzo}' for xkk__fzo in range(len(args)))
    if ovd__ofq:
        ovd__ofq += ', '
    ygz__xwo = ', '.join(f"{xheu__exzz} = ''" for xheu__exzz in kws.keys())
    ozavc__icq = 'matplotlib_obj, ' if obj_typ is not None else ''
    pxsl__agqze = f'def mpl_stub({ozavc__icq} {ovd__ofq} {ygz__xwo}):\n'
    pxsl__agqze += '    pass\n'
    lpgn__nfbxy = {}
    exec(pxsl__agqze, {}, lpgn__nfbxy)
    cablt__dpf = lpgn__nfbxy['mpl_stub']
    yefzw__ktey = numba.core.utils.pysignature(cablt__dpf)
    wrxa__imb = ((obj_typ,) if obj_typ is not None else ()) + args + tuple(kws
        .values())
    return signature(return_typ, *unliteral_all(wrxa__imb)).replace(pysig=
        yefzw__ktey)


def generate_axes_typing(mod_name, nrows, ncols):
    dmp__aczg = '{}.subplots(): {} must be a constant integer >= 1'
    if not is_overload_constant_int(nrows):
        raise_bodo_error(dmp__aczg.format(mod_name, 'nrows'))
    if not is_overload_constant_int(ncols):
        raise_bodo_error(dmp__aczg.format(mod_name, 'ncols'))
    mqtcc__olabb = get_overload_const_int(nrows)
    nuvai__coxy = get_overload_const_int(ncols)
    if mqtcc__olabb < 1:
        raise BodoError(dmp__aczg.format(mod_name, 'nrows'))
    if nuvai__coxy < 1:
        raise BodoError(dmp__aczg.format(mod_name, 'ncols'))
    if mqtcc__olabb == 1 and nuvai__coxy == 1:
        osxm__pjdz = types.mpl_axes_type
    else:
        if nuvai__coxy == 1:
            whpi__rplt = types.mpl_axes_type
        else:
            whpi__rplt = types.Tuple([types.mpl_axes_type] * nuvai__coxy)
        osxm__pjdz = types.Tuple([whpi__rplt] * mqtcc__olabb)
    return osxm__pjdz


def generate_pie_return_type(args, kws):
    rcsu__voesy = args[4] if len(args) > 5 else kws.get('autopct', types.none)
    if rcsu__voesy == types.none:
...
def generate_xcorr_return_type(func_mod, args, kws):
    lxz__wes = args[4] if len(args) > 5 else kws.get('usevlines', True)
    if not is_overload_constant_bool(lxz__wes):
        raise_bodo_error(
            f'{func_mod}.xcorr(): usevlines must be a constant boolean')
    if is_overload_true(lxz__wes):
...
class SubplotsTyper(AbstractTemplate):
    ...
    def generic(self, args, kws):
        ...
        rnwg__bneyu = generate_axes_typing('matplotlib.pyplot', nrows, ncols)
        return generate_matplotlib_signature(types.Tuple([types.
            mpl_figure_type, rnwg__bneyu]), args, kws)
...
def overload_subplots(fig, nrows=1, ncols=1, sharex=False, sharey=False,
    ...
    rnwg__bneyu = generate_axes_typing('matplotlib.figure.Figure', nrows, ncols
        )
    zivvl__hhxl = str(rnwg__bneyu)
    if not hasattr(types, zivvl__hhxl):
        zivvl__hhxl = f'objmode_type{ir_utils.next_label()}'
        setattr(types, zivvl__hhxl, rnwg__bneyu)
    pxsl__agqze = f"""def impl(
        fig,
        nrows=1,
        ncols=1,
        sharex=False,
        ...
        with numba.objmode(axes="{zivvl__hhxl}"):
            axes = fig.subplots(
                nrows=nrows,
                ncols=ncols,
                sharex=sharex,
    ...
    """
    lpgn__nfbxy = {}
    exec(pxsl__agqze, {'numba': numba, 'np': np}, lpgn__nfbxy)
    impl = lpgn__nfbxy['impl']
    return impl
