...
def _global_except_hook(exctype, value, traceback):
    ...
    jlqi__flg = MPI.COMM_WORLD
    vxvl__pnf = jlqi__flg.Get_rank()
    wzvgg__yel = 3.0
    bniz__fnzfu = True
    cbbkc__jhwa = jlqi__flg.Ibarrier()
    gajxo__msew = time.time()
    while time.time() - gajxo__msew < wzvgg__yel:
        time.sleep(0.1)
        if cbbkc__jhwa.Test():
            bniz__fnzfu = False
            break
    try:
        ...
        if bniz__fnzfu:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {vxvl__pnf}. \n')
