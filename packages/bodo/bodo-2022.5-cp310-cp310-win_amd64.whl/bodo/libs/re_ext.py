...
def box_regex_flag(typ, val, c):
    zcjhe__cnl = c.box(types.int64, val)
    nqqf__cpho = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(nqqf__cpho, (zcjhe__cnl,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    zcjhe__cnl = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, zcjhe__cnl)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    bdgw__bunyv = context.get_python_api(builder)
    qzdi__cepyt = builder.icmp_signed('==', val, bdgw__bunyv.borrow_none())
    with builder.if_then(qzdi__cepyt):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    rsv__aewpg = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(rsv__aewpg)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        oej__liyhc = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if oej__liyhc > 1:
            typ = types.List(types.Tuple([string_type] * oej__liyhc))
...
def overload_match_group(m, *args):
    ...
    qguya__payp = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            coh__afvr = args[0]
            with numba.objmode(out=qguya__payp):
                out = m.group(coh__afvr)
            return out
        return _match_group_impl_one
    ydhdl__lsif = 'tuple_str_{}'.format(len(args))
    setattr(types, ydhdl__lsif, types.Tuple([qguya__payp] * len(args)))
    mpr__cztxc = ', '.join('group{}'.format(nwg__gfjc + 1) for nwg__gfjc in
        range(len(args)))
    vhkkt__xmlqd = 'def _match_group_impl(m, *args):\n'
    vhkkt__xmlqd += '  ({}) = args\n'.format(mpr__cztxc)
    vhkkt__xmlqd += "  with numba.objmode(out='{}'):\n".format(ydhdl__lsif)
    vhkkt__xmlqd += '    out = m.group({})\n'.format(mpr__cztxc)
    vhkkt__xmlqd += '  return out\n'
    abjxb__jtmfp = {}
    exec(vhkkt__xmlqd, globals(), abjxb__jtmfp)
    aqzl__pkgjr = abjxb__jtmfp['_match_group_impl']
    return aqzl__pkgjr
...
def overload_match_groups(m, default=None):
    dxqlf__ksj = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=dxqlf__ksj):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(atqs__iasq is None for atqs__iasq in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=qguya__payp):
