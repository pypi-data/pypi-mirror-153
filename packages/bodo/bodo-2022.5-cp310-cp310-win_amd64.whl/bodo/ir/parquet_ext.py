...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    rgm__zap = pq_node.out_vars[0].name
    qqb__gjbg = pq_node.out_vars[1].name
    if rgm__zap not in lives and qqb__gjbg not in lives:
        return None
    elif rgm__zap not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif qqb__gjbg not in lives:
