...
def box_regex_flag(typ, val, c):
    ijs__cpo = c.box(types.int64, val)
    jboxx__lxc = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(jboxx__lxc, (ijs__cpo,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    ijs__cpo = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, ijs__cpo)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    gkdh__udr = context.get_python_api(builder)
    fxluv__okb = builder.icmp_signed('==', val, gkdh__udr.borrow_none())
    with builder.if_then(fxluv__okb):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    wfw__dgfjl = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(wfw__dgfjl)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        fcz__crof = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if fcz__crof > 1:
            typ = types.List(types.Tuple([string_type] * fcz__crof))
...
def overload_match_group(m, *args):
    ...
    upsi__hmhf = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            qeb__meclo = args[0]
            with numba.objmode(out=upsi__hmhf):
                out = m.group(qeb__meclo)
            return out
        return _match_group_impl_one
    ltkdy__kggw = 'tuple_str_{}'.format(len(args))
    setattr(types, ltkdy__kggw, types.Tuple([upsi__hmhf] * len(args)))
    jjpe__krfnj = ', '.join('group{}'.format(nggij__ouiok + 1) for
        nggij__ouiok in range(len(args)))
    kwmds__bcph = 'def _match_group_impl(m, *args):\n'
    kwmds__bcph += '  ({}) = args\n'.format(jjpe__krfnj)
    kwmds__bcph += "  with numba.objmode(out='{}'):\n".format(ltkdy__kggw)
    kwmds__bcph += '    out = m.group({})\n'.format(jjpe__krfnj)
    kwmds__bcph += '  return out\n'
    kkwjl__pcdsv = {}
    exec(kwmds__bcph, globals(), kkwjl__pcdsv)
    eop__dtxzm = kkwjl__pcdsv['_match_group_impl']
    return eop__dtxzm
...
def overload_match_groups(m, default=None):
    yddo__iuzi = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=yddo__iuzi):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(gwh__jzt is None for gwh__jzt in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=upsi__hmhf):
