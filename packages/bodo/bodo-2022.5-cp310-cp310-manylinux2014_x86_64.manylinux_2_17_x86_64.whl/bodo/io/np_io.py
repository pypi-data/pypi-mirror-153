...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        xtljb__bfezd = context.get_abi_sizeof(context.get_data_type(dtype.
            dtype))
        return context.get_constant(types.intp, xtljb__bfezd)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            vvytp__kqaeo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(vvytp__kqaeo.dtype)
            file_write(unicode_to_utf8(fname), vvytp__kqaeo.ctypes, 
                dtype_size * vvytp__kqaeo.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            vvytp__kqaeo = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(vvytp__kqaeo.dtype)
            idt__oujq = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                vvytp__kqaeo.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), vvytp__kqaeo.
                ctypes, start, count, idt__oujq)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            qsgp__erb = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                qsgp__erb = min(qsgp__erb, count * dtype_size)
            if qsgp__erb < 0:
                return -1
            return qsgp__erb
        return impl
