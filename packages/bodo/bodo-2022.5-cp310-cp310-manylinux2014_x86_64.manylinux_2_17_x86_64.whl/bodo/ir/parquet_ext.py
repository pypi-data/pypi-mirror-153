...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    yct__rkjew = pq_node.out_vars[0].name
    qga__kuvz = pq_node.out_vars[1].name
    if yct__rkjew not in lives and qga__kuvz not in lives:
        return None
    elif yct__rkjew not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif qga__kuvz not in lives:
