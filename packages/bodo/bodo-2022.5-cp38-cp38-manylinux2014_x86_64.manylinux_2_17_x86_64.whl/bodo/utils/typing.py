...
def decode_if_dict_array_overload(A):
    if isinstance(A, types.BaseTuple):
        lmrp__bfnoc = len(A.types)
        vcmki__xod = 'def f(A):\n'
        hzhu__nzu = ','.join(f'decode_if_dict_array(A[{i}])' for i in range
            (lmrp__bfnoc))
        vcmki__xod += '  return ({}{})\n'.format(hzhu__nzu, ',' if 
            lmrp__bfnoc == 1 else '')
        uoa__nbwva = {}
        exec(vcmki__xod, {'decode_if_dict_array': decode_if_dict_array},
            uoa__nbwva)
        impl = uoa__nbwva['f']
        return impl
    ...
    if isinstance(A, bodo.SeriesType):
        ...
        def impl(A):
            nek__bby = bodo.hiframes.pd_series_ext.get_series_data(A)
            xbz__xpwu = bodo.hiframes.pd_series_ext.get_series_index(A)
            name = bodo.hiframes.pd_series_ext.get_series_name(A)
            wwz__wxgw = decode_if_dict_array(nek__bby)
            return bodo.hiframes.pd_series_ext.init_series(wwz__wxgw,
                xbz__xpwu, name)
        return impl
...
def to_str_arr_if_dict_array(t):
    ...
    if isinstance(t, bodo.TableType):
        xrsag__gkq = tuple(to_str_arr_if_dict_array(t) for t in t.arr_types)
        return bodo.TableType(xrsag__gkq, t.has_runtime_cols)
...
class BodoError(NumbaError):
    ...
    def __init__(self, msg, loc=None, locs_in_msg=None):
        ...
        qux__zjsx = numba.core.errors.termcolor().errmsg
        super(BodoError, self).__init__(qux__zjsx(msg), loc)
...
def raise_bodo_error(msg, loc=None):
    if bodo.transforms.typing_pass.in_partial_typing:
        ...
        khnsn__wjzq = [] if loc is None else [loc]
        raise BodoError(msg, locs_in_msg=khnsn__wjzq)
...
class FileInfo:
    ...
    def get_schema(self, fname):
        usaxn__xscd = self.get_full_filename(fname)
        return self._get_schema(usaxn__xscd)
...
class FilenameModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        rmjam__vpla = dmm.lookup(bodo.typeof(fe_type.fname))
        rvln__xuaru = [(a, lfabe__kdzlh) for a, lfabe__kdzlh in zip(
            rmjam__vpla._fields, rmjam__vpla._members)]
        super().__init__(dmm, fe_type, rvln__xuaru)
...
def is_initial_value_type(t):
    if not isinstance(t, types.InitialValue) or t.initial_value is None:
        return False
    mvmt__jkfb = t.initial_value
    if isinstance(mvmt__jkfb, dict):
        mvmt__jkfb = mvmt__jkfb.values()
    return not any(isinstance(zyqz__pkr, (types.Poison, numba.core.
        interpreter._UNKNOWN_VALUE)) for zyqz__pkr in mvmt__jkfb)
...
def is_overload_constant_dict(val):
    return isinstance(val, types.LiteralStrKeyDict) and all(is_literal_type
        (zyqz__pkr) for zyqz__pkr in val.types) or is_initial_value_dict_type(
...
def is_overload_bool_list(val):
    return is_overload_constant_list(val) and all(is_overload_constant_bool
        (zyqz__pkr) for zyqz__pkr in get_overload_const_list(val))
...
def get_overload_const(val):
    ...
    if isinstance(val, types.LiteralList):
        jxm__fqj = []
        for zyqz__pkr in val.literal_value:
            xlsrq__crxo = get_overload_const(zyqz__pkr)
            if xlsrq__crxo == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                jxm__fqj.append(xlsrq__crxo)
        return jxm__fqj
    ...
    if isinstance(val, types.BaseTuple):
        jxm__fqj = []
        for zyqz__pkr in val.types:
            xlsrq__crxo = get_overload_const(zyqz__pkr)
            if xlsrq__crxo == NOT_CONSTANT:
                return NOT_CONSTANT
            else:
                jxm__fqj.append(xlsrq__crxo)
        return tuple(jxm__fqj)
...
def ensure_constant_arg(fname, arg_name, val, const_type):
    xlsrq__crxo = get_overload_const(val)
    chd__axop = _const_type_repr.get(const_type, str(const_type))
    if not isinstance(xlsrq__crxo, const_type):
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant {chd__axop} not {val}"
            )


def ensure_constant_values(fname, arg_name, val, const_values):
    xlsrq__crxo = get_overload_const(val)
    if xlsrq__crxo not in const_values:
        raise BodoError(
            f"{fname}(): argument '{arg_name}' should be a constant value in {const_values} not '{xlsrq__crxo}'"
            )
...
def check_unsupported_args(fname, args_dict, arg_defaults_dict,
    ...
    zdia__ridys = False
    for a in args_dict:
        xgj__qktu = get_overload_const(args_dict[a])
        wlvsb__phr = arg_defaults_dict[a]
        if (xgj__qktu is NOT_CONSTANT or xgj__qktu is not None and 
            wlvsb__phr is None or xgj__qktu is None and wlvsb__phr is not
            None or xgj__qktu != wlvsb__phr or xgj__qktu is not _no_input and
            wlvsb__phr is _no_input or xgj__qktu is _no_input and 
            wlvsb__phr is not _no_input):
            sehqp__fwzv = (
                f'{fn_str}: {a} parameter only supports default value {wlvsb__phr}'
                )
            zdia__ridys = True
            break
    if zdia__ridys and package_name == 'pandas':
        if module_name == 'IO':
            sehqp__fwzv += """
...
def get_overload_constant_dict(val):
    if isinstance(val, types.LiteralStrKeyDict):
        return {get_literal_value(ksvdj__bwf): get_literal_value(zyqz__pkr) for
            ksvdj__bwf, zyqz__pkr in val.literal_value.items()}
    ...
    if isinstance(val, types.DictType):
        ...
        return val.initial_value
    sakuf__tpt = [get_overload_const(zyqz__pkr) for zyqz__pkr in val.types[1:]]
    return {sakuf__tpt[2 * i]: sakuf__tpt[2 * i + 1] for i in range(len(
        sakuf__tpt) // 2)}
...
def get_overload_const_list(val):
    ...
    if is_initial_value_list_type(val):
        return val.initial_value
    if isinstance(val, types.LiteralList):
        return [get_literal_value(zyqz__pkr) for zyqz__pkr in val.literal_value
            ]
...
def parse_dtype(dtype, func_name=None):
    ...
    if isinstance(dtype, types.Number) or dtype == bodo.string_type:
        return dtype
    try:
        sabqe__aeacn = get_overload_const_str(dtype)
        if sabqe__aeacn.startswith('Int') or sabqe__aeacn.startswith('UInt'):
            return bodo.libs.int_arr_ext.typeof_pd_int_dtype(pd.api.types.
                pandas_dtype(sabqe__aeacn), None)
        if sabqe__aeacn == 'boolean':
            return bodo.libs.bool_arr_ext.boolean_dtype
        if sabqe__aeacn == 'str':
            return bodo.string_type
        return numba.np.numpy_support.from_dtype(np.dtype(sabqe__aeacn))
    except:
        pass
...
def get_index_names(t, func_name, default_name):
    ...
    sllx__xlk = '{}: index name should be a constant string'.format(func_name)
    if isinstance(t, MultiIndexType):
        qnwqx__rxx = []
        for i, uhaxa__zdj in enumerate(t.names_typ):
            if uhaxa__zdj == types.none:
                qnwqx__rxx.append('level_{}'.format(i))
                continue
            if not is_overload_constant_str(uhaxa__zdj):
                raise BodoError(sllx__xlk)
            qnwqx__rxx.append(get_overload_const_str(uhaxa__zdj))
        return tuple(qnwqx__rxx)
...
if types.SliceLiteral in numba.core.pythonapi._boxers.functions:
    ...
    def box_slice_literal(typ, val, c):
        gwfcv__stbq = typ.literal_value
        dopkc__yqr = []
        for rfpi__tywi in ('start', 'stop', 'step'):
            ivt__bnny = getattr(typ.literal_value, rfpi__tywi)
            gdcd__gzy = c.pyapi.make_none(
                ) if ivt__bnny is None else c.pyapi.from_native_value(types
                .literal(ivt__bnny), ivt__bnny, c.env_manager)
            dopkc__yqr.append(gdcd__gzy)
        ysilh__lma = c.pyapi.unserialize(c.pyapi.serialize_object(slice))
        zhti__ckpnb = c.pyapi.call_function_objargs(ysilh__lma, dopkc__yqr)
        for a in dopkc__yqr:
            c.pyapi.decref(a)
        c.pyapi.decref(ysilh__lma)
        return zhti__ckpnb
...
def box_list_literal(typ, val, c):
    sctv__dtgif = typ.literal_value
    vhv__och = [c.pyapi.from_native_value(types.literal(zyqz__pkr),
        zyqz__pkr, c.env_manager) for zyqz__pkr in sctv__dtgif]
    ulh__mpxdr = c.pyapi.list_pack(vhv__och)
    for a in vhv__och:
        c.pyapi.decref(a)
    return ulh__mpxdr
...
def list_literal_to_list(context, builder, fromty, toty, val):
    ...
    sinrp__vtqdb = types.List(toty.dtype)
...
def is_literal_type(t):
    ...
    return isinstance(t, (types.Literal, types.Omitted)) and not isinstance(t,
        ...
        is_literal_type(zyqz__pkr) for zyqz__pkr in t.types
...
def get_literal_value(t):
    ...
    if isinstance(t, types.Literal):
        if isinstance(t, types.LiteralStrKeyDict):
            return {get_literal_value(ksvdj__bwf): get_literal_value(
                zyqz__pkr) for ksvdj__bwf, zyqz__pkr in t.literal_value.items()
                }
        if isinstance(t, types.LiteralList):
            return [get_literal_value(zyqz__pkr) for zyqz__pkr in t.
                literal_value]
        return t.literal_value
    if isinstance(t, types.Omitted):
        return t.value
    if isinstance(t, types.BaseTuple):
        return tuple(get_literal_value(zyqz__pkr) for zyqz__pkr in t.types)
...
def dtype_to_array_type(dtype):
    ...
    if isinstance(dtype, (types.Number, types.Boolean, types.NPDatetime,
        types.NPTimedelta)):
        nek__bby = types.Array(dtype, 1, 'C')
        if ewi__djbgc:
            return to_nullable_type(nek__bby)
        return nek__bby
...
def get_udf_out_arr_type(f_return_type, return_nullable=False):
    ...
    pakg__qenet = dtype_to_array_type(f_return_type)
    pakg__qenet = to_nullable_type(pakg__qenet
        ) if return_nullable else pakg__qenet
    return pakg__qenet
...
def types_equality_exists(t1, t2):
    tfgi__rlb = numba.core.registry.cpu_target.typing_context
    try:
        tfgi__rlb.resolve_function_type(operator.eq, (t1, t2), {})
        return True
    except:
        return False
...
def is_hashable_type(t):
    whitelist_types = (types.UnicodeType, types.StringLiteral, types.
        ...
        PandasTimestampType)
    tozc__mwgx = (types.bool_, bodo.datetime64ns, bodo.timedelta64ns, bodo.
        pd_timedelta_type)
    if isinstance(t, whitelist_types) or t in tozc__mwgx:
        return True
    tfgi__rlb = numba.core.registry.cpu_target.typing_context
    try:
        tfgi__rlb.resolve_function_type(hash, (t,), {})
        return True
    except:
        return False
...
def to_nullable_type(t):
    ...
    if isinstance(t, DataFrameType):
        hfp__qdj = tuple(to_nullable_type(t) for t in t.data)
        return DataFrameType(hfp__qdj, t.index, t.columns, t.dist, t.
            is_table_format)
...
def is_common_scalar_dtype(scalar_types):
    qfqxa__tzcoa, rkc__zbztz = get_common_scalar_dtype(scalar_types)
    return rkc__zbztz
...
def get_common_scalar_dtype(scalar_types):
    ...
    try:
        kyns__gxyw = np.find_common_type([numba.np.numpy_support.as_dtype(t
            ) for t in scalar_types], [])
        if kyns__gxyw != object:
            return numba.np.numpy_support.from_dtype(kyns__gxyw), True
    except numba.core.errors.NumbaNotImplementedError as fqsec__rawya:
        pass
    ...
    zgcda__skz = itertools.groupby(scalar_types)
    if next(zgcda__skz, True) and not next(zgcda__skz, False):
...
def get_nullable_and_non_nullable_types(array_of_types):
    ...
    for typ in array_of_types:
        if typ == bodo.libs.bool_arr_ext.boolean_array:
            rom__fyz.append(types.Array(types.bool_, 1, 'C'))
        elif isinstance(typ, bodo.libs.int_arr_ext.IntegerArrayType):
            rom__fyz.append(types.Array(typ.dtype, 1, 'C'))
        elif isinstance(typ, types.Array):
            if typ.dtype == types.bool_:
                rom__fyz.append(bodo.libs.bool_arr_ext.boolean_array)
            if isinstance(typ.dtype, types.Integer):
                rom__fyz.append(bodo.libs.int_arr_ext.IntegerArrayType(typ.
                    dtype))
        rom__fyz.append(typ)
    return rom__fyz
...
def _gen_objmode_overload(func, output_type, attr_name=None, is_function=
    ...
    if is_function:
        aigh__ltqfg = getfullargspec(func)
        assert aigh__ltqfg.varargs is None, 'varargs not supported'
        assert aigh__ltqfg.varkw is None, 'varkw not supported'
        defaults = [] if aigh__ltqfg.defaults is None else aigh__ltqfg.defaults
        ryb__duy = len(aigh__ltqfg.args) - len(defaults)
        args = aigh__ltqfg.args[1:] if attr_name else aigh__ltqfg.args[:]
        ggte__aptl = []
        for i, swge__qwzns in enumerate(aigh__ltqfg.args):
            if i < ryb__duy:
                ggte__aptl.append(swge__qwzns)
            elif str(defaults[i - ryb__duy]) != '<deprecated parameter>':
                ggte__aptl.append(swge__qwzns + '=' + str(defaults[i -
                    ryb__duy]))
            else:
                args.remove(swge__qwzns)
        if aigh__ltqfg.kwonlyargs is not None:
            for swge__qwzns in aigh__ltqfg.kwonlyargs:
                args.append(f'{swge__qwzns}={swge__qwzns}')
                ggte__aptl.append(
                    f'{swge__qwzns}={str(aigh__ltqfg.kwonlydefaults[swge__qwzns])}'
                    )
        sig = ', '.join(ggte__aptl)
        args = ', '.join(args)
    else:
    ...
    bnqlm__nfant = f'self.{attr_name}' if attr_name else f'{func_name}'
    vcmki__xod = f'def overload_impl({sig}):\n'
    vcmki__xod += f'    def impl({sig}):\n'
    if single_rank:
        vcmki__xod += f'        if bodo.get_rank() == 0:\n'
        edm__vauq = '    '
    else:
        edm__vauq = ''
    vcmki__xod += (
        f"        {edm__vauq}with numba.objmode(res='{type_name}'):\n")
    if is_function:
        vcmki__xod += f'            {edm__vauq}res = {bnqlm__nfant}({args})\n'
    else:
        vcmki__xod += f'            {edm__vauq}res = {bnqlm__nfant}\n'
    vcmki__xod += f'        return res\n'
    vcmki__xod += f'    return impl\n'
    uoa__nbwva = {}
    yudop__fyvax = globals()
    if not attr_name:
        yudop__fyvax[func_name] = func
    exec(vcmki__xod, yudop__fyvax, uoa__nbwva)
    nthnv__scna = uoa__nbwva['overload_impl']
    return nthnv__scna


def gen_objmode_func_overload(func, output_type=None, single_rank=False):
    try:
        nthnv__scna = _gen_objmode_overload(func, output_type, is_function=
            True, single_rank=single_rank)
        overload(func, no_unliteral=True)(nthnv__scna)
    except Exception as fqsec__rawya:
        pass
...
def gen_objmode_method_overload(obj_type, method_name, method, output_type=
    ...
    try:
        nthnv__scna = _gen_objmode_overload(method, output_type,
            method_name, True, single_rank)
        overload_method(obj_type, method_name, no_unliteral=True)(nthnv__scna)
    except Exception as fqsec__rawya:
        pass
...
def gen_objmode_attr_overload(obj_type, attr_name, attr, output_type=None,
    single_rank=False):
    try:
        nthnv__scna = _gen_objmode_overload(attr, output_type, attr_name, 
            False, single_rank)
        overload_attribute(obj_type, attr_name, no_unliteral=True)(nthnv__scna)
    except Exception as fqsec__rawya:
        pass
...
class NumTypeStaticGetItem(AbstractTemplate):
    ...
    def generic(self, args, kws):
        val, dzd__ofb = args
        if isinstance(dzd__ofb, slice) and (isinstance(val, types.
            NumberClass) or isinstance(val, types.TypeRef) and isinstance(
            val.instance_type, (types.NPDatetime, types.NPTimedelta))):
            return signature(types.TypeRef(val.instance_type[dzd__ofb]), *args)
...
def construct_pysig(arg_names, defaults):
    vcmki__xod = f'def stub('
    for swge__qwzns in arg_names:
        vcmki__xod += swge__qwzns
        if swge__qwzns in defaults:
            if isinstance(defaults[swge__qwzns], str):
                vcmki__xod += f"='{defaults[swge__qwzns]}'"
            else:
                vcmki__xod += f'={defaults[swge__qwzns]}'
        vcmki__xod += ', '
    vcmki__xod += '):\n'
    vcmki__xod += '    pass\n'
    uoa__nbwva = {}
    exec(vcmki__xod, {}, uoa__nbwva)
    znvky__ztlhg = uoa__nbwva['stub']
    return numba.core.utils.pysignature(znvky__ztlhg)


def fold_typing_args(func_name, args, kws, arg_names, defaults,
    unsupported_arg_names=()):
    kws = dict(kws)
    hfruo__guq = len(arg_names)
    gfl__fsfu = len(args) + len(kws)
    if gfl__fsfu > hfruo__guq:
        wdxtr__fmxsy = 'argument' if hfruo__guq == 1 else 'arguments'
        kuo__zqdv = 'was' if gfl__fsfu == 1 else 'were'
        raise BodoError(
            f'{func_name}(): Too many arguments specified. Function takes {hfruo__guq} {wdxtr__fmxsy}, but {gfl__fsfu} {kuo__zqdv} provided.'
            )
    assuy__prf = bodo.utils.typing.construct_pysig(arg_names, defaults)
    try:
        ouoz__lpkr = bodo.utils.transform.fold_argument_types(assuy__prf,
            args, kws)
    except Exception as utvs__hdlcq:
        raise_bodo_error(f'{func_name}(): {utvs__hdlcq}')
    if unsupported_arg_names:
        ...
        for i, arg_name in enumerate(arg_names):
            if arg_name in unsupported_arg_names:
                ...
                vslq__qqftz[arg_name] = ouoz__lpkr[i]
                hqkg__ayls[arg_name] = defaults[arg_name]
        check_unsupported_args(func_name, vslq__qqftz, hqkg__ayls)
    return assuy__prf, ouoz__lpkr
...
class BodoArrayIteratorModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        rvln__xuaru = [('index', types.EphemeralPointer(types.uintp)), (
            'array', fe_type.arr_type)]
        super(BodoArrayIteratorModel, self).__init__(dmm, fe_type, rvln__xuaru)
...
def iternext_bodo_array(context, builder, sig, args, result):
    [cim__xfr] = sig.args
    [qfxt__mpfp] = args
    doqs__tnla = context.make_helper(builder, cim__xfr, value=qfxt__mpfp)
    ent__iqyod = signature(types.intp, cim__xfr.arr_type)
    accns__vpa = context.compile_internal(builder, lambda a: len(a),
        ent__iqyod, [doqs__tnla.array])
    xbz__xpwu = builder.load(doqs__tnla.index)
    gaera__diw = builder.icmp_signed('<', xbz__xpwu, accns__vpa)
    result.set_valid(gaera__diw)
    with builder.if_then(gaera__diw):
        glg__nep = signature(cim__xfr.yield_type, cim__xfr.arr_type, types.intp
            )
        value = context.compile_internal(builder, lambda a, i: a[i],
            glg__nep, [doqs__tnla.array, xbz__xpwu])
        result.yield_(value)
        ltrc__ufwq = cgutils.increment_index(builder, xbz__xpwu)
        builder.store(ltrc__ufwq, doqs__tnla.index)


def index_typ_from_dtype_name(elem_dtype, name):
    gosxy__cks = type(get_index_type_from_dtype(elem_dtype))
    if name is None:
        ggci__atoj = None
    else:
        ggci__atoj = types.StringLiteral(name)
    if gosxy__cks == bodo.hiframes.pd_index_ext.NumericIndexType:
        vcjhw__bjdf = gosxy__cks(elem_dtype, ggci__atoj)
    elif gosxy__cks == bodo.hiframes.pd_index_ext.CategoricalIndexType:
        vcjhw__bjdf = gosxy__cks(bodo.CategoricalArrayType(elem_dtype),
            ggci__atoj)
    else:
        vcjhw__bjdf = gosxy__cks(ggci__atoj)
    return vcjhw__bjdf
...
def check_objmode_output_type(ret_tup, ret_type):
    return tuple(_check_objmode_type(zyqz__pkr, t) for zyqz__pkr, t in zip(
...
def _fix_objmode_df_type(val, val_typ, typ):
    ...
    if val_typ != typ:
        ijjm__ala = pd.Index(val_typ.columns)
        mtyll__vqt = pd.Index(typ.columns)
        rwx__bwv = ijjm__ala.argsort()
        dikd__obna = mtyll__vqt.argsort()
        oozx__djvb = val_typ.copy(data=tuple(np.array(val_typ.data)[
            rwx__bwv]), columns=tuple(ijjm__ala[rwx__bwv]))
        amllv__zxkx = typ.copy(data=tuple(np.array(typ.data)[dikd__obna]),
            columns=tuple(mtyll__vqt[dikd__obna]))
        if oozx__djvb == amllv__zxkx:
            val_typ = typ
