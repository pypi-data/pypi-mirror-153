...
class SeriesIatType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        dphy__brl = 'SeriesIatType({})'.format(stype)
        super(SeriesIatType, self).__init__(dphy__brl)
...
class SeriesIatModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        fyki__mova = [('obj', fe_type.stype)]
        super(SeriesIatModel, self).__init__(dmm, fe_type, fyki__mova)
...
def init_series_iat(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        nce__cacr, = args
        kktr__vlq = signature.return_type
        wjtq__epxq = cgutils.create_struct_proxy(kktr__vlq)(context, builder)
        wjtq__epxq.obj = nce__cacr
        context.nrt.incref(builder, signature.args[0], nce__cacr)
        return wjtq__epxq._getvalue()
...
def overload_series_iat_setitem(I, idx, val):
    if isinstance(I, SeriesIatType):
        ...
        if I.stype.dtype == types.NPTimedelta('ns'
            ...
            def impl_dt(I, idx, val):
                mqjct__erhm = (bodo.hiframes.datetime_timedelta_ext.
                    _to_nanoseconds(val))
...
class SeriesIlocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        dphy__brl = 'SeriesIlocType({})'.format(stype)
        super(SeriesIlocType, self).__init__(dphy__brl)
...
class SeriesIlocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        fyki__mova = [('obj', fe_type.stype)]
        super(SeriesIlocModel, self).__init__(dmm, fe_type, fyki__mova)
...
def init_series_iloc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        nce__cacr, = args
        xaumw__cerry = signature.return_type
        ztr__acp = cgutils.create_struct_proxy(xaumw__cerry)(context, builder)
        ztr__acp.obj = nce__cacr
        context.nrt.incref(builder, signature.args[0], nce__cacr)
        return ztr__acp._getvalue()
...
def overload_series_iloc_getitem(I, idx):
    if isinstance(I, SeriesIlocType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl(I, idx):
                S = I._obj
                djzzh__pnphj = bodo.utils.conversion.coerce_to_ndarray(idx)
                fnehk__vpz = bodo.hiframes.pd_series_ext.get_series_data(S)[
                    djzzh__pnphj]
                ornj__ywhwq = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    djzzh__pnphj]
                dphy__brl = bodo.hiframes.pd_series_ext.get_series_name(S)
                return bodo.hiframes.pd_series_ext.init_series(fnehk__vpz,
                    ornj__ywhwq, dphy__brl)
            return impl
        if isinstance(idx, types.SliceType):

            def impl_slice(I, idx):
                S = I._obj
                fnehk__vpz = bodo.hiframes.pd_series_ext.get_series_data(S)[idx
                    ]
                ornj__ywhwq = bodo.hiframes.pd_series_ext.get_series_index(S)[
                    idx]
                dphy__brl = bodo.hiframes.pd_series_ext.get_series_name(S)
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            if is_scalar_type(val):
                if I.stype.dtype == types.NPDatetime('ns'
                    ...
                    def impl_dt(I, idx, val):
                        s = integer_to_dt64(val.value)
                        djzzh__pnphj = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                ...
                if I.stype.dtype == types.NPTimedelta('ns'
                    ...
                    def impl_dt(I, idx, val):
                        mqjct__erhm = (bodo.hiframes.datetime_timedelta_ext
                            ._to_nanoseconds(val))
                        s = (bodo.hiframes.pd_timestamp_ext.
                            integer_to_timedelta64(mqjct__erhm))
                        djzzh__pnphj = bodo.utils.conversion.coerce_to_ndarray(
                            idx)
                        bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                            djzzh__pnphj] = s
            ...
            def impl_arr(I, idx, val):
                djzzh__pnphj = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(I._obj)[
                    djzzh__pnphj] = bodo.utils.conversion.coerce_to_array(val,
                    False)
            return impl_arr
...
class SeriesLocType(types.Type):
    ...
    def __init__(self, stype):
        self.stype = stype
        dphy__brl = 'SeriesLocType({})'.format(stype)
        super(SeriesLocType, self).__init__(dphy__brl)
...
class SeriesLocModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        fyki__mova = [('obj', fe_type.stype)]
        super(SeriesLocModel, self).__init__(dmm, fe_type, fyki__mova)
...
def init_series_loc(typingctx, obj=None):
    ...
    def codegen(context, builder, signature, args):
        nce__cacr, = args
        pfruk__vkf = signature.return_type
        kxjiu__uuuf = cgutils.create_struct_proxy(pfruk__vkf)(context, builder)
        kxjiu__uuuf.obj = nce__cacr
        context.nrt.incref(builder, signature.args[0], nce__cacr)
        return kxjiu__uuuf._getvalue()
...
def overload_series_loc_getitem(I, idx):
    ...
    if isinstance(idx, types.Integer) and isinstance(I.stype.index,
        ...
        def impl_range_index_int(I, idx):
            ...
            fnehk__vpz = bodo.hiframes.pd_series_ext.get_series_data(S)
            ornj__ywhwq = bodo.hiframes.pd_series_ext.get_series_index(S)
            bgt__fmht = (idx - ornj__ywhwq._start) // ornj__ywhwq._step
            return fnehk__vpz[bgt__fmht]
        ...
        if isinstance(idx, types.Integer):
            ...
            if isinstance(S.index, RangeIndexType):
                ...
                def impl(S, idx):
                    ...
                    djzzh__pnphj = idx * I._step + I._start
                    return bodo.utils.conversion.box_if_dt64(fnehk__vpz[
                        djzzh__pnphj])
                return impl
        ...
        if idx == bodo.string_type or is_overload_constant_str(idx):
            if isinstance(S.index, bodo.hiframes.pd_index_ext.StringIndexType):
                ...
                def impl_str_getitem(S, idx):
                    rzkbn__gvdc = bodo.hiframes.pd_series_ext.get_series_index(
                        S)
                    omm__enb = rzkbn__gvdc.get_loc(idx)
                    if omm__enb is None:
                        raise IndexError
                    else:
                        val = bodo.hiframes.pd_series_ext.get_series_data(S)[
                            bodo.utils.indexing.unoptional(omm__enb)]
                    return val
    ...
    if isinstance(S, SeriesType):
        ...
        if is_list_like_index_type(idx) and isinstance(idx.dtype, (types.
            ...
            def impl_arr(S, idx, val):
                djzzh__pnphj = bodo.utils.conversion.coerce_to_ndarray(idx)
                bodo.hiframes.pd_series_ext.get_series_data(S)[djzzh__pnphj
                    ] = bodo.utils.conversion.coerce_to_array(val, False)
            return impl_arr
...
def overload_const_index_series_getitem(S, idx):
    if isinstance(S, (SeriesType, HeterogeneousSeriesType)) and isinstance(S
        ...
        index.data):
        hcl__vyiay = get_overload_const_tuple(S.index.data)
        if isinstance(idx, types.Integer) and not any(isinstance(lccaq__iof,
            int) for lccaq__iof in hcl__vyiay):
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[idx]
        if is_literal_type(idx):
            yrhd__cnmqb = get_literal_value(idx)
            if yrhd__cnmqb not in hcl__vyiay:
                raise_bodo_error(
                    f"Series label-based getitem: '{yrhd__cnmqb}' not in {hcl__vyiay}"
                    )
            arr_ind = hcl__vyiay.index(yrhd__cnmqb)
            return lambda S, idx: bodo.hiframes.pd_series_ext.get_series_data(S
                )[arr_ind]
...
def cast_series_iat(context, builder, fromty, toty, val):
    wjtq__epxq = cgutils.create_struct_proxy(fromty)(context, builder, val)
    rir__gdo = context.cast(builder, wjtq__epxq.obj, fromty.stype, toty.stype)
    qkvjp__oezaw = cgutils.create_struct_proxy(toty)(context, builder)
    qkvjp__oezaw.obj = rir__gdo
    return qkvjp__oezaw._getvalue()
