...
def _global_except_hook(exctype, value, traceback):
    ...
    nad__aea = MPI.COMM_WORLD
    kidr__rtmq = nad__aea.Get_rank()
    xbgk__iuf = 3.0
    nksyg__faok = True
    qzmii__yljly = nad__aea.Ibarrier()
    sbbgc__qpaqo = time.time()
    while time.time() - sbbgc__qpaqo < xbgk__iuf:
        time.sleep(0.1)
        if qzmii__yljly.Test():
            nksyg__faok = False
            break
    try:
        ...
        if nksyg__faok:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {kidr__rtmq}. \n')
