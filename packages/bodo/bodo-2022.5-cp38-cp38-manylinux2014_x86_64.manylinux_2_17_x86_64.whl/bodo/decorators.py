...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        anjv__ncqv = self.overloads[sig]
        rzfy__egbf = anjv__ncqv.metadata.get('distributed_diagnostics', None)
        if rzfy__egbf is None:
            kln__dqcdm = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(kln__dqcdm)
        rzfy__egbf.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        ncwsx__apg = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(ncwsx__apg)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        zlq__ragv = options.pop('distributed')
        pipeline_class = (pipeline_class if zlq__ragv else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        nxs__mtx = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if nxs__mtx else
            pipeline_class)
    hdgis__zycw = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(hdgis__zycw, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(hdgis__zycw)
        else:
            return master_mode_wrapper(hdgis__zycw)
    else:
        return hdgis__zycw
