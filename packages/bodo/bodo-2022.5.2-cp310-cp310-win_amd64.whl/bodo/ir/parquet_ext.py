...
def remove_dead_pq(pq_node, lives_no_aliases, lives, arg_aliases, alias_map,
    func_ir, typemap):
    mqa__cril = pq_node.out_vars[0].name
    yfslo__inza = pq_node.out_vars[1].name
    if mqa__cril not in lives and yfslo__inza not in lives:
        return None
    elif mqa__cril not in lives:
        pq_node.col_indices = []
        pq_node.df_colnames = []
        pq_node.type_usecol_offset = []
    elif yfslo__inza not in lives:
