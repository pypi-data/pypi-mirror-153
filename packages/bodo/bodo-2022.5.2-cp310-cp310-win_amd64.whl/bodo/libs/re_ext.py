...
def box_regex_flag(typ, val, c):
    phxwn__cauae = c.box(types.int64, val)
    epck__psamu = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(epck__psamu, (phxwn__cauae,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    phxwn__cauae = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, phxwn__cauae)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    gmt__ezd = context.get_python_api(builder)
    yqsb__uva = builder.icmp_signed('==', val, gmt__ezd.borrow_none())
    with builder.if_then(yqsb__uva):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    gpxfb__gwcw = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(gpxfb__gwcw)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        bqg__sic = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if bqg__sic > 1:
            typ = types.List(types.Tuple([string_type] * bqg__sic))
...
def overload_match_group(m, *args):
    ...
    hvghq__blab = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            xrtrb__hgtku = args[0]
            with numba.objmode(out=hvghq__blab):
                out = m.group(xrtrb__hgtku)
            return out
        return _match_group_impl_one
    fkk__jwow = 'tuple_str_{}'.format(len(args))
    setattr(types, fkk__jwow, types.Tuple([hvghq__blab] * len(args)))
    fczg__est = ', '.join('group{}'.format(ppts__mhep + 1) for ppts__mhep in
        range(len(args)))
    iuvmn__odqh = 'def _match_group_impl(m, *args):\n'
    iuvmn__odqh += '  ({}) = args\n'.format(fczg__est)
    iuvmn__odqh += "  with numba.objmode(out='{}'):\n".format(fkk__jwow)
    iuvmn__odqh += '    out = m.group({})\n'.format(fczg__est)
    iuvmn__odqh += '  return out\n'
    jkvdo__vei = {}
    exec(iuvmn__odqh, globals(), jkvdo__vei)
    zli__ahba = jkvdo__vei['_match_group_impl']
    return zli__ahba
...
def overload_match_groups(m, default=None):
    ujygh__tlrr = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=ujygh__tlrr):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(lfie__gjp is None for lfie__gjp in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=hvghq__blab):
