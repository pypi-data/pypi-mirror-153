...
@LastEditTime: 2022-06-06 17:33:52
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id=""):
        ...
        try:
            ...
            if not user_info:
                ...
                if result > 0:
                    user_info_model.set_dependency_cache(dependency_key=self.get_user_info_dependency_key(act_id, user_info.id_md5),cache_key=user_info_model.get_cache_key(prefix_key="dict",field="*",where="id_md5=%s",group_by="",order_by="",limit="1",params=[user_info.id_md5]),value=SevenHelper.json_dumps(user_info),cache_expire=60)
                    user_info_model.set_dependency_cache(dependency_key=self.get_user_info_dependency_key(act_id,"", user_info.open_id),cache_key=user_info_model.get_cache_key(prefix_key="entity",field="*",where="act_id=%s and open_id=%s",group_by="",order_by="",limit="1",params=[act_id, open_id]),value=SevenHelper.json_dumps(user_info),cache_expire=60)
