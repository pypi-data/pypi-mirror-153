...
class TaSignals:
    ...
    def on_balance_volume(self, df, n, key, window, front_run):
    ...
    def bollinger(self, df, key, window, front_run):

        tp = (df[key] + df['low'] + df['high'])/3
        df['std'] = tp.rolling(20).std(ddof=0)
        df['matp'] = tp.rolling(20).mean()
        df['bollinger_high'] = df['matp'] + 2*df['std']
        df['bollinger_low'] = df['matp'] - 2*df['std']
        data = {'bollinger_low': df['bollinger_low'].iloc[-1], 'bollinger_high': df['bollinger_high'].iloc[-1]}
        data = [
            {'key': 'bollinger_above_matp', 'value': 'Above bollinger matp' if (df[key].iloc[-1] > df['matp'].iloc[-1] + 25) else False},
            {'key': 'bollinger_below_matp', 'value': 'Below bollinger matp' if (df[key].iloc[-1] < df['matp'].iloc[-1] - 25) else False},
            {'key': 'bollinger_at_matp', 'value': 'At bollinger matp' if math.isclose(df[key].iloc[-1], df['matp'].iloc[-1], abs_tol=25) else False},
    ...
    def ma(self, df, key, window, front_run):
    ...
    def ema(self, df, key, window, front_run):
    ...
    def rsi(self, df, key, window, front_run):
        df = self.get_rsi(df, key)
        df['rsi_slope'] = df['rsi'].rolling(window=window).apply(self.get_slope, raw=True)
    ...
    def macd_slope(self, df, key, window, front_run):
        ...
        df['macd_slope'] = df['MACD_12_26_9'].rolling(window=window).apply(self.get_slope, raw=True)
        df['macd_sig_slope'] = df['MACDs_12_26_9'].rolling(window=window).apply(self.get_slope, raw=True)
        df['macd_hist_slope'] = df['MACDh_12_26_9'].rolling(window=window).apply(self.get_slope, raw=True)
    ...
    def divergence(self, df, key, window, front_run):
        close_low_slope, close_high_slope, rsi_low_slope, rsi_high_slope, df = self.rsi_close_div_peak_slopes(df, key, window)
    ...
    def peak_slope(self, df, key, window=4):
        lows, highs = self.range_slopes(df, key, 21, 21)
        lows[f'{key}_lows_slope'] = lows[key].rolling(window=window).apply(self.get_slope, raw=True)
        highs[f'{key}_highs_slope'] = highs[key].rolling(window=window).apply(self.get_slope, raw=True)
        return lows[f'{key}_lows_slope'].iloc[-1], highs[f'{key}_highs_slope'].iloc[-1], df

    def rsi_close_div_peak_slopes(self, df, key, window=4):
        close_low_slope, close_high_slope, df = self.peak_slope(df, key, window)
        rsi_low_slope, rsi_high_slope, df = self.peak_slope(df, 'rsi', window)
        df.merge(df)
