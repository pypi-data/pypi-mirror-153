    ...
    def _convert_tensor(self, tensor: tf.Tensor) -> tf.Tensor:
        ...
        if isinstance(converted_layer, TFOpLambda):
            if isinstance(node_input_tensors, list):
                ...
                if transform_signal and isinstance(converted_layer, SlicingOpLambda):
                    slice_spec = current_layer.inbound_nodes[0].call_kwargs.get('slice_spec')
                    if slice_spec is None:
                        slice_spec = current_layer.inbound_nodes[0].call_args[1:][0]
                    slice_spec_first_to_last = [slice_spec[0]] + slice_spec[2:] + [slice_spec[1]]
                    converted_tensor = converted_layer(node_input_tensors,
                                                       slice_spec=slice_spec_first_to_last)
                else:
                    converted_tensor = converted_layer(node_input_tensors, *current_layer.inbound_nodes[0].call_args[1:],
