...
def RequireProgramsInPath(programs: list[str]):
    """Checks if programs specified are in path"""
    missingPrograms = ""
    for program in programs:
        if(shutil.which(program) == None):
            missingPrograms += f"{program}, "
    if(missingPrograms):
        print("Missing following programs on path: " + missingPrograms)
        return False
    return True
