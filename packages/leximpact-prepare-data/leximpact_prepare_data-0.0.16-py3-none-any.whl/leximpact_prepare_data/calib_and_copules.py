...
# Cell
def get_fake_upper_bound(val: Union[float, int], min_len: int = 0):
    """
    ...
    if val == 1e15:
        ...
        try:
            ...
            if len(str(abs(int(val)))) <= min_len:
                return val
            if val < 0:
                return -(10 ** (len(str(abs(int(val))))))
            elif val == 0.0:
                return 0
            else:
...
# Cell
def keep_upper_bound_secret(json_content, min_len: int = 4):
    """
    ...
    # Change Copulas Lower Bound
    if json_content[0].get("buckets"):
        # In copule
        val = json_content[0].get("lower_bound")
        if val:
            new_val = get_fake_upper_bound(val, min_len)
            json_content[0]["lower_bound"] = (
                new_val if new_val < json_content[0]["upper_bound"] else val
            )
        for buck in json_content:
            if buck["buckets"] == [] or type(buck["buckets"]) is str:
                continue
            val = buck["buckets"][0].get("lower_bound")
            if val:
                new_val = get_fake_upper_bound(val, min_len)
                buck["buckets"][0]["lower_bound"] = (
                    new_val if new_val < json_content[0]["upper_bound"] else val
                )
    else:
        # In Calib
