...
class NURBSClass:
    ...
    def __str__(self):
        ...
        for j in range(int((self.degree+1)//2)):
            str=str+'\n'
    ...
    def check_NURBSParameters(self,tol):
        ...
        while ctlpt_nr < len(self.CPoints)-1:
            ctlpt_nr+=1
            #(abs(self.x-other.x)<=tol) & (abs(self.y-other.y)<tol)
            #???????????????????????????????
...
class BiarcFittingClass:
    def __init__(self,degree, CPoints, Weights, Knots, epsilon=0.01):
...
class ArcGeo:
    ...
    def to_ret_str(self):
        return "arc %6.4f %6.4f %0.3f %0.5f %0.5f" %(self.O.x, self.O.y, self.r, self.s_ang, self.ext)
...
class LineGeo:
    ...
    def to_ret_str(self):
        return "line %6.4f %6.4f %6.4f %6.4f" %(self.Pa.x, self.Pa.y, self.Pe.x, self.Pe.y)
...
class ExamplesClass:
    ...
    def calc_nurbs_1(self):
        ...
        degree, CPoints, Weights, Knots=self.get_nurbs_6()
    ...
    def calc_bspline_1(self):
        ...
        self.CPoints=[]
        for CPt in CPts:
            self.CPoints.append(PointClass(x=CPt[0],y=CPt[1]))

        return self.CPoints, Points, Tang
    ...
    def get_spline1(self):
        degree=3
        Knots = [0.0, 0.0, 0.0, 0.0, \
                 825.4, 1718.5, 1718.5, 1718.5, 1718.5]
        Weights = [1, 1, 1, 1, 1]
        CPoints=[]
        CPoints.append(PointClass(x=1209.5, y=1536.8))
        CPoints.append(PointClass(x=1473.1, y=1776.0))
        CPoints.append(PointClass(x=2021.8, y=2273.7))
        CPoints.append(PointClass(x=2190.9, y=1537.5))
        CPoints.append(PointClass(x=2278.8, y=1154.9))
        return degree, CPoints, Weights, Knots

    def get_spline12(self):
        degree=3
        Knots = [0.0, 0.0, 0.0, 0.0, \
                 825.4508, 1718.4858, 1718.4858, 1718.4858, 1718.4858]
        Weights = [1, 1, 1, 1, 1]
        CPoints=[]
        CPoints.append(PointClass(x=1209.535377237351, y=1536.985613961551))
        CPoints.append(PointClass(x=1473.093231476604, y=1776.03641291328))
        CPoints.append(PointClass(x=2021.787893310498, y=2273.710434697016))
        CPoints.append(PointClass(x=2190.890844687986, y=1537.506222039088))
        CPoints.append(PointClass(x=2278.76754381506, y=1154.927470072742))
        return degree, CPoints, Weights, Knots

    def get_spline2(self):
        degree=3
        Knots = [0.0, 0.0, 0.0, 0.0, \
                 873.1, 1750.7, \
                 2237.9, 2237.9, 2237.9, 2237.9]
        Weights = [1, 1, 1, 1, 1, 1]
        CPoints=[]
        CPoints.append(PointClass(x=1076.8, y=1383.1))
        CPoints.append(PointClass(x=1272.8, y=1747.7))
        CPoints.append(PointClass(x=1665.7, y=2478.8))
        CPoints.append(PointClass(x=2064.8, y=1200.5))
        CPoints.append(PointClass(x=2535.3, y=1421.1))
        CPoints.append(PointClass(x=2703.3, y=1499.8))
        return degree, CPoints, Weights, Knots

    def get_spline22(self):
        degree=3
        Knots = [0.0, 0.0, 0.0, 0.0, \
                 873.11847, 1750.7122, \
                 2237.9304, 2237.9304, 2237.9304, 2237.9304]
        Weights = [1, 1, 1, 1, 1, 1]
        CPoints=[]
        CPoints.append(PointClass(x=1076.876299498182, y=1383.101083784115))
        CPoints.append(PointClass(x=1272.824637898317, y=1747.725747644268))
        CPoints.append(PointClass(x=1665.725659326161, y=2478.843980328091))
        CPoints.append(PointClass(x=2064.851578621809, y=1200.501610514842))
        CPoints.append(PointClass(x=2535.324499997904, y=1421.093078487539))
        CPoints.append(PointClass(x=2703.276592580404, y=1499.841072194584))
        return degree, CPoints, Weights, Knots

    def get_spline3(self):
        degree=3
        Knots = [0.0, 0.0, 0.0, 0.0, \
                 665.4, 1699.7, 2886.1, \
                 3276.1, 3276.1, 3276.1, 3276.1]
        Weights = [1, 1, 1, 1, 1, 1, 1]
        CPoints=[]
        CPoints.append(PointClass(x=1522.6, y=1343.3))
        CPoints.append(PointClass(x=1592.7, y=1604.1))
        CPoints.append(PointClass(x=1771.7, y=2270.3))
        CPoints.append(PointClass(x=3391.5, y=1140.9))
        CPoints.append(PointClass(x=1402.5, y=591.8))
        CPoints.append(PointClass(x=1498.8, y=1194.2))
        CPoints.append(PointClass(x=1522.6, y=1343.3))
        return degree, CPoints, Weights, Knots

    def get_spline32(self):
        degree=3
        Knots = [0.0, 0.0, 0.0, 0.0, \
                 665.41455, 1699.7642, 2886.0566, \
                 3276.1555, 3276.1555, 3276.1555, 3276.1555]
        Weights = [1, 1, 1, 1, 1, 1, 1]
        CPoints=[]
        CPoints.append(PointClass(x=1522.610800701792, y=1343.303360462363))
        CPoints.append(PointClass(x=1592.683938812242, y=1604.11592184824))
        CPoints.append(PointClass(x=1771.681836064721, y=2270.346967352717))
        CPoints.append(PointClass(x=3391.539920642247, y=1140.903122306379))
        CPoints.append(PointClass(x=1402.493236034435, y=591.8422787139895))
        CPoints.append(PointClass(x=1498.782701097539, y=1194.233659006067))
        CPoints.append(PointClass(x=1522.610800701792, y=1343.303360462363))
        return degree, CPoints, Weights, Knots

    def get_spline4(self):
        degree=3
        Knots = [0.0, 0.0, 0.0, 0.0, \
                 131.7, 227.1, 227.1, 227.1, 227.1]
        Weights = [1, 1, 1, 1, 1]
        CPoints=[]
        CPoints.append(PointClass(x=121.5, y=102.5))
        CPoints.append(PointClass(x=160.5, y=149.0))
        CPoints.append(PointClass(x=227.7, y=229.3))
        CPoints.append(PointClass(x=254.8, y=136.8))
        CPoints.append(PointClass(x=266.2, y=97.9))
        return degree, CPoints, Weights, Knots

# knots, controlPoints
def fitting(knots,cpts,epsilon=0.01):
    # knots = [0.0, 0.0, 0.0, 0.0, \
    #         131.7, 227.1, 227.1, 227.1, 227.1]
    controlPoints=[]
    Weights = []
    for cp in cpts:
        controlPoints.append(PointClass(x=cp[0],y=cp[1]))
        Weights.append(1)
    # controlPoints.append(PointClass(x=121.5, y=102.5))
    # controlPoints.append(PointClass(x=160.5, y=149.0))
    # controlPoints.append(PointClass(x=227.7, y=229.3))
    # controlPoints.append(PointClass(x=254.8, y=136.8))
    # controlPoints.append(PointClass(x=266.2, y=97.9))
    # Weights = [1, 1, 1, 1, 1]
    biarcfitting=BiarcFittingClass(3, controlPoints, Weights, knots, epsilon)
    retstr = []
    for curve in biarcfitting.Curve:
        retstr.append(curve.to_ret_str())
    return retstr
# knots = [0.0, 0.0, 0.0, 0.0, \
#          131.7, 227.1, 227.1, 227.1, 227.1]
# controlPoints=[
...
#     [266.2,97.9]
# ]
# fitting(knots, controlPoints, 0.1)
