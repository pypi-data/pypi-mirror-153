...
def has_rdm_2e_dnup(trexio_file) -> bool:
    """Check that rdm_2e_dnup variable exists in the TREXIO file.
    ...
    rc = pytr.trexio_has_rdm_2e_dnup(trexio_file.pytrexio_s)
    if rc == TREXIO_FAILURE:
        raise Error(rc)
...
def read_rdm_2e_dnup(trexio_file: File, offset_file: int, buffer_size: int) -> tuple:
    """Read the rdm_2e_dnup indices and values from the TREXIO file.
    ...
    # read the number of integrals already in the file
    integral_num = read_rdm_2e_dnup_size(trexio_file)
    ...
    # as the last 2 arguments (for numpy arrays of indices and values, respectively)
    # read_buf_size contains the number of elements being read from the file, useful when EOF has been reached
    rc, n_int_read, indices, values = pytr.trexio_read_safe_rdm_2e_dnup(trexio_file.pytrexio_s,
                                                                         offset_file,
                                                                         verified_size,
                                                                         verified_size * 4,
                                                                         verified_size)
...
def read_rdm_2e_dnup_size(trexio_file) -> int:
    """Read the number of rdm_2e_dnup integrals stored in the TREXIO file.

    Parameter is a ~TREXIO File~ object that has been created by a call to ~open~ function.

    Returns:
        ~num_integral~: int
        Integer value of corresponding to the size of the rdm_2e_dnup sparse array from ~trexio_file~.
    ...
    rc, num_integral = pytr.trexio_read_rdm_2e_dnup_size(trexio_file.pytrexio_s)
    if rc != TREXIO_SUCCESS:
        raise Error(rc)
...
def write_rdm_2e_dnup(trexio_file: File, offset_file: int, buffer_size: int, indices: list, values: list) -> None:
    """Write the rdm_2e_dnup indices and values in the TREXIO file.
    ...
    indices: list OR numpy.ndarray
        Array of rdm_2e_dnup indices to be written. If array data type does not correspond to int32, the conversion is performed.

    values: list OR numpy.ndarray
        Array of rdm_2e_dnup values to be written. If array data type does not correspond to float64, the conversion is performed.
    ...
    if (convertIndices or flattenIndices) and convertValues:
        rc = pytr.trexio_write_safe_rdm_2e_dnup(trexio_file.pytrexio_s, offset_file, buffer_size, indices_32, values_64)
    elif (convertIndices or flattenIndices) and not convertValues:
        rc = pytr.trexio_write_safe_rdm_2e_dnup(trexio_file.pytrexio_s, offset_file, buffer_size, indices_32, values)
    elif not (convertIndices or flattenIndices) and convertValues:
        rc = pytr.trexio_write_safe_rdm_2e_dnup(trexio_file.pytrexio_s, offset_file, buffer_size, indices, values_64)
    else:
        rc = pytr.trexio_write_safe_rdm_2e_dnup(trexio_file.pytrexio_s, offset_file, buffer_size, indices, values)
