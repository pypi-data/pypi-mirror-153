...
class LarryConnarModel(RegressorMixin):
    ...
    def fit(self, X=None, y=None):
        ...
        model.add(Dense(10 * feature_n, input_shape=(feature_n,), activation='relu', name="first_d"))  # Hidden Layer, softplus(x) = log(exp(x) + 1)
        # model.add(Dense(5 * feature_n, activation='relu'))  # Hidden Layer, softplus(x) = log(exp(x) + 1)
        model.add(Dense(feature_n, activation='relu', name="second_d"))  # Hidden Layer, softplus(x) = log(exp(x) + 1)
        model.add(Dense(1, activation='linear', name="out_dens"))  # Output Layer, sigmoid(x) = 1 / (1 + exp(-x))
