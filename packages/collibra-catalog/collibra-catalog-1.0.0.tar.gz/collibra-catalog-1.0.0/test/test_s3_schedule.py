# coding: utf-8

"""
    Collibra Catalog API

    The Catalog API offers functionality related to the catalog product.<br/>It is mainly focused on facilitating the ingestion of information into Catalog. The API enables users to more easily connect Catalog to sources that are not necessarily natively supported in the product.  # noqa: E501

    The version of the OpenAPI document: 1.0
    Generated by: https://openapi-generator.tech
"""


from __future__ import absolute_import

import unittest
import datetime

import collibra_catalog
from collibra_catalog.models.s3_schedule import S3Schedule  # noqa: E501
from collibra_catalog.rest import ApiException

class TestS3Schedule(unittest.TestCase):
    """S3Schedule unit test stubs"""

    def setUp(self):
        pass

    def tearDown(self):
        pass

    def make_instance(self, include_optional):
        """Test S3Schedule
            include_option is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # model = collibra_catalog.models.s3_schedule.S3Schedule()  # noqa: E501
        if include_optional :
            return S3Schedule(
                id = 56, 
                cron_expression = '0', 
                cron_time_zone = '0', 
                last_run_time_stamp = 56, 
                next_run_date_long_value = 56, 
                cron_json = '0'
            )
        else :
            return S3Schedule(
        )

    def testS3Schedule(self):
        """Test S3Schedule"""
        inst_req_only = self.make_instance(include_optional=False)
        inst_req_and_optional = self.make_instance(include_optional=True)


if __name__ == '__main__':
    unittest.main()
