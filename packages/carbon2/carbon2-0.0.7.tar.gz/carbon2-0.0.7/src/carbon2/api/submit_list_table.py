...
def upload_to_server_with_table(server_url,table, user_id,target_url,driver_path="",save_folder="html_data",use_md5url_as_id=False,tag="",language="",publishtime="",
                     ...
                     no_submit=False,no_submit_file=False,use_trafilatura=False):
    print("==========Begin Upload===============")
...
def submit_page_list_with_table(server_url, table, user_id,csv_file,save_html_folder, use_md5url_as_id=False, driver_path="browsers/chromedriver.exe",tag="",language="",
    ...
    for idx,model in enumerate(list_model):
        ...
        if try_raise_error:
            r=upload_to_server_with_table(server_url,table, user_id, url, use_md5url_as_id=use_md5url_as_id, driver_path=driver_path,use_trafilatura=True,
            ...
            try:
                r=upload_to_server_with_table(server_url,table,user_id, url,use_md5url_as_id=use_md5url_as_id, driver_path=driver_path,tag=tag,use_trafilatura=True, language=language,save_folder=save_html_folder,publishtime=publishtime,not_check_exists=not_check_exists,no_submit=no_submit,no_submit_file=no_submit_file)
                if no_submit:
                    if r!=None:
                        list_result.append(r)
                print()
