...
async def package_message(bot: BotHandlerFactory,
                          event: WsContext,
    ...
    if event.event_type in ['AT_MESSAGE_CREATE', 'MESSAGE_CREATE']:
        ...
        if hasattr(message, 'message_reference'):
            reference = await bot.instance.message_api.get_message(message.channel_id,
                                                                   message.message_reference.message_id)
            reference_data = await package_message(bot, event, reference.message, True)
