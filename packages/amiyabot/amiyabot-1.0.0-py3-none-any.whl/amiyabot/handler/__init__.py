...
from typing import Any, List, Tuple, Union, Optional, Callable, Coroutine
...
class BotHandlerFactory:
    def __init__(self, appid: str, token: str):
        ...
        self.message_handler_middleware: Optional[MIDDLE_WARE] = None
    ...
    def handler_middleware(self, handler: MIDDLE_WARE):
        ...
        :return:
        """
        if not self.message_handler_middleware:
            self.message_handler_middleware = handler
        else:
            raise Exception('Only one middleware is allowed for a robot instance.')
