...
class SegmentationMaskMetric(Metric):
    def __init__(
        ...
        # NOTE: Allow caching original confusion matrix for multiple calculations that rely on the same data
        self.unfiltered_confusion: Optional[np.ndarray] = None
    ...
    def _calculate_confusion_matrix(
        ...
        confusion = (
            ...
            if self.unfiltered_confusion is None
            else np.copy(self.unfiltered_confusion)
        )
        if self.unfiltered_confusion is None:
            self.unfiltered_confusion = np.copy(confusion)
