...
def to_pascal_case(subject,firstCharLower=False,firstCharAlpha=True):
...
def to_camel_case(subject,firstCharLower=True,firstCharAlpha=True):
    subject = re.sub(r'[^a-zA-Z0-9_\s]', ' ', subject)
...
def to_snake_case(subject,firstCharAlpha=False):
    '''
...
def to_kebab_case(subject):
    subject = to_snake_case(subject)
    return subject.replace("_","-")

def to_dot_notation(subject):
    subject = to_snake_case(subject)
    return subject.replace("_",".")
