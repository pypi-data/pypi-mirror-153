...
class TestGetData:
    ...
    @pytest.mark.source
    @pytest.mark.get_data
    def test_filters_by_custom_date_returns(self, good_object_id, get_three_good_field_ids, connector, filtered_column_nm):
        connector.authenticate()
    ...
    def test_filters_by_custom_date_works(self,
        ...
        if filtered_column_nm not in get_three_good_field_ids:
            fields = get_three_good_field_ids.append(filtered_column_nm)
        else:
            fields = get_three_good_field_ids
        connector.authenticate()
        data = connector.get_data(good_object_id,
                                  fields,
                                  filters={
        ...
        for row in data["data"]:
            assert start < datetime.strptime(data[row][filtered_column_nm], filtered_column_date_format)
            assert datetime.strptime(data[row][filtered_column_nm], filtered_column_date_format) < end
