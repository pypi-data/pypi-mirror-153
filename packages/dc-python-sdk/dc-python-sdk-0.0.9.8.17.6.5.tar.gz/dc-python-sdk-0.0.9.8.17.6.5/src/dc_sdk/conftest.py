...
def generate_batch(batch_rows):
    ...
    return [to_dc_format(df), end - start]


@pytest.fixture(scope="session", params=[0.25, 0.5, 1.0, 10.0, 100.0, 1000.0])
def num_gigs(request):
    return request.param


@pytest.fixture(scope="session")
def load_data(connector, generate_batch, good_object_id, mapping_object, _replace, num_gigs, batch_size, batch_rows):
    start = time.time()
    size = num_gigs
    batch_number = 1
    rows_loaded = 0
    ...
    total_batches = math.ceil((size * GB) / float(sys.getsizeof(batch[0])))
    generate_data_time += batch[1]
    connector.load_data(batch[0], good_object_id, mapping_object, _replace, batch_number, total_batches)

    while batch_number <= total_batches:
        rows_loaded += len(batch[0])
    ...
    with open("load_test_results.txt", "a") as f:
        ...
        elapsed_time = end - start - generate_data_time
        hours = elapsed_time / (60. * 60.)
        minutes = (elapsed_time % (60. * 60.)) / 60.
        ...
        print(f"\t({end - start} total seconds - {generate_data_time} seconds to generate data)")
        f.write(f"Time elapsed to load {num_gigs} GB: {formatted_time}.")
        f.write(f"\t({end - start} total seconds - {generate_data_time} seconds to generate data)\n\n")
