...
class Casst(object):
    ...
    def __doc__(self, *args, **kwargs):
        return '''
        CASST-- SoundSTatistic

        Currently only works for BND 1, BND 3 and BND 0 **when FC0 == 25kHz or 10kHz 

        Single Shot, Prints $CASST immediatly: 
        --------------------------------------
            -$CCSST,1 -------------------- um.send_single_shot_sst()

        Continuous Printing (with each CAREV):  
        --------------------------------------
        ...
        How it works: 
        -------------
            -1 sample per probe length (i.e. each point has unique sampling of data) 
            -accummulate as many samples (up to max of 256) in CAREV cycle - total samples based on probe length, longer probe means more samples 
            -print summary statistics on samples when CAREV prints (roughly every 10 seconds) - min, lower quartile, median, upper quartile, max 
            -clear buffer and start accumulating samples again 


        MicroModem HW & SW configuration that effects CASST output:
        -----------------------------------------------------------
        Carrier:
        ...
        FML:
            -Sweep length - i.e. window size for averaging. 
        AGN:
        ...
        $CASST,0,58.728850,1,400,57.862410,58.381225,58.523375,58.728850,59.203695,191,*70
               |   |       |  |    |         |         |         |         |        |______ summary[len] number of probe lengths in summary (needs work, probe lengths probably not the right word)
               |   |       |  |    |         |         |         |         |_______________ summary[max] dB              
               |   |       |  |    |         |         |         |_________________________ summary[upper_quartile] dB    
               |   |       |  |    |         |         |___________________________________ summary[median] dB            
               |   |       |  |    |         |_____________________________________________ summary[lower_quartile] dB     
               |   |       |  |    |_______________________________________________________ summary[min] dB               
               |   |       |  |____________________________________________________________ number of samples (Need more info here....)
               |   |       |_______________________________________________________________ detector. This enum represents the detector type (e.g., psk chan, fsk chan, etc...) *only supports psk chan as of 3.2.2
               |   |_______________________________________________________________________ in water SPL dB average over last probe (SPL==sound pressure level?)
