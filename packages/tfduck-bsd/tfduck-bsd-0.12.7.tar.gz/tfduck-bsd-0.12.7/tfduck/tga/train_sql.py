...
from tfduck.tga.train_sql import TrainFeatureSql
    ...
    with new_user as (
                ...
                (
                    ...
                    (
                        SELECT
                            *,
                            row_number()  OVER  (PARTITION  BY  "#user_id") AS  row_no
                        FROM
                        (
                            SELECT
        ...
        from (
            ...
            (
                ...
                from (
                    ...
                    FROM neg_table
                ) where kere_nopoa_end_0 < (select count(1)*2 from nav_table)
            ) 
        ) st
    )
    , all_need_event as (
        ...
        WHERE
        rt_dur<={gconf.level_durs[-1]}  -- N小时内的事件
    ) 
    """
        return sql
