...
class SensorlessEstimatorError(enum.IntEnum):
    NONE                                     = 0x00000000
...
class EncoderError(enum.IntEnum):
    NONE                       = 0x00000000
    UNSTABLE_GAIN              = 0x00000001
...
class ThermistorCurrentLimiterError(enum.IntEnum):
    NONE    = 0x00000000
...
def _load_legacy_names():
    ...
    for var in dir(module):
        ...
        if isinstance(t, type) and issubclass(t, enum.Enum):
            for item in t:
                setattr(module, to_macro_case(var) + '_' + item.name, item.value)
