...
class Unum(object):
    ...
    UNIT_POW_SEP = ""
    """Separator between number/unit and expo."""
    ...
    def __str__(self):
        ...
        if Unum.AUTO_NORM and not self._normal:
            self.normalize(True)
            self._normal = True  
        result  = Unum.VALUE_FORMAT % self._value
        strUnit = self.strUnit()
        if strUnit:
            result += Unum.UNIT_INDENT + strUnit
        return result
