...
class DocCompiler:
    ...
    def __init__(
        ...
        font_source: Optional[str] = "google",
    ):
        self.user_js_files = js_files or []
        self.global_vars = global_vars or {}
        if font_source not in self.font_sources:
            raise ValueError(
                f"Unknown font source: {font_source}. Valid sources: {self.font_sources}"
            )
    ...
    def compile_header(self, doc: document):
        ...
        self._add_fonts(doc)
        if doc_styles := self._get_doc_styles(doc_tree):
    ...
    def _get_doc_styles(self, doc_tree) -> str:
        ...
        for selector, attributes in selector_styles.items():
            ...
            if base_selector in matched_selectors or doc_tree.cssselect(base_selector):
                attributes = [
