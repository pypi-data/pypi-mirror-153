...
class multiselect(component):
    def __init__(
        ...
        **kwargs,
        ...
        component_no = self.seq_next
        with self:
            d.div(id="ms-c-" + component_no, _class="ms-selections")
            if options:
                d.script(
                    "\n".join(
                        [f"add_ms_option('{component_no}','{op}');" for op in options]
                    )
                )
                if allow_user_added or suggest_user_add:
                    with d.span():
                        text_input(
                            suggestions=suggest_user_add, id="add-ms-op-" + component_no
                        )
                        d.button(
                            "Add Option",
                            onmousedown=f"add_ms_option('{component_no}');",
                        )
