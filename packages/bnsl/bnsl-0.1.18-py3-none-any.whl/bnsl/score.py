...
def nal(data, cols, alpha=0.3):
    ...
    data = np.delete(data, data[:, cols].min(axis=1) < 0, axis=0)
    if data.shape[0] == 0:
        return -np.inf
    else:
        return nal_counter(data, arities, cols, samplesize, alpha)
...
@njit(fastmath=True)
def nal_counter(data, arities, cols, samplesize, alpha=0.3):
    strides = np.empty(len(cols), dtype=np.uint32)
    idx = len(cols) - 1
    stride = 1
    while idx > -1:
