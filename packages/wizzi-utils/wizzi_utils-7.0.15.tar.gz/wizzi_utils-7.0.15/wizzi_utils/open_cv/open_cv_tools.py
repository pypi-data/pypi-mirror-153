...
class CameraWu:
    ...
    def set_resolution(self, w: int = 640, h: int = 480, ack: bool = True) -> bool:
        ...
        if old_res == new_res:
            mt.exception_error(e='resolution did not change. it is still {}'.format(old_res))
...
def get_dims_from_cap_imutils(cap) -> tuple:
    ...
    if cap.isOpened():
        ...
        orig_height = int(cap.cam.stream.get(cv2.CAP_PROP_FRAME_HEIGHT))
        out_dims = (orig_width, orig_height)
    return out_dims
...
class Mp4Creator:
    ...
    def finalize(self):
        if self.result is not None:
            self.result.release()
            self.result = None
            size_f = mt.file_or_folder_size(self.out_full_path)
            info = '{}File ready({} frames, frame size={}, file size={}): {}'.format(
                self.tabs * '\t', self.frames_count, self.out_dims, size_f, self.out_full_path)
            print(mt.add_color(info, ops=['g']))
        return
