...
class Beta4:
    ...
    def fit(self, X, y=None):
        ...
        p = Process(target=_fit_four, args=list(args))
        p.start()
    ...
    def irt(self, abilities, difficulties, discriminations):
        ...
        -------------------------------------------------------
        abilities : 
                Optimized ability estimation by gradient descent using Beta4-IRT

        difficulties : 
                Optimized difficulties estimation by gradient descent using Beta4-IRT

        discriminations : 
                Optimized discriminations estimation by gradient descent using Beta4-IRT
...
class Beta3:
    ...
    def fit(self, X, y=None):
        ...
        p = Process(target=_fit_three, args=list(args))
        p.start()
        abi, dif, dis = queue.get()
        p.join()
...
def _fit_four(*args):
    ...
    for _ in tqdm(range(epochs)):
        ...
        with tf.GradientTape() as g:
            g.watch(variables)
            pred = _irt_four(thi = thi, delj = delj, aj = aj, bj = bj)
...
def _fit_three(*args):
    """Train Beta^3- IRT with Gradient Descent
    ...
    for _ in tqdm(range(epochs)):
        ...
        with tf.GradientTape() as g:
            g.watch(variables)
            pred = _irt_three(thi = thi, delj = delj, aj = aj)
