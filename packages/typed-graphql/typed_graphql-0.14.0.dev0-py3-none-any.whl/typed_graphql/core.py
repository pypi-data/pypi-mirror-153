...
def graphql_type(cls, input_field: bool = False) -> GraphQLType:
    ...
    if is_dataclass(cls):
        for f in dataclass_fields(cls):
            ...
            field = Field(
                python_type_to_graphql_type(cls, f.type), resolve=resolver
            )
            fields[field_name] = field
...
def python_type_to_graphql_type(cls, t, nonnull=True, input_field=False):
    ...
    if str(t).startswith("graphql.type.definition.GraphQLList"):
        assert len(t.__args__) == 1
        return GraphQLList(python_type_to_graphql_type(cls, t.__args__[0], nonnull=True))
        ...
        return python_type_to_graphql_type(cls, t.__supertype__, input_field=input_field)
        ...
        return python_type_to_graphql_type(cls, get_arg_for_TypeVar(t, cls), nonnull=nonnull, input_field=input_field)
    else:
        try:
            if issubclass(t, str):
                if nonnull:
