...
class EncodeResponseFromSinker:
    ...
    def build_io_dataset_from_dict(self, io_property: dict):
        self._response.io_name = check_field(io_property, 'io_name')
        self._response.dataset_type = str_to_dataset_type(check_field(io_property, 'dataset_type'))
        self._response.data_type = str_to_data_type(check_field(io_property, 'data_type'))
        self._response.recoding = check_field(io_property, 'recoding')
    ...
    def build_io_dataset_from_dict(self, io_property: dict, io_value: any, timestamp):
        try:
            ...
            if isinstance(io_value, list):
                ...
                self.make_multi_io_value_array(io_dataset.multi_io_value, self._response.data_type, io_value)
            else:
                ...
                io_dataset.single_io_value.io_value.CopyFrom(
                    make_pb_io_value(io_value, self._response.data_type))
            self._response.io_dataset.extend(io_dataset)
        except Exception as e:
            print(str(e))
