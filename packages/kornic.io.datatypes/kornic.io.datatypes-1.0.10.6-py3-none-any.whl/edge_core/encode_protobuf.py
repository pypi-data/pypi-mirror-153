...
class EncodeBatchResponseFromSinker:
    ...
    def build_data_source_from_dict(self, data_source_property: dict):
        ...
        data_source.collection_type = str_to_collection_type(check_field(data_source_property, 'collection_type'))
        return data_source

    def set_data_source(self, data_source_property: ds_prop.DataSourceProperty):
        self._response.data_source.CopyFrom(data_source_property)
    ...
    def build_io_property_from_dict(self, io_property: dict):
        ...
        prop.dataset_type = str_to_dataset_type(check_field(io_property, 'dataset_type'))
        prop.io_type = str_to_io_type(check_field(io_property, 'io_type'))
        prop.data_type = str_to_data_type(check_field(io_property, 'data_type'))
        prop.ids.id1 = check_field(io_property, 'id1')
        prop.ids.id2 = check_field(io_property, 'id2')
        prop.ids.id3 = check_field(io_property, 'id3')
        prop.ids.id4 = check_field(io_property, 'id4')
        prop.recoding = check_field(io_property, 'recoding')
        prop.recoding = check_field(io_property, 'use_scale_convert', False, required=False)
        prop.recoding = check_field(io_property, 'in_min_limit', 0, required=False)
        prop.recoding = check_field(io_property, 'in_max_limit', 0, required=False)
        prop.recoding = check_field(io_property, 'out_min_limit', 0, required=False)
        prop.recoding = check_field(io_property, 'out_max_limit', 0, required=False)
        return prop
    ...
    def build_nested_io_from_dict(self, io_property: dict, io_value: any, timestamp):
        try:
            ...
            if isinstance(io_value, list):
                ...
                res_dataset.single_io_value.timestamp = self._now if timestamp is None else timestamp
                res_dataset.single_io_value.io_value.CopyFrom(
                    make_pb_io_value(io_value, nested_io.io_property.data_type))
            nested_io.get_value.CopyFrom(res_dataset)
            return nested_io
        except Exception as e:
            print(str(e))
