...
class DynamoFL(_Base):
    ...
    def _on_message(self, ws, res):
        ...
        if j['event'] == 'new-project':
            ...
            self.on_round_threads[threads_key] = gevent.spawn(self.train_callback, datasource_key, info)
        ...
        if j['event'] == 'project-complete':
            ...
            for p in self.project_participants:
                ...
                if project_key == p['project_key']:
                    self.on_round_threads[threads_key] = gevent.spawn(self.train_callback, p['datasource_key'], j['data']['project'])
    ...
    def train_callback(self, datasource_key, project_info):
        ...
        # Pull
        if os.path.isfile(federated_model_path):
            print(f'>>> Already downloaded round ({prev_round}) federated model.')
        else:
