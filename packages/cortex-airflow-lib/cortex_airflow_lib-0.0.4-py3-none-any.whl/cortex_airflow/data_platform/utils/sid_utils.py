...
from cortex_airflow.data_platform.utils.aws_utils import DynamoDB
import csv
import io

logger = logging.getLogger(__name__)
SID_MIN_DATE = np.datetime64('1800-01-01 00:00:00')
SID_MAX_DATE = np.datetime64('2100-01-01 00:00:00')


def check_completed_execution_id(execution_id: str, platform_url: str, loadmanager_url: str, login: str = None, password: str = None, credentials=None):
    if not credentials:
    ...
    if response.status_code != HTTPStatus.OK:
        raise ValueError(f'It was expected a {HTTPStatus.OK} http status code, it was received: {response.status_code} '
                         f'\n {response.text}')
    response_json = response.json()
    logger.info(f'EXECUTION RESPONSE: {response_json}')
    return response_json['completed'] and response_json.get('success', False)  # and not response_json.get('errors', False)


def get_execution_id_from_audit_table(s3_bucket: str, s3_key: str):
    # table_name = f"{Variable.get('ENVIRONMENT')}-audit" # TODO: ADICIONAR PROD ENVIRONMENT PARA O SID-INTEGRATION
    table_name = 'dev-audit'
    dynamo = DynamoDB(table_name=table_name)
    expression = Key('s3_bucket').eq(s3_bucket) & Key('s3_key').eq(s3_key)
    items = dynamo.query(expression=expression)
