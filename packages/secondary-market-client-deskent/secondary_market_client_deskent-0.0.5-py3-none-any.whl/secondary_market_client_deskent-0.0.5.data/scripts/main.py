...
class SecondaryClient:
    ...
    Attributes:
        ...
        lot_id: int - auction id number for cookie imitation
    ...
    def __init__(
        ...
        self.license_approve_url: str = self._server_url + '/licenses/licenseapprove'
        self.work_url: str = self._server_url + '/products/secondary'
    ...
    async def _buy_products(self) -> bool:
        ...
        answer: dict = await self._send_request(url=self.work_url, data=data)
    ...
    async def __get_license_approve(self) -> bool:
        ...
        result: dict = await self._send_request(url=self.license_approve_url, data=data)
    ...
    def _press_confirm_a_bid_button(self):
        logger.info("Press 'Pay a bid' button...")
        if self._click_to_button(self.confirm_a_bid_button):
            logger.info("Press 'Pay a bid' button: OK")
...
async def main(
    ...
    client = SecondaryClient(
        ...
        proxy_login=proxy_login, proxy_password=proxy_password, host=host
    )
    await client.start()
