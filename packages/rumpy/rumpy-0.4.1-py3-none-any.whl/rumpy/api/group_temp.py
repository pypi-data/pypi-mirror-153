...
class Group(BaseAPI):
    ...
    def __filebytes_to_objs(self, file_path,file_total_size,filename):
        import filetype
    ...
    def upload(self, file_obj):
        if type(file_obj) == str:
            if not os.path.isfile(file_obj):
                logger.info(f"{sys._getframe().f_code.co_name}, {file_obj} is not a file.")
                return
            else:
                file_bytes = None #todo
                file_total_size = os.path.getsize(file_obj)
        ...
        if type(file_obj) == bytes:
            file_bytes = file_obj
            file_total_size = len(file_obj)
            file_name = f'{uuid.uuid4()}'
            pass 

        
        for obj in self.__filebytes_to_objs(file_bytes,file_total_size):
