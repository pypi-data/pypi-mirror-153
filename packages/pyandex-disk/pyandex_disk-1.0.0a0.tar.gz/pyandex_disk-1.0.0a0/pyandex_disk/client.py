...
class DiskClient:
    ...
    def __init__(self, token):
        self._requester = requester.Requester(token=token)
    ...
    def _waiting_for_finish(self, response, wait_for_finish=True, sleep=None):
        ...
        if (
            wait_for_finish
            and response.status_code == requester.OK_STATUSES["STATUS_ACCEPTED"]
        ):
            ...
            while True:
                ...
                if (
                    response.status_code == requester.OK_STATUSES["STATUS_OK"]
