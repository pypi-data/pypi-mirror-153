...
class DrsClient(BaseServiceClient):
    ...
    def __init__(self, endpoint: ServiceEndpoint):
        ...
        self._events.add_fixed_types('download-ok', 'download-failure')
    ...
    def get_download_url(self,
                         drs_url: str,
                         no_auth: bool = False) -> Optional[str]:
        """
    ...
    def download_file(
        ...
        try:
            download_url: Optional[str] = self.get_download_url(url, no_auth=no_auth)
        ...
        try:
            ...
            self._events.dispatch('download-failure',
                                  DownloadFailureEvent.make(drs_url=url,
                                                            reason='Unexpected error while downloading the data',
                                                            error=e))
            self.exit_download(
                url,
                DownloadStatus.FAIL,
        ...
        if output_buffer_list is not None:
            ...
            with self.__output_lock:
                dataframe = file_to_dataframe(download_filename, data)
                try:
            ...
            output_file_path = f"{output_dir}/{download_filename}"
