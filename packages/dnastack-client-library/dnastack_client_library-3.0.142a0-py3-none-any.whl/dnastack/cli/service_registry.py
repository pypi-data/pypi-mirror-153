...
from dnastack import DataConnectClient, DrsClient
...
from dnastack.client.collections.client import STANDARD_COLLECTION_SERVICE_TYPE_V1_0, \
    EXPLORER_COLLECTION_SERVICE_TYPE_V1_0, CollectionServiceClient
    ...
    def __synchronize_endpoints_with(self, registry: ServiceRegistry):
        ...
        for sync_operation in sync_operations.values():
            if sync_operation.action in ('add', 'update', 'keep'):
                ...
                if config.defaults.get(short_type) is None:
                    config.defaults[short_type] = endpoint.id

            self.__print_sync_operation('Endpoint', sync_operation.action, endpoint)

        config.endpoints = sorted([sync_operation.endpoint for sync_operation in sync_operations.values()],
                                  key=lambda e: e.id)
