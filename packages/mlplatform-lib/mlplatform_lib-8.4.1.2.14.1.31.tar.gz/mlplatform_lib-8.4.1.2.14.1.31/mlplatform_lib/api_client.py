...
class ApiClient:
    ...
    def load_env(self):
        self.hyperdata_addr = os.environ["hyperdataAddr"] if "hyperdataAddr" in os.environ else "http://192.1.2.160:8080"
        self.virtualization_addr = os.environ["virtualizationAddr"] if "virtualizationAddr" in os.environ else "http://192.1.2.160:8100"
        self.proauth_addr = os.environ["proauthAddr"] if "proauthAddr" in os.environ else "http://192.1.2.160:28080"
        self.experiment_id = os.environ["experimentId"] if "experimentId" in os.environ else "1"
        ...
        self.pvc_mount_path = os.environ["pvcMountPath"] if "pvcMountPath" in os.environ else "/home/jovyan"
        self.serving_mount_path = os.environ["servingMountPath"] if "servingMountPath" in os.environ else None
        self.secret_mount_path = os.environ["secretMountPath"] if "secretMountPath" in os.environ else None
