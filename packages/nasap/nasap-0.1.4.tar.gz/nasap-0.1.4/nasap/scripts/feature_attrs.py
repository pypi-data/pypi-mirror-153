...
def main(gtf, forward_bw, reverse_bw, output_root='./tmp_output/' ):
  ...
  num_chr_list = [chrom for chrom in chr_list if re.match( r'chr\d+', chrom)]
  sort_chr_list = num_chr_list.sort(key=lambda arr: (arr[:3], int(arr[3:])))
  ...
  for chrom in sort_chr_list:
    ...
    rpkm_list.append( [gene_rpkm_dic[gene] for gene in gene_list] )
