...
def actualize_languages(path: str, main_lang: str = 'en', find_useless: Optional[str] = None, prefix: str = ''):
    ...
    for localizable_name in localizable_names:
        ...
        if find_useless:
            check_useless_keys(main_lang_meta, main_lang_strings, main_folder / localizable_name, code_pile, prefix)
...
def check_useless_keys(meta: List[str], strings: Dict[str, str], path: Path, code_pile, prefix):
    lang = path.parent.name.split('.')[0]
    for i, line in enumerate(meta):
        # кидать варнинг если ключ не используется в проекте
        key = f'{prefix}.{line[1:-1]}'
        if line in strings and key not in code_pile:
