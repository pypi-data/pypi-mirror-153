...
def parse_arguments():
    ...
    parser.add_argument('--pkg-bin', action='store_true', default=False,
                        help='use a task binary from package instead of repository task')
...
def run_test_tasks(tasks: dict, diff_output: bool, stop_on_failure: bool,
                   ...
                   skip_container_tasks: bool, task_bin: str) -> int:
    class Colors(object):
        BOLD = '\033[1m'
        GREEN = '\033[32m'
        ERROR = '\033[31m'
    ...
    base_cmd = f"{task_bin} --log_file={LOG_FILE} run"
    rc = 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        rc = run_test_tasks(tasks, not args.no_diff, args.stop_on_failure,
                            args.task, not args.no_colors, args.no_containers, task_bin)
