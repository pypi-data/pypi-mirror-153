...
from threadlocal_aws.clients import ecr, sso
...
def run_backend(args, instance):
    ...
    try:
        ...
        if "aws_access_key_id" in profile and "aws_secret_access_key" in profile and "aws_session_token" in profile:
            ...
            for filename in os.listdir(sso_cache):
                ...
                if isfile(full_file) and access(full_file, R_OK):
                    with open(full_file, "r") as cache_file:
                        ...
                        if cache_json and "startUrl" in cache_json and cache_json["startUrl"] == profile["sso_start_url"] and "accessToken" in cache_json and cache_json["accessToken"]:
                            ...
                            run_env["AWS_ACCESS_KEY_ID"] = role_creds["accessKeyId"]
                            run_env["AWS_SECRET_ACCESS_KEY"] = role_creds["secretAccessKey"]
                            run_env["AWS_SESSION_TOKEN"] = role_creds["sessionToken"]
