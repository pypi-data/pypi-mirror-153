...
class _UploadBase(object):
    ...
    def reload(self):
        ...
        _url = self._url(None)
        resp = self._client()._request(f"{_url}?expand=3")
    ...
    def _to_json_object(self, deep=True, top_level=True):
        ...
        for binding in self._bindings:
            ...
            if isinstance(val, _UploadBase):
                val = val._to_json_object(deep=deep, top_level=False)
            if val is not None:
...
class Task(_UploadBase):
    ...
    _bindings = (
        ...
        _binding('href'),
        _binding('state'),
        _binding('progress', expected=False),
    ...
    def _bind_custom_json(self, json):
        ...
        transformChain = json.get('transformChain', None)
        if transformChain:
            self.transform_type = transformChain.get('type')
            self.transforms = transformChain.get('transforms')
    ...
    def get_target_layer_name(self):
        ...
        '''
        return f'{self.target.workspace_name}:{self.layer.name}'
    ...
    def set_target(self, store_name=None, workspace=None, store_type=None):
        if self.target is None:
            ...
            if store_type not in Target._store_types:
                raise Exception(f"store_type must be one of {Target._store_types}")
            self.target = Target(None, self)
            self.target.store_type = store_type
            self.target.href = f"{self._url(None)}/target"
    ...
    def delete(self):
        ...
        if resp.status != 204:
            raise Exception(f'expected 204 response code, got {resp.status}', content)
    ...
    def upload_task(self, files, use_url=False, initial_opts=None):
        ...
        def addopts(base):
            if initial_opts:
                ...
                # options per key
                base = f"{base}&{'&'.join([f'option={v}:{k}' for k, v in initial_opts.items()])}"
            return base
        if use_url:
            if ext == '.zip':
                upload_url = files[0]
            ...
            url = self._url(f"imports/{self.id}/tasks?expand=3")
            upload_url = f"file://{os.path.abspath(upload_url)}"
            resp = self._client().post_upload_url(url, upload_url)
        elif ext == '.zip':
            url = self._url(f"imports/{self.id}/tasks/{fname}?expand=3")
