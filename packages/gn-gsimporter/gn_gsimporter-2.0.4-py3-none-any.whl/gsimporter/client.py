...
class _Client(object):
    ...
    def get_import(self, i):
        return parse_response(self._request(self.url(f"imports/{i}?expand=3")))
    ...
    def start_import(self, import_id=None, mosaic=False, name=None, target_store=False, charset_encoding="UTF-8"):
        ...
        if import_id is not None:
            url = self.url(f"imports/{import_id}")
            method = 'PUT'
        else:
    ...
    def post_multipart(self, url, files, fields=[]):
        ...
        for (key, value) in fields:
            L.append(f"--{BOUNDARY}")
            L.append(f'Content-Disposition: form-data; name="{str(key)}"')
            L.append('')
            L.append(str(value))
        for fpair in files:
            try:
            ...
            L.append(f"--{BOUNDARY}")
            L.append(f'Content-Disposition: form-data; name="{str(key)}"; filename="{str(filename)}"')
            L.append(f'Content-Type: {str(_get_content_type(filename))}')
            L.append('')
