...
from scrapy_demo.scrapy_demo import settings
...
class Spider(object):
    ...
    def change_pipline(pipeline_data: dict):
        ...
        if save_type == 'mongo':
            ...
            settings.ITEM_PIPELINES = {
                'scrapy_demo.scrapy_demo.pipelines.SpiderMongoDemoPipeline': 300,
            }
        elif save_type == 'csv':
            settings.FILE_PATH = pipeline_data.get('file_path')
            settings.ITEM_PIPELINES = {
                'scrapy_demo.scrapy_demo.pipelines.SpiderCsvPipeline': 300,
            }
    ...
    def change_setting(json_data: dict):
        ...
        if headers == None:
            settings.DOWNLOADER_MIDDLEWARES = {
                'scrapy_demo.scrapy_demo.middlewares.ScrapyDemoDownloaderMiddleware': 543,
            }
        else:
    ...
    def run(self):
        ...
        import os
        os.chdir(os.path.dirname(os.path.abspath(__file__)))  # 切换运行路径，解决找不到scrapy.cfg问题
        # 运行爬虫
