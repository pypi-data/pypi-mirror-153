...
__author__ = "Vera Del Favero, Daniel Wysocki"

from types import ModuleType
import numpy

class Kernel(object):
    def __init__(self, ndim, sparse=False, use_cython=True, xpy=numpy):
    ...
    def __eq__(self, other):
        ...
        if self.use_cython != other.use_cython:
            return False
...
class AddKernel(Kernel):
    ...
    def __init__(self, parent1, parent2):
        ...
        # use_cython should be consistent
        if self.parent1.use_cython == self.parent2.use_cython:
            self.use_cython = self.parent1.use_cython
        else:
            raise KeyError(
                "%s and %s are not consistently using cython"%(
...
class WhiteNoiseKernel(Kernel):
    ...
    '''
    def __init__(self, ndim, scale, sparse=True, use_cython=True, xpy=numpy):
    ...
    @classmethod
    def fit_simple(cls, x_train, scale=0.1, sparse=True, xpy=numpy, use_cython=True):
        # TODO fit noise kernel
        ndim = x_train.shape[1]

        # Return a Kernel initialized with the appropriate scale factor.
        return cls(ndim, scale, sparse=sparse, xpy=xpy, use_cython=use_cython)
    ...
    def from_attrs(cls, attrs):
        ...
        sparse = attrs["sparse"]
        use_cython = attrs["use_cython"]

        return cls(ndim, scale, sparse=sparse, use_cython=use_cython, xpy=xpy)
...
class CompactKernel(Kernel):
    ...
    '''
    def __init__(self, scale, order=1, sparse=True, use_cython=True, xpy=numpy):
        '''
        Parameters
        ----------
        scale: float
    ...
    @classmethod
    def fit_simple(cls, x_train, coeffs, order=1, sparse=True, xpy=numpy, use_cython=True):
        '''
        ...
        return cls(scale, order=order, sparse=sparse, xpy=xpy, use_cython=use_cython)

    @classmethod
    def fit_scott(cls, x_train, order=1, sparse=True, xpy=numpy, use_cython=True, **kwargs):
        '''
    ...
    def __call__(self, x, x_prime, train_err = None):
        ...
        if not self.use_cython:
            from .compact_kernel_evaluate import compact_kernel_evaluate
            ...
            from compact_kernel_cython import compact_kernel_evaluate
            K = compact_kernel_evaluate(
                                        x,
                                        x_prime,
                                        self.scale,
    ...
    def from_attrs(cls, attrs):
        ...
        return cls(scale, order=order, sparse=sparse, use_cython=use_cython, xpy=xpy)
...
class MaternKernel(Kernel):
    ...
    xpy: ModuleType, optional
        A module, either ``numpy`` or ``cupy``.  ``cupy`` is not yet supported.
    '''
    def __init__(self, ndim, nu, length_scale=1.0, use_cython=True, xpy=numpy):
    ...
    def __call__(self, x, x_prime, train_err = None):
        ...
        if not self.use_cython:
            ...
            from matern_kernel_cython import matern_kernel_evaluate
            K = matern_kernel_evaluate(
                                       x,
                                       x_prime,
                                       self.nu,
