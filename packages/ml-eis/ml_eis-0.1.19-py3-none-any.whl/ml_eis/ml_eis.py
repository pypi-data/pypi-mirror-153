...
def rdf_online(eis_data,cycling_data,n_estimators, max_features, max_depth,test_size,cyc_num):
    ...
    fig, ax = plt.subplots()
    fig.text(0.45,0.8,'Model $R^{2}$ = '+str(round(r2,2)))
    fig=plt.plot(cap_ret[0])
    plt.xlabel('Cycle Number')
    plt.ylabel('Capacity Retetion, [%]')

    return (fig,round(cap_ret[0][cyc_num-1],2),average_error(y_test,model.predict(X_test)))


def battery_clf(eis_data,cycling_data):
    Grade = {'98': 'AA', '97': 'A', '96': 'B+','95': 'B','94': 'C+','93': 'C'}
    X_features=ml_input(eis_data,cycling_data)
    rdf = joblib.load(filename)
    nn  = joblib.load(filename)
    gdb = joblib.load(filename)
    rdf_pred=rdf.predict(X_features)
    nn_pred=nn.predict(X_features)
    gdb_pred=gdb.predict(X_features)
    average_error= 0.15
    result = np.mean(rdf_pred[299],nn_pred[299],gdb_pred)
    clf=str(math.floor(result-average_error))
    return (Grade[clf])
...
def cap_ret_online_NN(eis_data,cycling_data, hidden_layers_sizes, learning_rate, alpha, n_iter, test_size, random_state=None):
    ...
    f_min=loadtxt(os.path.join(data_path,'freq_fit_min.csv'), delimiter=',')
    f_max=loadtxt(os.path.join(data_path,'freq_fit_max.csv'), delimiter=',')
    Z1_hf=loadtxt(os.path.join(data_path,'Z1_high_f.csv'), delimiter=',')
    Z2_hf=loadtxt(os.path.join(data_path,'Z2_high_f.csv'), delimiter=',')               
    ...
    plt.xlabel('Cycle Number')
    plt.ylabel('Capacity Retetion, [%]')
    avg_error=average_error(y_test,online_NN.predict(X_test))

    return (fig,round(cap_ret[0][cyc_num-1],2),avg_error)
