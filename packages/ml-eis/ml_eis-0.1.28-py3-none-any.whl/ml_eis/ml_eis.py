...
def battery_clf_t(eis_data,cycling_data):
    ...
    average_error= 0.15
    result = np.mean([rdf_pred[0][299],nn_pred[0][299],gdb_pred])
    clf=str(math.floor(result-average_error))
    return (Grade[clf],result,average_error)
...
def cap_ret_online_NN(eis_data,cycling_data, hidden_layers_sizes, learning_rate, alpha, n_iter, test_size,cyc_num,random_state=None):
    ...
    fig, ax = plt.subplots()
    fig.text(0.45,0.8,'Model $R^{2}$ = '+str(round(r2_online_NN,2)))
    fig=plt.plot(cap_ret_pred[0])
    plt.xlabel('Cycle Number')
    plt.ylabel('Capacity Retetion, [%]')
    avg_error=average_error(y_test,online_NN.predict(X_test))
    return (fig,round(cap_ret_pred[0][cyc_num-1],2),avg_error)
