...
def npm(repository: str, name: str):
    ...
    if bundler == "Vite":
        ...
        console.print(
            f"[green]Creating project with Vite, JavaScript and NPM[/]"
        )
...
def yarn(repository: str, name: str):
    ...
    if bundler == "Vite":
        os.chdir(os.path.join(home, repository))
        console.print(
            f"[green]Creating project with Vite, JavaScript and Yarn[/]"
        )
        if config.config["cli"]["displayOutput"]:
            subprocess.run(["yarn", "create", "vite", name, "--template", "react"], shell=True)
        else:
            subprocess.run(
...
def pnpm(repository: str, name: str):
    ...
    if bundler == "Vite":
        os.chdir(os.path.join(home, repository))
        console.print(
            f"[green]Creating project with Vite, JavaScript and Pnpm[/]"
        )
        if config.config["cli"]["displayOutput"]:
            subprocess.run(["pnpm", "create", "vite", name, "--", "--template", "react"], shell=True)
        else:
            subprocess.run(
