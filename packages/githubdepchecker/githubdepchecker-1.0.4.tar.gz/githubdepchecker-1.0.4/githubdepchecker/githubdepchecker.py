...
my_parser.add_argument("--createcsv", default=False, action=argparse.BooleanOptionalAction, help="Create csv of all the repositories whose language is JavaScript")
my_parser.add_argument("--org", nargs=1, type=str, help="Enter name of the organization you want to scan for javascript repositories")
            ...
            if args.update and not res:
                ...
                if fork_req.status_code == 202:
                    ...
                    try:
                        ...
                        # If user is checking his own repository then it will
                        # stop the process after committing the files since 
                        # we don't need to create pull request for updating our own repositories
                        
                        if variables["github_username"] != x[3]:
...
if args.createcsv:
    ...
    if args.org:
        ...
        url = "https://api.github.com/users/" + variables["github_username"] + "/repos"
    ...
    for i in req:
        if i["language"] == "JavaScript":
            createcsv_result.append([i["name"], i["html_url"]])

    if len(createcsv_result) != 0:
        with open(os.path.expanduser("~/repositories.csv"), 'w', encoding='UTF8', newline='') as f:
            writer = csv.writer(f)

            # write the header
            writer.writerow(createcsv_header)

            # write the data
            writer.writerows(createcsv_result)

            print("repositories.csv created in your home directory.")