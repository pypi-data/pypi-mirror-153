#!/usr/bin/python3
import json
from com.functional_test import onSubscribe_StockType_MD_TICK, onSubscribe_IndexType_MD_TICK, \
    onSubscribe_BondType_MD_TICK, onSubscribe_FundType_MD_TICK, onSubscribe_OptionType_MD_TICK, \
    onSubscribe_FuturesType_MD_TICK, onSubscribe_MD_KLINE, \
    onSubscribe_AD_FUND_FLOW_ANALYSIS, onSubscribe_MD_SECURITY_LENDING, onPlayback_FuturesType_MD_TICK, \
    onPlayback_MD_KLINE, onPlayback_AD_FUND_FLOW_ANALYSIS, \
    onPlayback_MD_SECURITY_LENDING, onPlayback_OptionType_MD_TICK, onPlayback_FundType_MD_TICK, \
    onPlayback_BondType_MD_TICK, onPlayback_IndexType_MD_TICK, onPlayback_StockType_MD_TICK, onPlaybackStatus, \
    onPlaybackResponse, onPlaybackControlResponse, onQueryResponse, onSubscribe_MD_TRANSACTION_and_MD_ORDER, \
    onPlayback_MD_TRANSACTION_and_MD_ORDER
...
class OnRecvMarketData:
        ...
        try:
            if marketdatajson["marketDataType"] == EMarketDataType.MD_TICK:  # .MD_TICK 快照
                if "mdStock" in marketdatajson:  # 股票
                    onSubscribe_StockType_MD_TICK(marketdatajson["mdStock"])
                elif "mdIndex" in marketdatajson:  # 指数
                    onSubscribe_IndexType_MD_TICK(marketdatajson["mdIndex"])
                elif "mdBond" in marketdatajson:  # 债券
                    onSubscribe_BondType_MD_TICK(marketdatajson["mdBond"])
                elif "mdFund" in marketdatajson:  # 基金
                    onSubscribe_FundType_MD_TICK(marketdatajson["mdFund"])
                elif "mdOption" in marketdatajson:  # 期权
                    onSubscribe_OptionType_MD_TICK(marketdatajson["mdOption"])
                elif "mdFuture" in marketdatajson:  # 期权
                    onSubscribe_FuturesType_MD_TICK(marketdatajson["mdFuture"])
            elif marketdatajson["marketDataType"] == EMarketDataType.MD_TRANSACTION:  # .MD_TRANSACTION:逐笔成交
                if "mdTransaction" in marketdatajson:
                    onSubscribe_MD_TRANSACTION_and_MD_ORDER(marketdatajson)
    ...
    def OnPlaybackMarketData(self, marketdatajson):
        try:
            if marketdatajson["marketDataType"] == EMarketDataType.MD_TICK:  # .MD_TICK 快照
                if "mdStock" in marketdatajson:  # 股票
                    onPlayback_StockType_MD_TICK(marketdatajson["mdStock"])
                elif "mdIndex" in marketdatajson:  # 指数
                    onPlayback_IndexType_MD_TICK(marketdatajson["mdIndex"])
                elif "mdBond" in marketdatajson:  # 债券
                    onPlayback_BondType_MD_TICK(marketdatajson["mdBond"])
                elif "mdFund" in marketdatajson:  # 基金
                    onPlayback_FundType_MD_TICK(marketdatajson["mdFund"])
                elif "mdOption" in marketdatajson:  # 期权
                    onPlayback_OptionType_MD_TICK(marketdatajson["mdOption"])
                elif "mdFuture" in marketdatajson:  # 期权
                    onPlayback_FuturesType_MD_TICK(marketdatajson["mdFuture"])
            elif marketdatajson["marketDataType"] == EMarketDataType.MD_TRANSACTION:  # .MD_TRANSACTION:逐笔成交
                if "mdTransaction" in marketdatajson:
                    onPlayback_MD_TRANSACTION_and_MD_ORDER(marketdatajson)
            elif marketdatajson["marketDataType"] == EMarketDataType.MD_ORDER:  # .MD_ORDER:逐笔委托
                if "mdOrder" in marketdatajson:
                    onPlayback_MD_TRANSACTION_and_MD_ORDER(marketdatajson)
            elif marketdatajson["marketDataType"] == EMarketDataType.MD_CONSTANT:  # .MD_CONSTANT:静态信息
