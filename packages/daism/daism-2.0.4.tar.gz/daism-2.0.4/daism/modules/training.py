...
def dnn_training(mixsam,mixfra,random_seed,modelpath,num_epoches=300,lr=1e-4,batchsize=64,ncuda=0,sum2one=False,plot = False):
    ...
    for epoch in range(num_epoches):
        ...
        epoch_loss.append(np.mean(mae_tr))     
        # early-stopping  
        if epoch >= min_epoch:
    ...
    if plot == True:
        ...
        plt.plot(Epoch,epoch_loss,c='r')
        plt.plot(Epoch,mae_ve,c='b')
        plt.legend(labels=['training set','validation set'],loc='upper right')
        plt.xlabel('epoch')
        plt.ylabel('MAE loss')
        plt.savefig(modelpath+"loss_curve.jpg")
