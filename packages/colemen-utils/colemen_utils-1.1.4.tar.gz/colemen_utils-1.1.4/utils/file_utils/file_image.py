...
# from utils.file_utils.file_utils import get_data as f.get_data
# from utils.file_utils.file_utils import exists as f.exists
# from utils.file_utils.file_utils import delete as f.delete
# from utils.file_utils.file_image import get_keywords as f.get_keywords
# from utils.file_utils.file_image import save_file_obj as f.save_file_obj
# from utils.file_utils.file_search import by_name as f.by_name

# from utils.string_utils.string_format import to_snake_case as csu.to_snake_case
...
def _keywords_to_list(keywords,delimiter=",",**kwargs):
    ...
    if to_snake_case is True:
        newlist = [csu.to_snake_case(x) for x in newlist]
...
def tags_to_snakecase(files):
    ...
    for file in file_array:
        ...
        new_tags = []
        for tag in tags:
            new_tags.append(csu.to_snake_case(tag))
        if len(new_tags) > 1:
...
def get_meta_only(file_path):
    ...
    # et = f.exiftool.ExifTool(executable_=r"Z:\Structure\Archive\Programming Packages - Libraries\Python\colemen_file_utils\.venv\exiftool.exe")
    with f.exiftool.ExifTool(executable_=r"Z:\Structure\Archive\Programming Packages - Libraries\Python\colemen_file_utils\.venv\exiftool.exe") as et:
...
def apply_keyword_synonyms(files,synonyms,**kwargs):
    ...
    for file in files_array:
        kws = f.get_keywords(file)
        kws_hash_prime = csu.to_hash(kws)
...
def apply_synonyms(file_keys,synonyms):
    ...
    # new_file_keys_hash = csu.to_hash(new_file_keys)
...
def auto_tag_by_folder_name(watch_path,**kwargs):
    ...
    apply_current_tags = obj.get_kwarg(['apply_current_tags'],False,(bool),**kwargs)
    ...
    while True:
        ...
        for file in moved_files:
            ...
            result = f.by_name(file['file_name'], watch_path)
...
def apply_current_folders_as_tag(files,**kwargs):
    save = obj.get_kwarg(['save'],False,(bool),**kwargs)
    if f.directory.exists(files):
        files = f.get_files(files, ext="images",include_meta_data=True)
    # if files is False:
    files_array = _parse_file_obj_list(files)
    update_array = []
    # new_file_array = []
