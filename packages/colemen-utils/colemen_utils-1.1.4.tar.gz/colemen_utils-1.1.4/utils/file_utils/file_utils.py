...
import utils.directory_utils as directory

# from utils.string_utils.string_format import array_in_string as csu.array_in_string
# from utils.string_utils.string_format import file_path as csu.file_path
# from utils.string_utils.string_format import extension as csu.extension
# from utils.string_utils.string_generation import variations as csu.variations
# from utils.string_utils.string_generation import to_hash as csu.to_hash
...
def _decompress_single_file_gzip(file_path):
    temp_path = csu.file_path(f"{os.path.dirname(file_path)}/{get_name_no_ext(file_path)}.decomp")
    contents = False
    try:
        with gzip.open(file_path, 'rb') as file:
            with io.TextIOWrapper(file, encoding='utf-8') as decoder:
                contents = decoder.read()
                f.write(temp_path, contents)
...
def import_project_settings(file_name):
    ...
    if exists(settings_path) is False:
        settings_path = f.by_name(file_name, os.getcwd(), exact_match=False)
        if settings_path is False:
            return False
    return f.as_json(settings_path)
...
def _parse_copy_data_from_obj(file_obj):
    ...
    if isinstance(file_obj, (dict)):
        for syn in f.resources.SRC_PATH_SYNONYMS:
            synvar = csu.variations(syn)
            for synonym_variant in synvar:
                if synonym_variant in file_obj:
                    data['src_path'] = file_obj[synonym_variant]
        for syn in f.resources.DEST_PATH_SYNONYMS:
            synvar = csu.variations(syn)
            for synonym_variant in synvar:
                if synonym_variant in file_obj:
...
def get_data(file_path, **kwargs):
    ...
    if ftp is not None:
        file_data = {}
        path = csu.file_path(file_path, url=True)
    ...
    file_path = csu.file_path(file_path)
...
def gen_relative_path(src_base, dst_base, file_path):
    ...
    '''
    src_base = csu.file_path(src_base)
    dst_base = csu.file_path(dst_base)
    file_path = csu.file_path(file_path)
...
def gen_dst_path(src_base, dst_base, file_path):
    ...
    if src_base not in file_path and dst_base not in file_path:
        # print(f"tatertots")
        return csu.file_path(f"{dst_base}/{file_path}")
    relative_path = gen_relative_path(src_base, dst_base, file_path)
    return csu.file_path(f"{dst_base}/{relative_path}")
...
def gen_src_path(src_base, dst_base, file_path):
    ...
    file_path = csu.file_path(file_path)
    if src_base not in file_path and dst_base not in file_path:
        return csu.file_path(f"{src_base}/{file_path}")
    relative_path = gen_relative_path(src_base, dst_base, file_path)
    return csu.file_path(f"{src_base}/{relative_path}")
            ...
            for file in files:
                ...
                if file_data is not None:
                    ...
                    if len(exclude) > 0:
                        ...
                        if csu.array_in_string(exclude, file_data['file_path']) is True:
                            continue

                    if len(include) > 0:
                        # print(f"filtering includes")
                        if csu.array_in_string(include, file_data['file_path']) is False:
                            continue

                    if len(extension_array) > 0:
                        # print(f"filtering extension_array")
                        file_ext = csu.extension(file['extension'])
...
def _add_jpg_meta_data(file_array):
    ...
    for file in file_array:
        if file['extension'] == ".jpg":
            ...
            file['file_path'] = csu.file_path(file['file_path'],url=True)
            file['file_path_exif_copy'] = csu.file_path(f"{file['file_path']}_original",url=True)
    ...
    if len(jpg_array) > 0:
        ...
        with f.exiftool.ExifTool(executable_=r"Z:\Structure\Archive\Programming Packages - Libraries\Python\colemen_file_utils\.venv\exiftool.exe") as et:
...
def get_files_ftp(search_path=False, **kwargs):
    ...
    for path in search_path:
        ...
        for root, folders, files in ftp.walk(path):
            for file in files:
                file_path = csu.file_path(os.path.join(root, file), url=True)
                ...
                if file_data is not None:
                    ...
                    if len(extension_array) > 0:
                        file_ext = csu.extension(file_data['extension'])
                        if file_ext not in extension_array:
                            continue
...
class GetFilesThreaded:
    ...
    def __init__(self, search_path, **kwargs):
        ...
        self.extension_array = csu.extension(
            obj.get_kwarg(['extensions', 'ext', 'extension'], [], (str, list), **kwargs))
    ...
    def _get_data_thread(self, file_path):
        ...
        if file_data is not None:
            ...
            if len(self.exclude) > 0:
                if csu.array_in_string(self.exclude, file_data['file_path']) is True:
                    return

            if len(self.include) > 0:
                if csu.array_in_string(self.include, file_data['file_path']) is False:
                    return
    ...
    def master(self):
        ...
        for path in self.search_path:
            ...
            for root, folders, files in os.walk(path):
                ...
                data = {
                    "thread_id": csu.to_hash(json.dumps(file_paths)),
        ...
        #     extension_array = csu.extension(obj.get_kwarg(['extensions', 'ext', 'extension'], [], (str, list), **kwargs))
        #     if isinstance(extension_array, (str)):
        #         extension_array = [extension_array]
