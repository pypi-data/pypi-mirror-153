...
def _keywords_to_list(keywords,delimiter=",",**kwargs):
    ...
    if to_snake_case is True:
        newlist = [csu.mod.to_snake_case(x) for x in newlist]
    
    return newlist

def genCssMediaScales(src_path):
...
def tags_to_snakecase(files):
    ...
    for file in file_array:
        ...
        new_tags = []
        for tag in tags:
            new_tags.append(csu.mod.to_snake_case(tag))
        if len(new_tags) > 1:
...
def get_meta(files,force_update=False):
    ...
    # et = exiftool.ExifTool(executable_=r"Z:\Structure\Archive\Programming Packages - Libraries\Python\colemen_file_utils\.venv\exiftool.exe")
    for file in file_array:
        file['file_path'] = csu.mod.file_path(file['file_path'],url=True)
        file['file_path_exif_copy'] = csu.mod.file_path(f"{file['file_path']}_original",url=True)
...
def apply_keyword_synonyms(files,synonyms,**kwargs):
    ...
    for file in files_array:
        ...
        kws_hash_prime = csu.gen.to_hash(kws)
        ...
        kwsHash = csu.gen.to_hash(new_tags)
...
def apply_synonyms(file_keys,synonyms):
    ...
    # file_keys_hash = csu.gen.to_hash(file_keys)
    added_keys = False
    ...
    # new_file_keys_hash = csu.gen.to_hash(new_file_keys)
