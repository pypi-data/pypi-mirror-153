...
# from utils.string_utils.string_conversion import bool_to_int,bool_to_string,to_bool
from utils.dict_utils.dict_utils import get_kwarg as _get_kwarg
from utils.dict_utils.dict_utils import set_defaults as _set_defaults
from utils.list_utils.list_utils import append as _append
from utils.string_utils.string_format import strip_any as _strip_any
from utils.string_utils.string_format import strip_empty_lines as _strip_empty_lines
from utils.file_utils.file_utils import exists as _file_exists
from utils.file_utils.file_read import read as _read_file
...
def parse_column_data(value=None):
    ...
    if len(res) > 0:
        ...
        # print(new_data)
        new_data = _set_defaults(defaults,new_data)
        output = new_data
    return output
...
def _parse_table_column_lines(lines):
    ...
    for line in lines:
        ...
        if is_line_key(line):
            ...
            data['keys'] = _append(data['keys'],parse_key(line))
            pk = parse_primary_key(line)
            if pk is not None:
                data['primary_keys'] = _append(data['primary_keys'],pk['primary_key'])
            # data['primary_keys'] = _append(data['primary_keys'],parse_primary_key(line))

        if is_line_constraint(line):
            data['constraints'] = _append(data['constraints'],parse_constraint(line))


        data['columns'] = _append(data['columns'],parse_column_data(line))
...
def parse_list(value):
    ...
    vals = value.split(",")
    vals = [_strip_any(x,[" "]) for x in vals]
    return ','.join(vals)
...
def parse_col_comment(col_data):
    ...
    for s in col_data['shlexed']:
        ...
        if s.lower().startswith('comment'):
            ...
            col_data['comment'] = _strip_any(cmt,[" "])
            break
...
def parse_primary_keys(sql):
    ...
    if match is not None:
        if len(match) > 0:
            ...
            keys = keys.split(",")
            keys = [_strip_any(x,[" "]) for x in keys]
            primary_keys = keys
        # print(match)
    return primary_keys
...
def parse(sql):
    ...
    for s in raw_statements:
        ...
        if state['purpose'] == "CREATE TABLE":
            ...
            _append(data['tables'],state['data'])
            continue
        ...
        if state['purpose'] == "CREATE SCHEMA":
            ...
            _append(data['schemas'],state['data'])
            continue
...
#             if _file_exists(self.data['file_path']):
