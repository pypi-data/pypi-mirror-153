...
'''
    Utility methods used for searching for files.
'''
...
from utils.string_utils.string_format import extension as _format_extension
...
def by_name(file_name, search_path=None, **kwargs):
    ...
    extensions = _format_extension(obj.get_kwarg(['extensions', 'ext'], [], (list, str), **kwargs))
    if isinstance(extensions, str):
        extensions = [extensions]

    recursive = obj.get_kwarg(['recursive', 'recurse'], True, bool, **kwargs)
    case_sensitive = obj.get_kwarg(['case_sensitive'], True, (bool), **kwargs)
    exact_match = obj.get_kwarg(['exact_match'], True, (bool), **kwargs)
    regex = obj.get_kwarg(['regex', 'use regex'], False, (bool), **kwargs)
    ...
    for root, folders, files in os.walk(search_path):
        for file in files:
            ...
            test_file_ext = _format_extension(os.path.splitext(os.path.basename(current_file_path))[1])
