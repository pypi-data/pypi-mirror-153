...
def decompress_zip_file(file_path,dst=None,**kwargs):
    ...
    file_data = f.get_data(file_path)
    if file_data:
        if dst is None:
    ...
    if recursive is True:
        ...
        while(cycle):
            ...
            if cycle_count > 100:
                ...
                return None
            zips = f.get_files(start_path,extension=["zip","rar"],show_count=False)
        ...
        zips = f.get_files(start_path,extension="zip",show_count=False)
        for zf in zips:
...
def decompress_single_zip_file(src,dst=None,**kwargs):
    ...
    src = csu.file_path(src)
    dst = csu.file_path(dst)
    directory.create(dst)

    src_ext = f.get_ext(src)
