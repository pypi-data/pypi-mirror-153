...
# import facades.string_facade as strUtils
# import utils.files.file as file
# import utils.object_utils as obj
# import utils.files.file_write as write
# import utils.files.dir_compression as compression
# import utils.files.dir_search as search
...
def get_files(search_path=False, **kwargs):
    return f.get_files(search_path, **kwargs)
    ...
    for path in search_path:
        ...
        for root, folders, files in os.walk(path):
            ...
            for current_dir in folders:
                ...
                ignore = False
                if len(exclude) > 0:
                    if csu.array_in_string(exclude, dir_data['file_path']) is True:
                        continue
                if len(include) > 0:
                    if csu.array_in_string(include, dir_data['file_path']) is False:
                        continue
    ...
    for folder in dirs:
        ...
        try:
            ...
            if dirs_only is False:
                files = f.get_files(search_path=folder['file_path'], include=include_files,
                ...
                if empty_files is True:
                    for file in files:
                        f.write(file['dst_path'], "EMPTY TEST FILE CONTENT")
                else:
                    f.copy(files)
        except:
