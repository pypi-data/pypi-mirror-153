...
def history_matrix(ctx):
    ...
    if window_size > 0:
        ...
        # truncated estimation pass that ends at the forecasting date.
...
class Snapshot:
    ...
    + **vec** – The particle state vectors.

    :param time: The simulation time.
    :param steps_per_unit: The number of time-steps per unit time.
    :param history: The :class:`History` component.
    :param hist_ix: The index of this snapshot in the history matrix.
    """
    def __init__(self, time, steps_per_unit, history, hist_ix):
        self.time = time
        self.hist_ix = hist_ix
        self.weights = history.matrix['weight'][hist_ix]
        self.vec = history.matrix[hist_ix]
    ...
    def back_n_steps(self, n):
        ...
        .. note:: This returns the state vector array, **not** a
           :class:`Snapshot`.

        :param n: The number of time-steps to step back.
        """
        return earlier_states(self.__history.matrix, self.hist_ix, n)
...
class History:
    ...
    def load_state(ctx, group):
        ...
        return History(ctx, matrix, offset, times)
    ...
    def summary_window(self, ctx, start, end):
        ...
        # Identify the time-steps associated with each time unit in the
        # summary window.
        window_times = [
            (time_val, step_num + self.offset)
            for (step_num, time_val) in time.steps()
            if start <= time_val <= end
            and step_num % steps_per_unit == 0]

        # Add the initial state (time-step zero) if it falls in the window.
        start_time = ctx.settings['time']['sim_start']
        if start <= start_time <= end:
            window_times.insert(0, (start_time, 0 + self.offset))

        return [Snapshot(time, steps_per_unit, self, hist_ix)
    ...
    def snapshot(self, ctx, expect_time=None):
        ...
        steps_per_unit = ctx.settings['time']['steps_per_unit']
        return Snapshot(time, steps_per_unit, self, self.index)
    ...
    def previous_snapshot(self, ctx, expect_time=None):
        ...
        if expect_time is not None and time != expect_time:
            ...
            raise ValueError(msg_fmt.format(time, expect_time))
        steps_per_unit = ctx.settings['time']['steps_per_unit']
        return Snapshot(time, steps_per_unit, self, self.index - 1)
