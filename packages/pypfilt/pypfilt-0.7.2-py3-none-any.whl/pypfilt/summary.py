...
class Table(abc.ABC):
    ...
    def add_rows(self, ctx, fs_date, window, insert_fn):
        ...
        :param window: A list of :class:`~pypfilt.state.Snapshot` instances
            that capture the particle states at each time unit in the
            simulation window.
...
class ParamCovar(Table):
    ...
    def n_rows(self, ctx, start_date, end_date, n_days, forecasting):
        return n_days * self.__num_params * (self.__num_params - 1) // 2

    def add_rows(self, ctx, fs_date, window, insert_fn):
        from . import stats

        for snapshot in window:
            x = state.repack(snapshot.vec['state_vec'][self.__smooth_fields])
            ...
            for ix1 in range(self.__num_params):
                ...
                for ix2 in range(ix1 + 1, self.__num_params):
                    ...
                    row = (fs_date, snapshot.time, name1, name2,
                           covars[ix1, ix2])
                    insert_fn(row)
        ...
        for snapshot in window:
            ...
            if np.count_nonzero(mask):
                ...
                for pctl in self.__probs:
                    for field in self.__field_names:
                        row = (fs_date, snapshot.time, pctl, 0, 0,
                               'model', field)
                        insert_fn(row)
                    for (val, _) in self.__stat_info:
                        row = (fs_date, snapshot.time, pctl, 0, 0, 'stat', val)
                        insert_fn(row)


class EnsembleSnapshot(Table):
    """
    Record the particle state vectors at each time unit of the estimation and
    forecasting passes.

    .. note:: These snapshots capture the ensemble at each time unit.
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        self.__each_time_unit = ctx.settings.get_chained(
            ['summary', 'tables', name, 'each_time_unit'],
            False)
        ...
        return [fs_date, date, weight] + fields

    def n_rows(self, ctx, start_date, end_date, n_days, forecasting):
        num_particles = ctx.settings['filter']['particles']
        return num_particles * n_days

    def add_rows(self, ctx, fs_date, window, insert_fn):
        for snapshot in window:
            # Record the state of each particle.
            for (row_ix, row) in enumerate(snapshot.vec['state_vec']):
                weight = snapshot.weights[row_ix]
                insert_fn(tuple((fs_date, snapshot.time, weight, *row)))
...
class ForecastSnapshot(Table):
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        date = time_field('date')
        fields = ctx.component['model'].field_types(ctx)
        fields.insert(0, fs_date)
        fields.insert(1, date)
        return fields
    ...
    def n_rows(self, ctx, start_date, end_date, n_days, forecasting):
        ...
        if self.__each_time_unit:
            num_snapshots = n_days
        else:
            num_snapshots = 1
    ...
    def add_rows(self, ctx, fs_date, window, insert_fn):
        for snapshot in window:
            if snapshot.time < fs_date:
                continue
            if not self.__each_time_unit:
                if snapshot.time != fs_date:
                    continue
            ...
            for ix in sample_ixs:
                row = snapshot.vec['state_vec'][ix]
                insert_fn(tuple((fs_date, snapshot.time, *row)))
...
class SimulatedObs(Table):
    ...
    def field_types(self, ctx, obs_list, name):
        ...
        # Ensure the observation model does not define a 'fs_date' field, or
        # we cannot record the forecasting date for each observation.
        ...
        for field in simulated_fields:
            if field[0] == 'fs_date':
                msg_fmt = 'Observation model for {} defines "fs_date" field'
                raise ValueError(msg_fmt.format(self.__obs_unit))
        self.__obs_dtype = fields_dtype(ctx, simulated_fields)

        # Add a 'fs_date' column to the simulated observation fields.
        fs_date = time_field('fs_date')
        simulated_fields.insert(0, fs_date)
    ...
    def n_rows(self, ctx, start_date, end_date, n_days, forecasting):
        ...
        # Need one row for each particle, for each day.
        n_px = ctx.settings['filter']['particles']
        return n_px * n_days
        ...
        for unit in self.__obs_units:
            ...
            for snapshot in window:
                ...
                for ix, pr in enumerate(self.__probs[::-1]):
                    row = (unit, fs_date, snapshot.time, pr,
                           qtls[ix], qtls[- (ix + 1)])
                    insert_fn(row)
...
class BackcastMonitor(Monitor):
    ...
    def end_sim(self, ctx, start_date, end_date):
        if self.__forecasting:
            return
...
class BackcastPredictiveCIs(Table):
    ...
    def n_rows(self, ctx, start_date, end_date, n_days, forecasting):
        ...
        n_obs_models = len(ctx.component['obs'])
        n_backcast_days = len(self.__monitor.window)
        return len(self.__probs) * n_backcast_days * n_obs_models
...
class HDF5(object):
    ...
    def allocate(self, ctx, forecasting=False):
        ...
        start_date = ctx.settings['time']['sim_start']
        end_date = ctx.settings['time']['sim_until']
        self.__start_date = start_date
        self.__end_date = end_date
        if forecasting:
            # Forecasting from the start of the simulation period.
            self.__fs_date = self.__start_date
        else:
            ...
            self.__fs_date = ctx.settings['time']['until']
        ...
        # NOTE: start statistics at the beginning of the simulation period.
        self.__all_dates = [start_date]
        for (step, time) in ctx.component['time'].steps():
            if step % steps_per_unit == 0 and time > start_date:
        ...
        for mon in self.__monitors.values():
            mon.begin_sim(ctx, start_date, end_date, n_days, forecasting)
        ...
        n_rows = {n: t.n_rows(ctx, start_date, end_date, n_days,
                              forecasting)
    ...
    def summarise(self, ctx, window):
        ...
        if num_dates == 0:
            logger.debug("Summary.summarise: no dates in window")
            return
        ...
        for mon in self.__monitors.values():
            mon.monitor(ctx, fs_date, window)

        tables = self.__df if self.__df is not None else []

        for tbl in tables:
            insert_fn = self.__insert[tbl]
            self.__tbl_dict[tbl].add_rows(ctx, fs_date, window, insert_fn)

        if end_date == self.__end_date:
            for mon in self.__monitors.values():
                mon.end_sim(ctx, fs_date, window)

            for tbl in tables:
                insert_fn = self.__insert[tbl]
                self.__tbl_dict[tbl].finished(ctx, fs_date, window, insert_fn)
    ...
    def save_forecasts(self, ctx, fs, filename):
        """
