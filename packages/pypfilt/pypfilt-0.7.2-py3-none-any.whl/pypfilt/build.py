...
def build_time(settings):
    ...
    """
    time_scale = instantiate(settings['components']['time'])
    return time_scale
...
def validate_time(time_scale, settings):
    ...
    """
    time_start = settings['time']['start']
    time_until = settings['time']['until']

    if isinstance(time_start, str):
        time_start = time_scale.from_unicode(time_start)
    elif not time_scale.is_instance(time_start):
        msg_fmt = 'Invalid value for time.start: "{}"'
        raise ValueError(msg_fmt.format(type(time_start)))

    if isinstance(time_until, str):
        time_until = time_scale.from_unicode(time_until)
    elif not time_scale.is_instance(time_until):
        msg_fmt = 'Invalid value for time.until: "{}"'
        raise ValueError(msg_fmt.format(type(time_until)))
...
def sample_lookup_columns(settings, prng, lookup_tables):
    ...
    for name in sample_names:
        ...
        sampled_ixs[name] = prng.integers(num_values, size=particles)
...
def load_observations(settings, obs_models, time_scale):
    ...
    for (obs_unit, obs_model) in obs_models.items():
        ...
        # Ensure the observation table includes a 'date' column.
        # Otherwise the simulation cache will not work as intended.
        if 'date' not in obs_table.dtype.names:
            msg_fmt = 'No "date" column in {} table at {}'
