...
# generating results
def get_results(genes, mode, from_own, font_size, edge_thicness):
    node_positions = {}
    position = 0
...
def get_comp_results(genes_1, genes_2, font_size, edge_thicness):
    node_positions = {}
    position = 0
...
def run(**kwargs):
    ...
    edge_thicness = kwargs.get("edge_thicness")
    ...
    if font_size is None:
        font_size = 10
    if edge_thicness is None:
        edge_thicness = 0.1
    if perform_go_analysis:
        go_data = get_results(genes_1, "GO", from_go, font_size, edge_thicness)
    if perform_pw_analysis:
        pw_data = get_results(genes_1, "PW", from_pw, font_size, edge_thicness)
    if len(genes_2) != 0:
        comp_data = get_comp_results(genes_1, genes_2, font_size, edge_thicness)
