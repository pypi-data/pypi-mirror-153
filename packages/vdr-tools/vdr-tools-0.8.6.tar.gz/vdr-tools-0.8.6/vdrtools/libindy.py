...
def _load_cdll() -> CDLL:
    ...
    libvdrtools_prefix_mapping = {"darwin": "lib", "linux": "lib", "linux2": "lib", "win32": ""}
    libvdrtools_suffix_mapping = {"darwin": ".dylib", "linux": ".so", "linux2": ".so", "win32": ".dll"}

    os_name = sys.platform
    logger.debug("_load_cdll: Detected OS name: %s", os_name)

    try:
        libvdrtools_prefix = libvdrtools_prefix_mapping[os_name]
        libvdrtools_suffix = libvdrtools_suffix_mapping[os_name]
    except KeyError:
        logger.error("_load_cdll: OS isn't supported: %s", os_name)
        raise OSError("OS isn't supported: %s", os_name)

    libvdrtools_name = "{0}vdrtools{1}".format(libvdrtools_prefix, libvdrtools_suffix)
    logger.debug("_load_cdll: Resolved libvdrtools name is: %s", libvdrtools_name)

    try:
        res = CDLL(libvdrtools_name)
        logger.debug("_load_cdll: <<< res: %s", res)
        return res
    except OSError as e:
        logger.error("_load_cdll: Can't load libvdrtools: %s", e)
        raise e
...
def _set_logger():
    ...
    def _log(context, level, target, message, module_path, file, line):
        libvdrtools_logger = logger.getChild('native.' + target.decode().replace('::', '.'))

        level_mapping = {1: ERROR, 2: WARNING, 3: INFO, 4: DEBUG, 5: TRACE, }

        file_str = file.decode() if file is not None else None

        libvdrtools_logger.log(level_mapping[level],
                           "\t%s:%d | %s",
                           file_str,
                           line,
                           message.decode())
