...
from com.dvsnier.config.journal.common_logging import Logging
from com.dvsnier.directory.common_dir import generate_complex_file_name
from com.dvsnier.process import DVS_TEMP, ENVIRONMENT_VARIABLE_TEMP
...
def execute(cmds, quiet=True):
    ...
    stdouts = None
    logging = Logging()
    dvs_temp = DVS_TEMP()  # type: str
    logging.set_kw_output_dir_name(dvs_temp)
    logging.set_kw_file_name('log')
    logging.set_kw_level(logging.DEBUG)
    logging.set_logging_name('dvs-execute-task')
    logging.build()
    if os.path.exists(dvs_temp):
        logging.info('the currently found environment configuration variable({}: {}).'.format(
            ENVIRONMENT_VARIABLE_TEMP, dvs_temp))
    else:
        os.makedirs(dvs_temp)
    start = time.time()
