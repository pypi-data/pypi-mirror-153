...
def _send_email(obj, mails, bcc=None, cc=None):
    ...
    with mail.get_connection() as connection:
        ...
        try:
            ...
            logger.debug("Error send message", exc_info=True)
            obj.problems = True
            obj.save()
...
def send_email(obj):
    ...
    if not isinstance(obj, EmailNotification):
        try:
            ...
            logger.debug("Email not found %r"%(obj, ), exc_info=True)
            return
    if SEND_ONLY_EMAIL:
...
def send_daily():
    ...
    for email in EmailNotification.objects.filter(sent=False, enqueued=True):
        try:
            ...
            logger.debug("Error send message %r, continue with next"%(email,), exc_info=True)
            import traceback
            traceback.print_exc()
    return "sent %d" % (sent,)
