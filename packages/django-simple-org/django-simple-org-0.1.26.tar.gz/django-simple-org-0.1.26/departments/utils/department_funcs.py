...
def create_department(data: dict) -> dict:
    ...
    if parent.organization_id != data["organization_id"]:
        raise ValueError(f"organization_id:{data['organization_id']} different of parent organization_id: {parent.organization_id}")
    if parent.path:
        path = parent.path
...
def mv_show_order(organization_id: int, data: dict):
    ...
    #
    # for item in data:
    #     obj = Department.objects.filter(id=item["id"], organization_id=organization_id)
    #     if not obj:
    #         raise ValueError(f"organization {organization_id} not found department {item['id']}")
    #     obj.update(show_order=item["show_order"])
