...
def generate_conf_small_power():
    print('generating configuration files and store in :'+FILECONF_SMALLPOWER_PKL)
    start=time.time()
    f = open(FILECONF_SMALLPOWER_PKL,'wb')
...
FOLDERPKL = os.getenv('HOME')+'/smallPower_daily/'

if os.path.exists(os.path.dirname(__file__) + '/parameters_conf.py'):
    from .parameters_conf import *

appdir     = os.path.dirname(os.path.realpath(__file__))
CONFFOLDER = os.path.dirname(appdir) + '/confFiles/'
FILECONF_SMALLPOWER = CONFFOLDER + 'smallpower_configfiles.ods'
FILECONF_SMALLPOWER_PKL = CONFFOLDER + 'smallpower_conf.pkl'
FILE_PLC_XLSM = glob.glob(CONFFOLDER+'*ALPHA*.xlsm')[0]
LOG_FOLDER=os.path.dirname(appdir)+'/logs/'


if os.path.exists(FILECONF_SMALLPOWER_PKL):
    smallpower_objs= Utils().loads_pickle(FILECONF_SMALLPOWER_PKL)
else:
    smallpower_objs=generate_conf_small_power()

[BECKHOFF_PLC,PALETTES,DFCONSTANTS,CONSTANTS,ENUM_MODES_HUB,
    UNITDEFAULTCOLORS,PLC_INDICATOR_TAGS,DFTAGCOLORCODE,
    USEFUL_TAGS,TAGS_FOR_INDICATORS] = smallpower_objs
