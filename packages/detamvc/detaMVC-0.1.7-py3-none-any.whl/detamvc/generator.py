...
def build_base(p, project_name):
    makedirs(p)
    content_attrs = {'Obj': project_name.title()}
    __builder(
        project_path=p, 
...
def __create_form_attrs_orig(attributes):
    # TODO: enhance the forms created for various data types
    # textarea, color, date...
    fa = list()
    for f, t in attributes.items():

        fa.append(f"""<label for="{f}">{f.title()}</label><br>""")

        # process datatypes as provided. Defaults to 'str' -> 'text'
        ty = "text"
        if t in ('int', 'float'):
            ty = "number"
        elif t == 'text':
        ...
        # create corresponding field for input based on type
        if ty in ("text", "number"):
            fld = f"""<input type="{ty}" name="{f}" class="{'form-check-input' if ty == 'checkbox' else 'form-control'}" value="{{{{vals.get('{f}', '')}}}}"{ ' step="0.01"' if t == 'float' else ''}><br>"""
        elif ty == 'textarea':
            fld = f"""<textarea name="{f}" class="form-control">{{{{vals.get('{f}', '')}}}}</textarea><br>"""
        elif ty == 'bool-select':
            fld = f"""<select name="{f}" class="form-select">
